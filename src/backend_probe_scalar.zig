...
pub const marker_value: u32 = 0x4442_3411;

comptime {
    if (builtin.cpu.arch != .x86_64 or builtin.os.tag != .windows) {
        @compileError("scalar backend probe requires Windows x86-64");
    }
    ...
    {
        @compileError("scalar backend probe contains a gated CPU feature");
    }
}
...
test "scalar backend marker returns the expected value" {
