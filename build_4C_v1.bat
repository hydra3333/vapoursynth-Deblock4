...
set "TRANSCRIPT_TEMP=%TEMP%\Deblock4_build_4C_v1_%RANDOM%_%RANDOM%_full_output.log"
"%ComSpec%" /D /V:OFF /C call "%~f0" --worker >"%TRANSCRIPT_TEMP%" 2>&1
set "OUTER_EXIT_CODE=%ERRORLEVEL%"

set "TRANSCRIPT_DIR=%~dp0zig-out\inspection_4C"
if not exist "%TRANSCRIPT_DIR%" md "%TRANSCRIPT_DIR%" >nul 2>nul
set "TRANSCRIPT_FILE=%TRANSCRIPT_DIR%\build_4C_v1_full_output.log"
...
set "DIAGNOSTIC_INDEX=%TRANSCRIPT_DIR%\build_4C_v1_diagnostic_index.txt"
...
rem ============================================================================
rem Deblock4 Stage 4C complete proof matrix.
rem
rem Charter v1.27 C-DELIV-11: the already-reviewed 2C inline PowerShell and
rem resident prior-stage audit calls are retained, not rewritten. Stage 4C
rem adds no new PowerShell machinery; new 4C controls below use CMD plus the
rem existing portable-Python runner only for the temporary-tree mutation.
rem
rem Implements Stage 4C scope v1.2 section 7 (4C-T1..T5) and retains the
...
set "DEBLOCK4_INSPECTION_DIR=zig-out\inspection_4C"
set "DEBLOCK4_BUILD_ROOT=zig-out\stage_4c"
set "DEBLOCK4_CACHE_ROOT=.zig-cache\stage_4c"
...
where git >nul 2>nul || (echo ERROR: git.exe not found & exit /b 1)
...
if not defined DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER (
    echo ERROR: DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER must name the W3D-supplied absolute .cmd path.
    exit /b 1
)
if not exist "!DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER!" (
    echo ERROR: W3D Stage 4C differential runner not found: !DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER!
    exit /b 1
)
...
if /I not "!OBSERVED_ZIG_VERSION!"=="0.16.0" (
    echo ERROR: Stage 4C requires Zig 0.16.0.
    exit /b 1
)

echo === Prepare Stage 4C proof directories
...
if errorlevel 1 goto :fail
echo === Stage 4C proof directories ready
...
if errorlevel 1 goto :fail
call :run_stage4c_w3d_differential
if errorlevel 1 goto :fail
...
rem T-S5-3/T-S5-5 Debug force-down cases under the raised Classic v2 ceiling.
...
if errorlevel 1 goto :fail
call :run_stage4c_tail_perturbation
if errorlevel 1 goto :fail
...
>>"!DEBLOCK4_INSPECTION_DIR!\proof_matrix_summary.txt" echo STAGE4C T1 T2 T3 T4 T5 P3 K30 K31 PASS
...
set "E2E_DIR=%~1"
set "DEBLOCK4_FORCE_DOWN="
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" valid_auto x86_64_v2_with_sse41 STAGE_2C_CLASSIC_PASS "!E2E_DIR!\classic_valid_auto.txt" 1 1
...
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" valid_coercion x86_64_v2_with_sse41 STAGE_2C_CLASSIC_PASS "!E2E_DIR!\classic_valid_coercion.txt" 1 1
...
for %%C in (n02b) do (
    call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" %%C x86_64_v1_baseline STAGE_2C_EXPECTED_ERROR_PASS "!E2E_DIR!\classic_%%C.txt" 1 0
    if errorlevel 1 exit /b 1
)
...
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" n04 x86_64_v2_with_sse41 STAGE_2C_N04_PASS "!E2E_DIR!\classic_n04.txt" 1 1
call :find_present "N04 exact implementation cap summary" "!E2E_DIR!\classic_n04.txt" "tier=x86_64_v2_with_sse41 reason=intentionally-capped(x86_64_v2_with_sse41) actual=x86_64_v3_with_avx2"
exit /b !ERRORLEVEL!
...
for %%S in (deblock4_backend_probe deblock4_dll_probe deblock4_classic_v2_process_u8 deblock4_classic_v2_process_u16 !FORCE_EXPORT! !DIAG_EXPORT! !TRACE_EXPORT!) do (
...
:run_stage4c_w3d_differential
set "MARKER=4C-T2 and vspipe T4 W3D-owned scalar-v2 differential"
echo === !MARKER!
set "DEBLOCK4_PLUGIN_PATH=!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_BUILD_ROOT!\ReleaseFast\bin\Deblock4.dll"
set "DEBLOCK4_STAGE4C_INSPECTION_DIR=!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_INSPECTION_DIR!\T2_W3D_differential"
set "DEBLOCK4_STAGE4C_EXPECTED_VERSION=!IDENTITY_STRING!"
set "DEBLOCK4_STAGE4C_EXPECTED_V1=x86_64_v1_baseline"
set "DEBLOCK4_STAGE4C_EXPECTED_V2=x86_64_v2_with_sse41"
set "DEBLOCK4_STAGE4C_RUN_KIND=positive"
call :make_dir "!DEBLOCK4_STAGE4C_INSPECTION_DIR!"
set "CMD=call "!DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER!""
set "OUT=!DEBLOCK4_INSPECTION_DIR!\T2_W3D_differential\runner_output.txt"
call :capture_both
set "DEBLOCK4_STAGE4C_RUN_KIND="
exit /b !ERRORLEVEL!
...
:run_stage4c_tail_perturbation
set "MARKER=4C-T5 direct one-lane T1 hook present"
set "CMD=findstr direct N==1 tail sentinel"
findstr /C:"filterHorizontalTail(T, 1, actual.plane(), column_start, edge_y, 1, thresholds);" "src\classic_vector_backend.zig" >nul <nul
if errorlevel 1 (set "exit_code=1" & goto :fail)
echo === 4C-T5 direct one-lane T1 hook present

set "PERTURB_DIR=%TEMP%\Deblock4_4C_tail_perturb"
if exist "!PERTURB_DIR!" rmdir /s /q "!PERTURB_DIR!"
echo.
echo === 4C-T5 copy project to temporary tail-perturbation tree
robocopy "!DEBLOCK4_PROJECT_ROOT!" "!PERTURB_DIR!" /E /XD .git .zig-cache zig-out /NFL /NDL /NJH /NJS /NP
set "copy_code=!ERRORLEVEL!"
if !copy_code! GTR 7 (set "MARKER=4C-T5 copy perturbation tree" & set "CMD=robocopy" & set "exit_code=!copy_code!" & goto :fail)
set "DEBLOCK4_STAGE4C_MUTATE_FILE=!PERTURB_DIR!\src\classic_vector_backend.zig"
set "MUTATE_SCRIPT=%TEMP%\Deblock4_4C_tail_mutate_%RANDOM%_%RANDOM%.py"
>"!MUTATE_SCRIPT!" echo import os
>>"!MUTATE_SCRIPT!" echo from pathlib import Path
>>"!MUTATE_SCRIPT!" echo p = Path(os.environ["DEBLOCK4_STAGE4C_MUTATE_FILE"])
...
>>"!MUTATE_SCRIPT!" echo mutated = data.replace(old, new, 1)
>>"!MUTATE_SCRIPT!" echo assert not mutated == data
>>"!MUTATE_SCRIPT!" echo p.write_bytes(mutated)
>>"!MUTATE_SCRIPT!" echo print("STAGE_4C_T5_MUTATION_APPLIED")
set "MARKER=4C-T5 apply one-lane tail mutation to temporary copy"
set "CMD=call tools\run_vs.cmd --python-script "!MUTATE_SCRIPT!""
call :run
del /Q "!MUTATE_SCRIPT!" >nul 2>nul
set "OUT=!DEBLOCK4_INSPECTION_DIR!\T5_tail_mutant_T1_expected_failure.txt"
pushd "!PERTURB_DIR!"
zig build test-classic-v2 --prefix "zig-out\stage_4c_mutant" --cache-dir ".zig-cache\stage_4c_mutant" -Doptimize=ReleaseFast -Denable_force_down=false -Denable_verbose_detection=false -Denable_trace_lifecycle=false --error-style verbose --summary all > "!DEBLOCK4_PROJECT_PREFIX!!OUT!" 2>&1
set "mutant_t1_code=!ERRORLEVEL!"
popd
if "!mutant_t1_code!"=="0" (set "MARKER=4C-T5 mutant T1 expected rejection" & set "CMD=zig build test-classic-v2" & set "exit_code=1" & goto :fail)
type "!OUT!"
set "MUTANT_PREFIX=!PERTURB_DIR!\zig-out\stage_4c_mutant_dll"
set "MUTANT_CACHE=!PERTURB_DIR!\.zig-cache\stage_4c_mutant_dll"
pushd "!PERTURB_DIR!"
zig build --prefix "!MUTANT_PREFIX!" --cache-dir "!MUTANT_CACHE!" -Doptimize=ReleaseFast -Denable_force_down=false -Denable_verbose_detection=false -Denable_trace_lifecycle=false --error-style verbose > "!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_INSPECTION_DIR!\T5_tail_mutant_build.txt" 2>&1
set "mutant_build_code=!ERRORLEVEL!"
popd
if not "!mutant_build_code!"=="0" (set "MARKER=4C-T5 mutant production build" & set "CMD=zig build" & set "exit_code=!mutant_build_code!" & goto :fail)
set "DEBLOCK4_PLUGIN_PATH=!MUTANT_PREFIX!\bin\Deblock4.dll"
set "DEBLOCK4_STAGE4C_INSPECTION_DIR=!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_INSPECTION_DIR!\T5_tail_mutant_T2"
set "DEBLOCK4_STAGE4C_EXPECTED_VERSION=!IDENTITY_STRING!"
set "DEBLOCK4_STAGE4C_EXPECTED_V1=x86_64_v1_baseline"
set "DEBLOCK4_STAGE4C_EXPECTED_V2=x86_64_v2_with_sse41"
set "DEBLOCK4_STAGE4C_RUN_KIND=tail-mutant-expected-failure"
call :make_dir "!DEBLOCK4_STAGE4C_INSPECTION_DIR!"
set "OUT=!DEBLOCK4_INSPECTION_DIR!\T5_tail_mutant_T2\runner_expected_failure.txt"
call "!DEBLOCK4_STAGE4C_DIFFERENTIAL_RUNNER!" > "!OUT!" 2>&1
set "mutant_t2_code=!ERRORLEVEL!"
type "!OUT!"
set "DEBLOCK4_STAGE4C_RUN_KIND="
rem The W3D mutant runner owns its decisive semantics: it returns 0 only when
rem the Python differential returned the expected difference-detected class
rem and emitted STAGE_4C_TAIL_MUTANT_CORRECTLY_REJECTED.
if not "!mutant_t2_code!"=="0" (set "MARKER=4C-T5 mutant T2 expected rejection" & set "CMD=call W3D differential runner" & set "exit_code=!mutant_t2_code!" & goto :fail)
findstr /C:"STAGE_4C_TAIL_MUTANT_CORRECTLY_REJECTED" "!OUT!" >nul <nul
if not "!ERRORLEVEL!"=="0" (set "MARKER=4C-T5 mutant T2 rejection marker" & set "CMD=findstr STAGE_4C_TAIL_MUTANT_CORRECTLY_REJECTED" & set "exit_code=1" & goto :fail)
echo === 4C-T5 mutant T2 correctly rejected by W3D differential
rmdir /s /q "!PERTURB_DIR!"
if exist "!PERTURB_DIR!" exit /b 1
exit /b 0
