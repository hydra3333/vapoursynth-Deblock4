...
Controlling documents:
    ...
    PREVAILS over everything, including this header. NO VERSION IS PINNED
    HERE: this header carried a pinned charter version and it staled twice.
    ...
    Architecture (highest RATIFIED/APPLICABLE version). It SUPERSEDES
    Deblock4_MPEG2_Grid_Field_DCT_Knowledge. DO NOT reason from the
    superseded document.

    The remaining applicable knowledge documents hold established facts in
    their own domains; the pre-implementation knowledge sweep is a duty, not
    background. The stage scope names the bounded authority/knowledge set
    required for that stage.

Applicable-knowledge exclusions:
    Do NOT treat physical presence in dev_documentation as proof that a file
    is applicable project knowledge.

    - Any folder whose name begins "superseded" or
      "scheduled_for_deletion" is excluded.
    - T1\ is a WORKSHOP / PROCESS tree. Material may be drafted,
      consolidated or reviewed there, but it is NOT searchable/applicable/
      final project knowledge merely because it exists there. Knowledge that
      is to become applicable must be promoted OUT into the live
      dev_documentation tree at its proper canonical home.
    - GAIS_investigations\ is historical research material and is ignored
      for current T1 search and adjudication. Do not use raw GAIS material as
      a current project-knowledge authority or as the basis for a coding
      decision. Use the applicable non-GAIS project record carrying any
...
Environment (constant; W3X corrects at issuance if changed):
    ...
    x64 developer prompt (VsDevCmd -arch=amd64). The source EDITOR is
    Visual Studio Code with the Zig extensions (the Visual Studio Zig
    extension is not used); this affects editing only - every build/
    validation command in this project assumes the Visual Studio developer
    prompt.
...
State at this writing:
    ...
    has NO filtering kernel - its dispatch arms are pass-through copies.

    No Deblock4 kernel scope may be drafted before the D4-Q14 experiment
    reports and W3X ratifies the architecture allowed to enter kernel/oracle
    development.

    The current high-level work sequence is:
        T1 documentation consolidation
        -> T8 provenance-gap closure
        -> T5 detector mathematics
        -> T6 / D4-Q14 planning as a separate ratification.

    If this header is consulted while T1 is still running, the HIGHEST
    COMMITTED Deblock4_T1_Resume_Brief section 0a is the live recovery/task
    handoff and prevails over older status/orientation summaries. At the
    2026-08-19 recovery point, T1S01a5 Classification Repair v1.1 is complete
    from W3D's side and the rewritten T1S01a5 ledger is the next designer
    artifact. This is document-review work, not a coding scope.
...
*Revision history*
```text
v1.4 (2026-08-19) Recovery/currency update after T1 search/adjudication rules
     and the downstream sequence changed. Adds the explicit applicable-
     knowledge exclusions: superseded*/scheduled_for_deletion*, T1\ as a
     process/workshop tree rather than applicable knowledge, and
     GAIS_investigations\ as historical research ignored for current T1
     search/adjudication. Updates the live high-level sequence to
     T1 -> T8 -> T5 -> separately-ratified T6/Q14 and makes the highest
     T1 Resume Brief section 0a the live handoff while T1 is running. Records
     the 2026-08-19 recovery point only to prevent a replacement session from
     restarting the already-completed Classification Repair v1.1. No source,
     environment, identity, delivery or kernel-authority rule changed.
v1.3 (2026-08-18) Removes the pinned charter version from the controlling-
     documents block; the charter is now named only as the highest committed
     version. That pin had staled twice.
     STANDING REMINDER, because this header is a CODING-session artifact and
     no coding session is currently possible: no Deblock4 kernel scope may be
     drafted before D4-Q14 reports and W3X ratifies the architecture allowed
     to enter kernel development. If W3X issues this header, W3X corrects any
     identity and generation values at issuance.
...
v1.1 (2026-08-14) Environment clarification (W3X): Visual Studio 2026
     developer prompt for builds; Visual Studio Code with Zig extensions as
     the editor.
...
*Revision note*
```text
v1.4 is intentionally a SMALL bootstrap-header change. The detailed T1 state,
ledger findings and review history belong in the T1 Resume Brief / task
register, not in a coding-session header. The one current T1 endpoint is
included only so a successor does not mistake the absence of a coding scope
for permission to restart an already-completed document-search round.
```
