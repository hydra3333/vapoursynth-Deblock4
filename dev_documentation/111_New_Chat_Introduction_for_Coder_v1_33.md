# Deblock4 - New Chat Introduction for Coder (v1.33)
...
IF YOU ARE REPLACING A CODER CHAT THAT DIED DURING T1, START HERE:
    ...
    It is the maintained task list: which sub-tranches are closed, which is
    live, and what is owed. IT PREVAILS OVER THE CURRENT-STATE SUMMARY IN
    THIS INTRODUCTION, because the brief is bumped every time a sub-tranche
    is issued or closed and this document is not.

    NO REVIEW YOU HAVE DELIVERED IS EVER LOST. Every verdict W3C has
    returned is preserved in the task register's decision log with its
    reason, and usually with a record of what the designer got wrong and
    what you supplied instead. You are never asked to redo a delivered
    review.
...
which is pinned for the reason given at item 2:

    1. Deblock4_Project_Status, section 0
    2. Deblock4_MPEG2_Deblocking_Investigation_and_Decided_Architecture_v1_05.md
       ...
       every ledger entry is a PROPOSAL awaiting W3X. A higher generation
       means STOP and ask W3X what was ratified.
    3. Deblock4_Standing_Task_Register_T_Series - the live work queue and
       its DECISION LOG
    4. Deblock4_T1_W3C_Review_Scope - what binds YOU during T1
    5. the current charter / active bounded scope when W3X issues one
...
orientation aligned to the HIGHEST COMMITTED VERSION of Project Status, the
charter and the README, and to the W3X-ratified MPEG-2 authority at v1.05 -
the one deliberate pin. The charter and the MPEG-2 authority prevail in
...
3. That you hold the HIGHEST COMMITTED Project Status as the live-state
   record, and - if T1 is running - the HIGHEST COMMITTED T1 resume brief,
   whose section 0a prevails over any state summary in this document.
...
2. Confirmation that you hold the HIGHEST COMMITTED Project Status as the live
   state and that MPEG-2 authority v1.05 is the W3X-ratified source of truth
   - still v1.05 deliberately, because no T1 remedy has been applied to it.
...
4. Immediate action: THE T1 DOCUMENTATION SWEEP - independent review of the
   designer's ledger adjudications, NOT code and NOT kernel implementation.
   T1 runs BEFORE T5 and T6/Q14; W3X reversed that sequence on 2026-08-17.
   Take the live sub-tranche from the resume brief's section 0a, not from
   this document. ANY OLDER TEXT SAYING Q14 OR T5 IS THE IMMEDIATE ACTION IS
   STALE.
