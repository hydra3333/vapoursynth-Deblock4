# Deblock4 - New Chat Introduction for Coder (v1.28)
...
CODER-SPECIFIC CONSEQUENCES:
 - Do NOT implement Deblock4 filtering mathematics under any existing
   document. No kernel scope exists; the next artifact is an experiment.
 - Older text describing field-separated input, mpeg2_field_separated,
   luma_step/chroma_step midpoint machinery or midpoint_threshold_scale
   describes a REJECTED design. midpoint_threshold_scale is orphaned as a
   mechanism; its parameter fate is a kernel-scope question.
 - The current Deblock4LumaStepY / midpoint audit-property model CANNOT
   express mixed B2 geometry as a single per-frame string; expect it to
   change.
 - Verify every external factual claim (specs, source, patents, papers)
   against the actual source before it enters the record. The GAIS
   calibration record above is why.
```
