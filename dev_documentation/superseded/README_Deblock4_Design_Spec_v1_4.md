...
**Design specification revision:** 1.4  
**Date:** 2026-07-24  
...
`Deblock4`, but it has NO grid selection and NO midpoint machinery - its grid is
the fixed H.264 4x4 boundary set, not a selectable MPEG-2-aware grid.
...
it emerges from the same tables. `Classic` provides equal-or-better output to
HolyWu's Deblock at matched parameters as a same-algorithm reference, subject to
the project's own verification (integer-exact backends; float within tolerance),
and HolyWu's plugin may be used as an external cross-check oracle during
development.
...
Backend selection is not a single global decision. The public API exposes
`backend` as a per-filter-instance parameter, so two filter instances in one
script may legitimately resolve to different backends.

The correct decomposition is:

    global, once
        CPU and operating-system capability discovery, into an immutable
        process/plugin-wide capability record.
    ...
    per frame
        no capability test, no backend selection, no branch on backend.

A function-pointer call in the frame path is fine. A feature-test branch or a
backend-choice branch in the frame path is not.

Capability discovery uses a correctly synchronised one-time initialiser.
Per-instance entry points are immutable after creation and are therefore safe
for concurrent fmParallel frame processing without further synchronisation.
...
**Two filters, Classic first (v1.4).** The plugin ships two filters (section
...
first regardless. Stages 2-5 (canonical scalar core, quality decisions,
SSE4.1, AVX2) describe how ONE algorithm is built and are executed TWICE: first
for `Classic`, then for `Deblock4`. `Classic` leads because it is a known,
externally-referenceable algorithm, so it proves the harness and the R76/G9
guard (section 1.0 rationale) before the novel MPEG-2 algorithm is attempted.
`Deblock4` then reuses the proven shared infrastructure and repeats only the
algorithm-specific stages. Stage 6 (integration/release) covers both filters.
The MPEG-2 `Deblock4` filter remains the project's end goal; Classic-first is a
de-risking SEQUENCE choice, not a change of objective.
