...
```text
Current position (2026-08-03, Stage 2C design RELEASED; verify against repo):
    ...
    STAGE 2C (Classic scalar oracle + HolyWu differential): DESIGN ARC
    COMPLETE AND RELEASED. Seven W3C review rounds resolved (D2 F1-F12;
    revision F1-F7; D3 F1-F8; D3-followup F1; combined F1-F9+Q4/Q5;
    revised-package F1-F8; updated-package F1-F6; v1.4-package F1-F6;
    v1.5-package convergence F1-F3). W3X called the v1.5-package review
    FINAL, ratified and released the scope. IMPLEMENTATION IS IN FLIGHT:
    W3C is authoring the 2C delivery against the released contract.

    RELEASED 2C AUTHORITY SET (dev_documentation/reference/ - always cite
    the highest committed version; these were the release versions):
      D0  ..._D0_Preface_and_Binding_Knowledge_Index_v1_9.md
          LIVING index K1-K29. Highlights: K19 three-layer comparison;
          K26 external-oracle execution pin (signed-shift sentinels:
          B2/B5 negative-LEFT-shift C++-UB probes, B4 negative-RIGHT-
          shift side-delta probe; mandatory reference-DLL SHA-256;
          rebuild=new artefact); K27 destination/property contract
          ("identical" = PLANE BYTES; audit props still written); K28
          actual per-plane geometry (never inferred chroma bounds); K29
          valid integer depths extend to 32 (refuse 17..32 explicitly).
          Section 5 = settled policies + the narrow 2C exception (five
          authorised items ONLY). Section 6 = the two-sided knowledge-
          sweep protocol; its 6.1 block goes verbatim in every scope.
      D1  holywu_r9/ pinned r9 snapshot (SHA256SUMS; LF bytes preserved;
          W3X-owned read-only) + README_provenance v1.4 (H0 external-
          reference tool is the SOLE authorised reader).
      D2  ..._D2_HolyWu_Real_Schedule_v1_6.md - thrice-verified line-by-
          line source account; Schedule A; footprints; tables 61/61;
          WP-1..WP-6; T-1 tabled to 3C; T-2/T-3 RESOLVED.
      D3  ..._D3_Scalar_Obligations_and_Sanity_Gate_v1_8.md - THE
          acceptance basis: O-1..O-8 (incl. O-1c 73 tuples + bits 8..16,
          O-1d integer-depth refusal, O-7 boundary a/b/c/c2, O-8
          production routing), gate G1-G6 + constant-fill negative
          control, 7d mandatory O/G-to-test crosswalk, section 9b Open
          Rule Questions register, Appendices A/B/C derivation models.
      D4  Deblock4_Scope_Stage_2C_D4_Classic_Scalar_Oracle_v1_6.md - the
          released coder scope: ratified decisions S1-S7 (S1 integer-only
          + float refusal; S2 retain clamps; S3 legal-shared-domain
          differential; S4 oracle-construction exception - POST-
          ACCEPTANCE THE DELIVERED SCALAR PATH IS THE ORACLE; S5
          implemented-tier availability + precedence + always-on
          reason= line; S6 identity -> 0.1.0-dev+2C; S7 testable K24);
          A-requirements incl. A1b format/storage (u8 at 8-bit, u16 at
          9..16, bitsPerSample drives scale/peak); harness H0-H6 (W3C
          delivers scripts/guards, W3X generates DLL/record/sentinel
          evidence - evidence is NOT committed source); 7b authorised
          integration surfaces (closed list); 7c SUCCESSOR-MATRIX runner
          (build_2C_v1.bat re-runs still-applicable 1C gates expecting
          +2C; build_1C_v1.bat = immutable history, never run on the 2C
          tree); section 10 register.
      Addendum A v1_2 - six K26 sentinel fixtures (16x8, flat filler is
          self-neutralising; whole-frame exactness self-check MANDATORY:
          output differs in EXACTLY the 4 readback cells).
      Addendum B v1_2 - authority-fixed corpus: C01-C17 (closed-form
          checkerboards, model-verified non-vacuity counts, subsampled
          420 AND 422 chroma, depths 8/10/12/16, second geometry) +
          N-cases (N01a/b float 16+32-bit; N01c1/c2 integer 17/32-bit
          via query_video_format + exhaustive 17..32 guard; N02a/b
          explicit v2 AND v3 unavailable; N03 EFFECTIVE precedence; N04
          fixed auto-caps case).
      Creation-error table v1_6 - ratified and controlling; THREE 2C
          Classic rows (float; integer-depth; backend-unavailable) with
          the three-part integer-depth proof recorded.

    Open Rule Questions state: Q1 CLOSED on release (S2 ratified). Q2
    CLOSED as subsumed by W3X's standing instruction to W3C (report only
    materially significant findings) plus W3C's bounded-next-review
    discipline. Q3 remains a WATCH item (trigger: two consecutive clean
    sweeps). New entries go in the register (D3 9b / D4 section 10),
    never raised ad hoc.

Immediate designer action:
    The design is RELEASED - do not reopen it. W3D's next duties arrive
    when the W3C 2C delivery lands: (1) static review of the delivery
    against D3 v1_8 obligation-by-obligation via the mandatory 7d
    crosswalk; (2) verify K17 packaging (self-contained, touched-paths
    hashes, scoped restore block, superseded/ untouched, generated
    reference evidence NOT staged as source); (3) confirm the successor-
    matrix runner composition and the +2C identity sweep; (4) inspect
    W3X's reference-build record + sentinel observations against
    Addendum A expected values (mismatch = HARD STOP, never rewrite D3);
    (5) derivation questions re-answerable from D3 Appendices A/B/C
    (transparent Python models - re-create and run them, they are the
    fastest way to check any expected value). Read D0 (latest) FIRST.
    ...
    (8) The 2C endgame lesson: late-round findings were almost entirely
    INCOMPLETE PROPAGATION of agreed fixes (counts, cross-references,
    version pointers), not new defects. When editing a multi-document
    set, grep-before-edit, then run a full-set residual sweep for stale
    pointers/counts BEFORE delivering - and treat "update X" as "update
    X and everything that cites X".
```
