...
WHERE TO GO INSTEAD:
    ...
    T1 sweep state ........ Deblock4_T1_Resume_Brief SECTION 0a (latest) -
                            the maintained task list: which sub-tranche is
                            live, which are closed, and what is owed
...
*Revision note*
```text
v1.22 (2026-08-18) Banner routing only; NO roadmap content changed and none
     should be trusted as current. Adds the T1 resume brief's section 0a to
     the where-to-go-instead list, since it is the maintained record of the
     sweep's state and was reachable only through the task register. The
     existing do-not-skip-this-document clause is unchanged and still binds:
     this document remains in the T1 sweep population.
```
