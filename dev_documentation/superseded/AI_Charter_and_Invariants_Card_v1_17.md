...
G10 DEBUG-ONLY CODE IS STRUCTURALLY ABSENT FROM PRODUCTION BINARIES, BY THE
    ...
    visible choice in the build invocation.
    ...
    may only REDUCE (force-down by masking real capability); it must be
    structurally unable to fabricate capability the CPU lacks. G5's no-bypass
    rule is unchanged: a debug seam is never a route around the guard.
```
