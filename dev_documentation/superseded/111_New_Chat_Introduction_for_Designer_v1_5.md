...
Immediate designer action:
    The next bounded work is the FILTER-CREATION stage: the VapourSynth entry
    point, the scaffolding sweep (retiring the probe/smoke/dll_probe files and
    the remaining LF holdouts, per C-STY-10's sweep test), and dispatch wiring
    that CONSUMES the EFFECTIVE capability record 1B.3 built and proved. When
    W3X asks, author that scope against charter v1.19 and scope discipline.
    Three hard-won lessons to carry forward. (1) From 1B.1: G6 bans PE-EXPORT
...
```text
1. AI_Charter_and_Invariants_Card_v1_16.md          CONTROLLING
...
3. Deblock4_Verification_And_Tiering_Decisions_v1_9.md   INFORMATIVE DURABLE
...
4. Deblock4_Project_Status (latest version)         INFORMATIVE
   Read for the current proof state, what is done, what is open, and why
   Stage 1B.3 is complete and committed; the filter-creation stage is next.
...
6. Deblock4_Toolchain_Findings (latest version)     INFORMATIVE
   ...
   the proven multi-feature-level dispatch idiom that Stage 1B.3 will follow.

7. Deblock4_DISPATCH_RELATED_Backend_Objects_Explained (latest)  INFORMATIVE
   Read for the consolidated explanation of the backend-object / dispatch
   structure (how the named-level objects, @extern anchors, and future 1B.3
   dispatch table fit together).

8. Deblock4_MPEG2_Grid_Field_DCT_Knowledge (latest version)  INFORMATIVE
...
9. Deblock4_Concise_Project_Summary_v1.1.md         INFORMATIVE
   Compact cross-check that your mental model matches the user-facing shape.

10. 111_New_Chat_Introduction_for_Coder (latest)    INFORMATIVE
...
CONFIRMS each object stays within its level; it does not derive a closure.
...
included in the v3 target but not RELIED UPON, and Stage 1B.2 must not expect
FMA emission. A later explicit decision could introduce fused semantics.
...
- Stage 1B.2 CONFIRMS within-level and PRODUCES requirements; Stage 1B.3
...
- Stage 1B.2 within-level assembly confirmation and vzeroupper (NOT "feature
  closures" - the level is the contract)
- Stage 1B.3 runtime detection: ONE mechanism for target + detection if Zig
  0.16 exposes a stable named-level definition; else a project-owned canonical
  level descriptor with an explicit CPUID/XGETBV detector derived from it
  (explicit unit is a FALLBACK, not the presumed outcome)
...
1. The exact current ratified charter and README versions in the repo. This
   file says charter v1.19 / README v1.9; confirm nothing newer has superseded
...
2. Whether the Project Status document reflects the latest Stage 1B progress
   ...
   creation stage next).
3. The state of the Stage 1B.3 W3C delivery: it exists and awaits designer
   review; confirm whether it has been reviewed/accepted/committed yet.
4. The precise state of the coder handover - the coder introduction is current
   and reconciled here; confirm the coder chat's first-orientation report. Note
   the 1B.3 coder chat died after delivering; a fresh coder chat may be needed
   for any rework.
...
3. Current position: Stages 1A, 1A.1, 1B.1, 1B.2 complete; design round
   closed; charter v1.19, README v1.9 (or whatever you actually received -
   state it); Stages 1B.2 and 1B.3 complete and committed.
4. Immediate designer action:
    Author the filter-creation scope when W3X asks (VS entry point, scaffolding
    sweep, dispatch wiring consuming the EFFECTIVE record), against charter
    v1.19.
