# Deblock4 - New Chat Introduction for Designer (v1.21)
...
```text
DEBLOCK4 MPEG-2 - PREVAILING AUTHORITY (ADDED 2026-08-16, READ THIS FIRST)

For ALL MPEG-2 deblocking matters - block geometry, field/frame coding,
grid modes, chroma organisation, prior art, Deblock4 architecture - the
PREVAILING AUTHORITY is:

    Deblock4_MPEG2_Deblocking_Investigation_and_Decided_Architecture
    (latest version; read its SECTION 0 "CURRENT ARCHITECTURE POSITION")

It SUPERSEDES Deblock4_MPEG2_Grid_Field_DCT_Knowledge and PREVAILS over
every raw GAIS response and over any MPEG-2 statement elsewhere in this
documentation set INCLUDING THIS INTRODUCTION. Where this introduction
and that document disagree, THAT DOCUMENT WINS.

STATE: Deblock4 has NO kernel - its dispatch arms are validated
pass-throughs. The architecture is DECIDED but GATED on a measurement not
yet run (the D4-Q14 discriminator experiment). NO KERNEL SCOPE MAY BE
DRAFTED BEFORE IT REPORTS.

WHAT CHANGED ON 2026-08-16 (invalidating older statements here):
 - INPUT CONTRACT: the whole INTERLEAVED FRAME plus a three-value declared
   mode (progressive / frame-coded-interlaced / field-pictures).
   SeparateFields is NOT SUPPORTED - it tears frame-organised blocks
   across two clips. Any text describing field-separated input, or the
   "mpeg2_field_separated" parameter, is SUPERSEDED.
 - 4:2:0 CHROMA is ALWAYS frame-organised (8 consecutive chroma rows) even
   under field DCT (H.262-VERIFIED; closes checklist item G1).
   4:2:2/4:4:4 chroma FOLLOWS luma and cannot inherit that simplification.
 - ARCHITECTURE: B2 (classify each 16x16 macroblock FRAME/FIELD/UNKNOWN,
   then DERIVE edge topology; mixed-neighbour seams are an explicit edge
   type) is PRIMARY; D (detector-free, conservative) is the MANDATORY
   fallback and comparator. The union step-4 / midpoint-threshold design
   (Architecture A) is REJECTED - its geometry was an artefact of
   separated-field coordinates, and its activation premise has a proven
   in-principle false-activation floor. Motion-based classification (C)
   is rejected.
 - VERTICAL EDGES are geometry-invariant at x = 8k in every case and
   filter within a row; frame-vs-field DCT changes HORIZONTAL edge
   geometry only. SIMD is not impeded.
 - NOTHING is inherited from Classic - not code, not proofs, not
   thresholds. Deblock4 gets its own oracle, fixtures and proof chain.

STANDING RULE - EXTERNAL RESEARCH: GAIS output is a REASONING AID ONLY.
Its citation record in this project: five patent citations, ALL
misattributed; two paper citations, one refuted and one distorted; a
false claim of having read source code; one self-contradiction. Its
reasoning about METHOD was valuable. No factual claim, citation or
quotation from it enters project knowledge without independent
verification.
```

```text
DESIGNER-SPECIFIC LESSONS FROM THE 2026-08-16 ARC (both W3D failures):
 1  SWEEP SPECIFICATIONS, NOT JUST DECISIONS. A fully ratified Deblock4
    grid architecture sat unread in README_Deblock4_Design_Spec sections
    3.11/3.13 through four rounds of external investigation, because W3D
    had classified that document as "fallback general guidance". ANY
    knowledge sweep MUST include specification documents. A W3X ruling on
    the README's status is pending for exactly this reason.
 2  STATE RECOMMENDATIONS AS POSITIONS TO TEST. W3D was wrong four times
    in this arc (an unachievable byte-identity proof predicate; a casual
    temporal-hysteresis proposal that would have broken fmParallel
    determinism; and two pro-Architecture-A arguments - threshold scaling
    is a HARD decision with a tunable threshold, not soft confidence, and
    it avoids classifier flicker but NOT threshold flicker). W3C caught
    every one before cost was incurred. That is the process working;
    write recommendations so it can keep working.

POSTPONED PROCESS WORK (do not start unprompted; W3X paused it):
    T1 consolidation sweep of the 17 MPEG-2-bearing documents (only
    README 3.11/3.13 read so far); T2 retire the Grid Knowledge doc with
    its currency-audit edit; T3 de-duplicate MPEG-2 content into
    references; T4 boundary-set mathematics; T5 detector mathematics;
    T6 the D4-Q14 experiment plan (depends on T5); T7 commit.
```
...
v1.21 (2026-08-16) Added the DEBLOCK4 MPEG-2 PREVAILING AUTHORITY block and
designer-specific lessons to IMMEDIATE ORIENTATION; marked section 3.11
SUPERSEDED IN MECHANISM while preserving its three still-true items
(notably the measured LG-recorder regime-3 fact). No other change.
...
```text
*** SUPERSEDED IN MECHANISM, 2026-08-16 - READ THIS BEFORE THE SECTION ***
Everything below assumes SEPARATED-FIELD input and the midpoint machinery.
Both are superseded: input is now the whole interleaved frame (the
midpoint/union design, Architecture A, was evaluated and REJECTED). See
the MPEG-2 Deblocking Investigation and Decided Architecture document.

THREE THINGS BELOW REMAIN TRUE AND VALUABLE - do not lose them:
 (a) MEASURED REAL-SOURCE FACT: the target LG VHS-to-DVD recorder
     produces REGIME 3 (adaptive per-macroblock frame/field DCT mix) in
     every practical speed mode (XP/SP/LP/EP); only MLS hard-sets
     frame-DCT. MIXED MATERIAL IS THEREFORE THE NORMAL CASE for this
     project's target footage, not a hypothetical. This is why a
     per-macroblock architecture (B2) is being pursued at all.
 (b) The three regimes are distinguished by frame_pred_frame_dct and
     picture_structure, and frame_pred_frame_dct is readable per frame
     via mediainfo --Details=1 - directly useful for the D4-Q14
     ground-truth experiment.
 (c) The encoder chooses per macroblock, so no single uniform vertical
     grid is correct everywhere in regime 3. That premise SURVIVES; only
     the midpoint ANSWER to it was rejected.
```
