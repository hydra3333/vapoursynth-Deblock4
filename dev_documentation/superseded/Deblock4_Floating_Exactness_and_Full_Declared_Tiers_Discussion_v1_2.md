...
**Revision note:** v1.2 is a substantive revision. It records the decisions and
preferences reached after v1.0, promotes them from exploratory possibilities
to W3X's current position for designer review, removes questions already
answered, and narrows the remaining open work to tolerance design, exact
feature closures, and resulting document amendments. v1.1 changed only the
header and is superseded.
...
11. Stage 1B.2 should determine the actual full declared feature closure of
    each backend. It should not subtract FMA merely for identity, and it should
    not require FMA instructions to appear when strict source semantics do not
    request a fused operation. 
    --- This item now appears as it it may not have  been incorrectly stated, refer to
    items 1,2,3 for decisions about this; there were substantial problems with
    the designer chat at the time this document was created until it ceased to function,
    so it is conjectured this document was not fully updated correctly - a substantial
    portion of the tail of that chat was completely lost (including complex discussion and a range
    of decisions arising including in relation to coder's questions). The end-point was
    that this project code would check for v3 tier first and if the cpu satisfied that then use
    it in full, if it failed then fallback to tier 2 and do the same with fallback to
    tier 1 and use tier 1 if tier 2 was not satisfied.
...
> relation to the restoration objective.
>
...
Stage 1B.2 must determine:
000
- the complete feature closure of each object;
- which features are explicit, inherited, added, or subtracted;
- whether a named architecture level accurately represents the intended tier;
- the exact runtime checks required;
- representative emitted instruction classes;
- absence of instructions outside the declared tier.

Stage 1B.2 should not require an FMA instruction merely because the feature is
available, unless the compiler chooses to use it as a result of us specifiying
the tier used to compile a module. If strict source arithmetic does not
request a fused operation, its absence is legitimate and the decision is up
to the compiler.
...
12. Stage 1B.2 measures one closure per backend;
13. tolerance design is deferred until real kernels exist.

## 10.2 Remaining open questions

1. Do integer outputs remain exact across backends?
2. What exact features comprise the scalar, SSE4.1, and AVX2 tiers?
3. Does the AVX2 tier include FMA, BMI1, BMI2, F16C, LZCNT, POPCNT, or another
   bundled architecture-level feature set?
4. Should explicit feature sets or a named architecture level be used?
5. Which float internal masks and decisions must remain exact?
6. What numerical metrics and tolerance values become normative?
7. What corpus and analytical proof are required?
8. What reproducibility promise is made for float output across machines?
9. Should frame properties or diagnostics record the selected backend and tier?
10. Which charter, README, roadmap, and harness clauses must be amended?
...
1. establish one full declared closure for scalar, SSE4.1, and AVX2;
2. compile representative probes under those exact contracts;
3. inspect object-level emitted instructions;
4. prove baseline code contains no gated instructions;
5. prove each runtime guard matches the complete object closure;
6. retain FMA if it belongs to the chosen AVX2 tier rather than subtracting it
   for identity;
...
The prior two-closure-per-backend form is no longer the preferred plan.
