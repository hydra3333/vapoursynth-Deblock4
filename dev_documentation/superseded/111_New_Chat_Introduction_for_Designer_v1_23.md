# Deblock4 - New Chat Introduction for Designer (v1.23)
...
READ THIS SEQUENCE FIRST AFTER ANY DESIGNER-CHAT RESET:
    ...
    Project Status v1.28 section 0
        ...
        -> Standing Task Register v1.1
        -> T5 detector mathematics
        -> T6 D4-Q14 experiment plan / current W3X task
...
DESIGNER ROLE NOW:
 ...
 - Next designer work begins with T5: derive the detector/feature mathematics
   that Q14 will measure. T6 is the experiment plan immediately after it. W3X
   may permit one coordinated T5+T6 package, but the T5 mathematics and
   predeclared criteria must be frozen/ratified before held-out judgement.
 - The earlier T1 formal 17-document consolidation sweep remains paused by W3X.
   Targeted v1.03-v1.05 consolidation recovered load-bearing material but did
   NOT discharge T1.
...
2a. Deblock4_Standing_Task_Register_T_Series_v1_1.md (or later) - LIVE WORK QUEUE
    Read immediately after the MPEG-2 authority. It carries T1-T7 dependencies,
    including T1 paused, T5 first, then T6. The authority controls decisions;
    the register controls the outstanding work queue.
...
```text
- T5 detector/feature mathematics, frozen before held-out judgement
...
2. Confirm Project Status v1.28 (or later) section 0 and MPEG-2 authority v1.05
...
5. Immediate designer action: T5 detector/feature mathematics, then T6 Q14
   ...
   package is acceptable only if T5 is frozen before held-out judgement. T6
   must use per-MB truth, predeclared metrics, calibration/held-out separation,
   and evaluate B2 AND D honestly.
