...
phases per Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_0.md. Stage 1C registers
...
PHASE STATUS (read before coding): Phase 1 (the pure foundation -
deblock4_version, common_instance_data_structure, filter_call_parameters, and
the two instance-data records) has been DELIVERED, reviewed, and ACCEPTED
(green on the real toolchain: version 1/1, common-instance 2/2,
filter_call_parameters 9/9, the two instance-data roots compile clean, all in
Debug + ReleaseSafe + ReleaseFast; additions-only git status). The next bounded
work is PHASE 2: backend_tier_selection (+ its auto/explicit-ok/too-high/
invalid unit tests) and the two *_instance_creation modules with the
constant-format refusal. Read the whole scope for context; deliver ONLY the
...
(Stage 1B.3 complete; Stage 1C active - Phase 1 accepted, Phase 2 next).
...
## 2.8a `Scopes/Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_0.md` - CONTROLLING
...
Phase 1 (pure foundation) is delivered and accepted; Phase 2 (tier selection +
instance creation) is next; Phase 3 (frame path, plugin registration, sweep,
full proof) follows.
...
3. Current milestone: Stages 1A..1B.3 COMPLETE; Stage 1C ACTIVE - Phase 1
   (pure foundation) accepted, Phase 2 next; Stage 1 incomplete.
4. Immediate scope: STAGE 1C Phase 2 (backend_tier_selection + the two
   *_instance_creation modules + constant-format refusal), per scope v1_5 and
   the delivery addendum. Read the whole scope; deliver only the released
   phase. No pixel/deblocking code (that is 2C/2D).
...
Scopes/Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_0.md  (phase delivery order)
