# Deblock4 - New Chat Introduction for Designer (v1.28)
...
Standing Task Register, the scope manifest, the charter and the README.
...
- T1 CONSOLIDATION SWEEP - IN PROGRESS, and it comes before everything below.
  ...
  being adjudicated in DECLARED SUB-TRANCHES because a designer session that
  dies mid-document loses everything not yet delivered; the LAST sub-tranche
  is DECLARED AS FINAL because whole-document cross-entry consistency is
  checked there and NOWHERE EARLIER. NOTHING has yet been ratified into any
  authority document - every ledger remedy is a PROPOSAL.
  DO NOT TRUST A SUB-TRANCHE COUNT FROM ANY DOCUMENT BUT THE RESUME BRIEF.
  The structure has been extended once already, when it emerged that the
  authority's BODY had been assigned to "later sub-tranches" without any
  being named.
...
5. Immediate designer action: THE T1 CONSOLIDATION SWEEP, unless W3X directs
   ...
   0a - IT PREVAILS OVER THIS CHECKLIST AND OVER EVERY STATE SUMMARY IN THIS
   DOCUMENT. It is bumped whenever a sub-tranche is issued or closed; this
   document is not. If they disagree, the brief is right and you should tell
   W3X this document has staled.
   EXPECT LEDGER VERSION NUMBERS TO MISLEAD YOU: sub-tranches have run to
   four and five ledger versions with the TECHNICAL result unchanged from
   v1.0, the later rounds being designer method and framing defects that W3C
   found. The brief sets out those sequences for exactly that reason.
