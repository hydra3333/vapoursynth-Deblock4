...
**Version:** 1.3
**Date:** 2026-07-29
**Status:** Informative successor orientation; not controlling; aligned to ratified charter v1.16
...
```text
Current position:
    Stages 1A, 1A.1 and 1B.1 are complete and committed.
    Charter ratified at v1.16. README design spec at v1.9.
    ...
    (see Deblock4_Toolchain_Findings F1-F5). Stage 1B.2 is the active next
    stage: within-level object-code and assembly CONFIRMATION (each object
    stays inside its named psABI level), vzeroupper inspection, and PRODUCTION
    of the whole-level requirements that Stage 1B.3 will enforce.

Immediate designer action:
    Author the Stage 1B.2 scope when W3X asks, and review coder deliveries
    against charter v1.16 and README v1.9. The hard-won 1B.1 lesson to carry:
    G6 bans PE-EXPORT of gated code, NOT the Zig export keyword. Gated backend
    functions ARE declared export fn in their own single-target objects
    (emission + linkage, and object-mode export does not PE-export); the DLL
    root reaches them by @extern, address-taken and never called; absence from
    the export table is enforced by a standing dumpbin /EXPORTS gate. Do not
    let a successor re-derive the falsified alternatives.
...
4. Deblock4_Project_Status (latest version)         INFORMATIVE
   ...
   Stage 1B.2 is next. Non-controlling; the README and charter prevail.
...
1. The exact current ratified charter and README versions in the repo. This
   file says charter v1.16 / README v1.9; confirm nothing newer has superseded
...
2. Whether the Project Status document reflects the latest Stage 1B progress
   since this handover (it was current at v1.10 here).
3. Whether a formal Stage 1B.2 scope has yet been authored, and by whom.
4. The precise state of the coder handover - the coder introduction is current
   and reconciled here; confirm the coder chat's first-orientation report.
...
3. Current position: Stages 1A, 1A.1, 1B.1 complete; design round closed;
   charter v1.16, README v1.9 (or whatever you actually received - state it).
