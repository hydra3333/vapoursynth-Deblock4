...
Stage 5C      NEXT. Classic AVX2 backend: the same width-generic vector
              code at 256-bit, plus its own edge/tail proof and the
              performance measurements.
Stage 2D..5D  Deblock4 filter: scalar core (grids, schedules, midpoint,
              proper chroma), then its vector backends.
