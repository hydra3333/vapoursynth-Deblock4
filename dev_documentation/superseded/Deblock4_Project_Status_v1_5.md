...
excluded; under `.strict` it is present-but-unused, so 1B.2 must NOT expect FMA
emission. Stage 1B.2 PRODUCES the whole-level feature requirements; it does not
...
produces the whole-level feature requirements (the runtime guard that CHECKS
the level is a Stage 1B.3 artifact, not 1B.2); it does not derive a bespoke
closure. It requires no new
...
build then runs for Classic first, then Deblock4. See README v1.6 section 1.0.
