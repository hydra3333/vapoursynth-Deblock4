...
Charter:
    filename          AI_Charter_and_Invariants_Card_v1_17.md
    internal version  1.17
...
Starting commit:
    <commit hash>
    (RATIFIED ALTERNATIVE, recorded v1.18: W3X may instead supply the exact
     attached source tree as the implementation base, because documentation or
     other non-source material may be committed between coding scopes. The
     attached tree then IS the base; W3C treats it as exact and needs no
...
## 2.3a Version currency and paired documents - verify before relying
...
C-DELIV-01  EVERY DELIVERY IDENTIFIES ITS EXACT BASE AND SCOPE.
            ...
            The starting commit is the commit W3X reported at the beginning of
            the scope. W3C does not infer it from an earlier conversation or
            from the status document. Where W3X instead supplies the exact
            attached source tree as the base (the ratified alternative in the
            session bootstrap header, v1.18), the delivery identifies that
            exact package in place of the commit hash.

            A delivery that cannot identify its exact base is provisional and
            is not applied.
...
C-DELIV-03  PATCHES REQUIRE ANCHOR VERIFICATION BEFORE APPLICATION.
            ...
            Every patch states the exact starting commit or, for a separately
            ...
            Before applying a patch, W3X confirms:
                ...
                the repository is on the stated branch and starting commit;
            ...
            A patch that fails commit verification, anchor verification, or
...
C-DELIV-04  WHOLE-FILE DELIVERIES ARE COMPLETE AND PATH-EXPLICIT.
            ...
            A whole-file delivery states:
                ...
                exact starting commit for an existing-file replacement;
...
C-DELIV-08  MINIMUM APPLY AND VALIDATION DISCIPLINE.
            ...
            WHOLE-FILE DELIVERY:
                ...
                verify branch and starting commit;
...
C-DELIV-09  INCREMENTAL EMISSION FOR INTERRUPT-SAFETY AND REVIEW CONTINUITY.
            ...
            When processing any scope or phase that produces more than one
            module (or more than a few files), W3C EMITS completed work
            INCREMENTALLY to W3X as it is finished - each completed module or
            small coherent group of modules - rather than withholding all
            output until a single final package. Each increment carries a
            one-line marker of the form "increment N of ~M: <what>".

            PURPOSE (and its honest limit): the emitted increments are a
            resumable baseline if the session is interrupted, and running
            review checkpoints for W3X. ONLY DELIVERED ARTIFACTS SURVIVE an
            interruption. W3C must NOT claim to preserve, checkpoint, or
            resume un-emitted internal reasoning or partially-written work;
            the recoverable state is exactly the last COMPLETE increment
            emitted to W3X, and at most the module in flight is lost. W3C
            states this honestly and never implies finer-grained recovery
            than delivered output provides.

            THE FINAL DELIVERABLE IS UNCHANGED. Incremental emission is for
            continuity and interrupt-safety only; it does NOT replace the
            properly-packaged final deliverable. At the end of the scope or
            phase, W3C RE-PACKAGES the complete work as one final deliverable
            meeting C-DELIV-01..08 in full (exact base, per-file delivery
            form, manifest, apply-and-validate discipline, patch and/or
            whole-file plus the validation batch). The increments are a
            running preview; the packaged deliverable is the artifact of
            record that W3X applies.
```
