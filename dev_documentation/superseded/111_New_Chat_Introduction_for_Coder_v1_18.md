...
Deblock4_Scope_Stage_1C_Filter_Creation_v1_5.md (CONTROLLING) and delivered in
phases per Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_1.md. Stage 1C registers
...
toolchain), and COMMITTED. Phase 3 is split 3a/3b: PHASE 3a (the current bounded
work) = the frame path (common_frame_mechanics, common_frame_property_helpers,
classic_/deblock4_frame_properties, the tier-switch bodies in both
*_ar_all_frames_ready in the settled C5 order, common_ar_error,
lifecycle_trace_debug) PLUS real deblock4_plugin registration; stop for review.
PHASE 3b (after 3a accepted) = the scaffolding sweep (C-STY-10), build_1C_v1.bat,
the two tests/stage_1c_*_passthrough.vpy harnesses, and the full proof matrix.
Two pins for 3a: (a) the creation callbacks must expose the exact translated
VSPublicFunction C-ABI signature (the same [*c] correction the routers received
in Phase 2), body otherwise unchanged; (b) do NOT restructure the permanent
activation-reason switch - only replace its minimal branch bodies with the
settled C5-order handlers. Read the whole scope for context; deliver ONLY the
phase W3X releases, and stop at its boundary. W3X releases phases, not W3C.
...
code in Stage 1C - that is the per-algorithm 2C/2D+ work. Stage 1C wires the
entry point and per-filter dispatch (consuming the EFFECTIVE record) but still
executes no gated backend arithmetic until the per-filter scalar oracle exists;
G5 continues to govern.
...
## 2.1 `Deblock4_Project_Status_v1_15.md` - INFORMATIVE

Read first for the reported proof state, open work, and the current position
(Stage 1B.3 complete; Stage 1C active - Phases 1+2 committed, Phase 3a in progress).
...
## 2.3 `AI_Charter_and_Invariants_Card_v1_22.md` - CONTROLLING

This is the W3X-ratified charter baseline, internal version 1.20 (G10 debug-
...
## 2.8 `Scopes/Deblock4_Scope_Stage_1C_Filter_Creation_v1_5.md` - CONTROLLING
## 2.8a `Scopes/Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_1.md` - CONTROLLING
## 2.8b `Scopes/Deblock4_Stage_1C_Phase_3a_Designer_Briefing_v1_0.md` - INFORMATIVE (context)

Read 2.8 and 2.8a last, together. The scope is the Stage 1C design authority
(module map, dispatch architecture, proof matrix, gap resolutions
C1-C9/P1-P5); the addendum governs PHASE DELIVERY ORDER (read the whole scope
for context, deliver only the released phase). You BUILD against the scope and
addendum.

2.8, 2.8a and 2.8b together form the Phase 3a REVIEW SET (charter 2.3a version
group; use the LATEST filename version of EACH - members may bump as guidance
accrues, so verify current versions with W3X). The briefing (2.8b) is W3D's
REVIEW guidance - it tells you what W3D will check on your Phase 3a delivery
(the VSPublicFunction creation-callback signature with unchanged body, the
unrestructured permanent switch, the complete one-line lifecycle trace, the C5
order). Read it for that project context so you build toward what will pass
review; you build against the scope/addendum, and the briefing is the mirror
showing how your work will be assessed. It is not a separate set of
requirements to satisfy beyond the scope.
...
Phases 1 and 2 are accepted and committed; Phase 3a is the current bounded
work.
...
force-down seam, all built and proved. STAGE 1C (filter creation) is now ACTIVE:
Phase 1 (pure foundation) and Phase 2 (tier selection + instance creation +
permanent-skeleton routers) are accepted and committed; Phase 3a (frame path +
real plugin registration) is the current bounded work; Phase 3b (sweep + full
proof) follows.
...
active: Stage 1C's scaffolding SWEEP is an authorised, scoped cleanup (retiring
the probe/smoke/dll_probe files per C-STY-10's sweep test, in Phase 3) - that is
in-scope when the Stage 1C scope calls for it, and is not the unbounded
refactoring this rule forbids.
...
2. Whether the attached charter filename and internal version both identify
   ratified v1.20; stop on any mismatch. (A future successor document is
...
INCREMENTAL EMISSION (charter C-DELIV-09). For any scope or phase that produces
more than one module, emit completed work to W3X INCREMENTALLY as you finish
it - each completed module or small coherent group - with a one-line marker
"increment N of ~M: <what>". This gives W3X a resumable baseline if the session
is interrupted and running review checkpoints. Be honest about the limit: ONLY
DELIVERED ARTIFACTS SURVIVE an interruption - do NOT claim to preserve or resume
un-emitted internal reasoning; the recoverable state is the last COMPLETE
increment emitted, and at most the module in flight is lost. The increments do
NOT replace the final deliverable: at the end you RE-PACKAGE the whole scope as
one properly-formed deliverable (exact base, per-file form, manifest, apply/
validate discipline, plus the validation batch) meeting C-DELIV-01..08. The
increments are a preview; the package is the artifact of record.
...
3. Current milestone: Stages 1A..1B.3 COMPLETE; Stage 1C ACTIVE - Phases 1 and
   2 accepted and committed, Phase 3a the current bounded work; Stage 1
   incomplete.
4. Immediate scope: STAGE 1C Phase 3a (frame path + real deblock4_plugin
   registration), per scope v1_5 and addendum v1_1; the two 3a pins are the
   VSPublicFunction creation-callback signature and the unrestructured
   permanent switch. Read the whole scope; deliver only the released phase. No
   pixel/deblocking code (that is 2C/2D).
...
7. That you hold the prevailing branch-main source (upload-if-unsure; no commit
   id tracked) and the relevant scaffold files.
...
Scopes/Deblock4_Scope_Stage_1C_Filter_Creation_v1_5.md   (CONTROLLING scope)
Scopes/Deblock4_Stage_1C_Delivery_Plan_Addendum_v1_1.md  (phase delivery order)
the accepted Phase 1 source (5 pure modules) as the prevailing base
all source files and test contracts touched by the released phase
```
