...
**Version:** 1.05
**Date:** 2026-08-16
**Author:** W3C ratification-recording revision, based on W3D's acceptance of
v1.03/v1.04 and W3X's explicit ratification of v1.04 on 2026-08-16
**Status:** W3X-RATIFIED. PREVAILING SINGLE SOURCE OF TRUTH for MPEG-2
deblocking matters in Deblock4. This v1.05 records the already-made W3X
ratification of v1.04 and reconciles status/sequence wording only; it makes no
new MPEG-2 algorithm or architecture decision.
...
**Supersession:** this document absorbs and supersedes
...
- `[H.262-VERIFIED]` - read directly in authoritative ITU-T H.262 text by W3C,
  either in the 2026-08-16 V4 verification report or in the v1.04 provenance
  re-audit recorded in section 2.1; no such tag is sourced from GAIS;
...
- `[W3X-RATIFIED]` - explicitly ratified by W3X and part of the prevailing
...
TARGET-DEVICE REALITY - WHY B2 IS NOT A THEORETICAL CORNER-CASE DESIGN
----------------------------------------------------------------------
The measured LG VHS-to-DVD target recorder uses frame pictures with
`frame_pred_frame_dct = 0` in EVERY practical restoration speed mode
XP/SP/LP/EP. In the project's historical vocabulary this is REGIME 3: the
encoder is permitted to choose FRAME or FIELD DCT per applicable macroblock.
Only MLS measured `frame_pred_frame_dct = 1` and therefore supplies a useful
uniform-frame-DCT control. [MEASURED, section 6.2]

This is the strongest existing device-specific reason B2 deserves serious
measurement rather than dismissal as over-engineering: the adaptive-capable
regime is NORMAL target-device operation. Precision is essential, however:
`frame_pred_frame_dct = 0` proves that per-macroblock adaptation is PERMITTED;
it does NOT by itself prove that both dct_type values actually occur in every
picture or recording. D4-Q14 extracts per-macroblock truth to measure that.

For cheap corpus triage before Q14, the project has already used:
    mediainfo --Details=1 <file.mpg>
to inspect `frame_pred_frame_dct` and `picture_structure`. This identifies the
picture-level regime and helps select ground-truth material; it is NOT the
per-macroblock dct_type truth source. [section 3.1]
...
16. [W3X-RATIFIED] The experiment must not
    encode a forced binary outcome "B2 fails => D ships". The architecture entering the
...
**v1.04 naming-consistency audit:** every ACTIVE processing-order reference in
this document uses Schedule-SA/Schedule-SB/Schedule-SC. Bare historical
`Schedule A/B/C` wording appears only where the old README names are being
identified explicitly. Architecture A/B/B2/C/D remains reserved for the
geometry-architecture alternatives. No ambiguous active A/B shorthand remains.
...
F2  [DERIVED FROM H.262-VERIFIED FACTS]
    ...
    still acts on 8x8 actual chroma samples. This is a geometry consequence
    of the 8x8 block definition plus the per-component sampling matrices and
    macroblock structures; it is not presented as a verbatim H.262 sentence.
...
## 2.1 H.262 provenance re-audit - v1.04

W3D explicitly asked that consolidation not quietly turn GAIS confirmation into
standard authority. W3C therefore re-audited every active `[H.262-VERIFIED]`
F-series claim against the authoritative ITU-T H.262 (02/2000) text.

```text
F1  DIRECT W3C RE-VERIFICATION, v1.04:
    definition 3.12 defines a block as an 8-row by 8-column matrix of samples;
    6.1.4 likewise states that source/reconstructed blocks have 8 lines and
    8 samples.

F2  RECLASSIFIED DERIVED:
    6.1.1.8 defines 4:2:0 Cb/Cr matrices as half the Y dimensions; 6.1.1.9 and
    6.1.1.10 define the 4:2:2/4:4:4 component-matrix relationships; combining
    those facts with F1/F3 yields the plane-relative geometry statement.

F3  DIRECT W3C RE-VERIFICATION, v1.04:
    6.1.3 explicitly gives 4:2:0 = 6 blocks (4Y+1Cb+1Cr),
    4:2:2 = 8 blocks (4Y+2Cb+2Cr), and
    4:4:4 = 12 blocks (4Y+4Cb+4Cr).

F4  DIRECT W3C VERIFICATION:
    W3C verification report V4.1 and re-check in v1.04, H.262 6.1.3.
    It explicitly distinguishes frame-DCT/field-DCT luma organisation and says
    4:2:2/4:4:4 chroma follow luma while 4:2:0 chroma remains frame-organised.

F5  DIRECT W3C VERIFICATION:
    W3C verification report V4.3, H.262 macroblock syntax / dct_type semantics.
```

**Audit result:** no active `[H.262-VERIFIED]` claim in this document depends on
GAIS. F1 and F3 were not explicit enough in the original W3C V4 report, so v1.04
records the direct W3C standard re-verification here rather than pretending the
older report said more than it did. F2 is deliberately labelled DERIVED rather
than overstating a direct quotation. The W3C V4 report remains the quotation
record for F4/F5 and the original G1 closure.
...
This is the practical **ground-truth triage route** W3D referred to: it tells us
which files/pictures are in a syntax regime where per-macroblock adaptation is
possible and therefore where deeper truth extraction is worth doing.
MediaInfo/ffprobe do not provide the required labelled per-macroblock dct_type
map for the experiment; D4-Q14 needs bitstream/decoder-side extraction for that.
...
Every practical restoration mode XP/SP/LP/EP therefore operates in what the
older project vocabulary called **REGIME 3**: frame pictures with
`frame_pred_frame_dct = 0`, so applicable macroblocks may choose FRAME or FIELD
DCT independently. MLS is a useful uniform-frame-DCT control.

**Decision significance:** this adaptive-capable regime is the NORMAL measured
operating regime of the target LG recorder, not an academic codec corner case.
That is the strongest existing target-device argument that B2's per-macroblock
topology work is warranted rather than gratuitous complexity.

**Evidence precision:** the measurement does NOT yet prove that every practical
recording, or every picture in it, actually contains both dct_type values.
`No / 0` proves permission/capability at picture level. D4-Q14 exists precisely
to extract per-macroblock truth and measure the real FRAME/FIELD/NO_DCT
distribution. In this document, historical shorthand such as "regime-3 mix"
must be read as "adaptive-capable per-MB regime" unless a later measured Q14
result explicitly proves actual mixture.
...
This W3X-ratified document prevails on MPEG-2 design facts.
```
...
## 15.4 Dataset discipline [W3X-RATIFIED]
...
## 15.5 Decision rule [W3X-RATIFIED]
...
# 23. DEVELOPMENT SEQUENCE AFTER RATIFICATION

The single-source authority is now ratified. The formal documentation
consolidation tasks T1-T3 remain live but W3X has PAUSED T1 while the Q14 work
is set up; that sequencing choice does not reopen or weaken this authority.

```text
1. T5: derive and ratify the detector / feature mathematics needed by Q14.
   T5 may be the first frozen part of one coordinated T5+T6 design package, but
   it must be fixed before held-out experiment judgement.
2. T6: draft and ratify the D4-Q14 experiment plan, with predeclared metrics,
   viability criteria, calibration/held-out separation and per-MB truth method.
3. Run Q14 with bitstream-side per-MB truth on calibration and held-out target
   material.
4. Ratify B2, D, or a reopened architecture based on evidence.
5. Resume/complete T1-T3 documentation consolidation at W3X's chosen point,
   before stale duplicate MPEG-2 guidance can govern implementation.
...
R1  ITU-T H.262 (02/2000)
    Normative MPEG-2 block/DCT-organisation authority. Original W3C V4 report
    verifies F4/F5; v1.04 section 2.1 records W3C's direct provenance re-audit
    for F1/F3 and correctly marks F2 as a derived geometry consequence.
...
R6  README_Deblock4_Design_Spec_v1_12.md
    ...
    prevail over this W3X-ratified document.
...
R7  Deblock4_MPEG2_Grid_Field_DCT_Knowledge_v1_2.md
    ...
    superseded by this W3X-ratified document.
...
After this document is ratified as the single MPEG-2 authority, the
documentation de-duplication pass should also reconcile the successor
introductions explicitly:

```text
111_New_Chat_Introduction_for_Coder_v1_28.md
    top MPEG-2 authority block is directionally correct;
    later section 2.7 still preserves superseded separated-field/midpoint
    mechanism wording and should be reduced to a pointer to this document.

111_New_Chat_Introduction_for_Designer_v1_21.md
    top MPEG-2 authority block is directionally correct;
    section 3.11 has a prominent SUPERSEDED-IN-MECHANISM warning but still
    carries the old midpoint answer and historical "mix" shorthand after it.
    Preserve only the durable measured LG regime fact by pointing here; do not
    preserve the rejected mechanism as live guidance.
```

Both introductions already tell successor chats to read this document's
section 0 first. The de-duplication goal is therefore simplification, not a new
architecture change.
...
```text
v1.05 (2026-08-16) RATIFICATION-RECORDING / HANDOFF-CURRENCY ONLY. Records
      W3X's explicit ratification of v1.04, replaces residual W3X-PENDING
      markers with W3X-RATIFIED, and reconciles section 23 to the current W3X
      sequencing: T1 remains paused while T5 detector mathematics and T6 Q14
      planning proceed. No geometry, architecture, mathematics or acceptance
      decision changed.
v1.04 (2026-08-16) W3C revision after W3D's v1.03 review. W3D ACCEPTED the
      v1.03 consolidation and the experiment-integrity proposals, subject to
      W3X ratification, and requested one substantive clarification plus two
      consistency audits. Adds the LG target-device significance to section 0:
      XP/SP/LP/EP are measured `frame_pred_frame_dct=0` / adaptive-capable
      REGIME 3, making the hard regime normal target operation and B2 a
      warranted architecture to test; explicitly preserves the evidence limit
      that this does not yet prove actual FRAME/FIELD mixture. Adds the
      MediaInfo `--Details=1` picture-level triage route to section 0 and
      clarifies that it is not per-MB truth. Completes the Schedule-SA/SB versus
      Architecture-A/B naming audit. Completes an H.262 provenance re-audit:
      F1/F3 directly re-verified by W3C against H.262, F4/F5 traced to W3C V4
      plus re-check, and F2 correctly reclassified as DERIVED rather than
      overstated as a direct standard sentence. Records the no-forced-fallback,
      held-out-data and D4-D13 items as W3D-ACCEPTED / W3X-PENDING rather than
      pretending W3X ratification has already occurred. Adds explicit
      coder/designer-introduction de-duplication targets. No code change.
