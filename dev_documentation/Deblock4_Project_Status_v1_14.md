...
Deblock4 has completed Stages 1A, 1A.1, 1B.1, 1B.2 and 1B.3; Stage 1 as a
whole is not complete (the filter-creation stage remains).
...
Stage 1B.3 complete - runtime capability guard: ACTUAL/EFFECTIVE records,
                      ...
                      committed (verify current HEAD before the next scope).
```

The project is not yet a functional VapourSynth deblocking filter. The current
DLL and executables comprise build/linkage/interop scaffolding PLUS the real,
proven Stage 1B.3 capability detector and the first-class deblock4_selftest
executable; no filter entry point or dispatch wiring exists yet.
