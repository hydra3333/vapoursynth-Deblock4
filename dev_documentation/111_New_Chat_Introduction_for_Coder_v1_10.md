...
Stages 1B.1 and 1B.2 are COMPLETE and committed. The active bounded scope is
**Stage 1B.3 - the runtime capability guard** (scope
Deblock4_Scope_Stage_1B3_Runtime_Capability_Guard_v1_3.md). It implements: CPU
capability detection via raw CPUID/XGETBV over the verified Set-A bit table plus
the Set-B XCR0 check; whole-level v3 -> v2 -> v1 resolution into an immutable
ACTUAL (process-wide) record and an EFFECTIVE (per-instance) record; the shared
module skeleton (deblock4_config.zig declarations-only switchboard,
print_helper_functions.zig always-on, print_diag_helper_functions.zig gated,
cpu_capability_detection.zig); a first-class deblock4_selftest.exe built from the
same source modules as the DLL; and a debug-only force-down seam. It does NOT
wire dispatch to call backends and does NOT create the VapourSynth entry point
(both later stages).

NOTE: a first W3C delivery for scope v1.3 EXISTS and is under designer review;
the prior coder chat died after producing it. A fresh coder is likely picking up
either to rework that delivery against review findings or to continue - confirm
with W3X which, and obtain the delivery and any review notes before coding.

DEBUG-ONLY CODE USES THE G10 THREE-LAYER PATTERN (new since charter v1.17):
...
code in Stage 1B.3. Do not call SSE4.1 or AVX2 code, and do not wire dispatch to
invoke a backend: G5 forbids execution; 1B.3 builds and PROVES the guard, it
does not consume it.
...
Read first for the reported proof state, open work, and the Stage 1B.3
position (scoped v1.3, first delivery under review).
...
## 2.3 `AI_Charter_and_Invariants_Card_v1_17.md` - CONTROLLING

This is the W3X-ratified charter baseline, internal version 1.17 (adds G10,
the debug-only code inclusion pattern). Verify the
...
## 2.4a `Deblock4_Verification_And_Tiering_Decisions_v1_9.md` - INFORMATIVE DURABLE RECORD
...
**Stages 1A, 1A.1, 1B.1 and 1B.2 are complete; Stage 1 is not complete.**
...
Stage 1B.3 (active) implements the runtime capability guard, the shared config/
print module skeleton, the self-test executable, and the debug-only force-down
seam; scoped at v1.3, a first W3C delivery is under designer review.
...
not. Those require Stage 1B.2 compile-and-assembly evidence. The named v1/v2/v3
feature contracts themselves are already settled.
...
inspection proof, not a refactoring scope (isolation and one-DLL linkage were
the completed Stage 1B.1 proof).
...
2. Whether the attached charter filename and internal version both identify
   ratified v1.17; stop on any mismatch. (A future successor document is
...
```text
Deblock4_Project_Status_v1_10.md
Deblock4_Concise_Project_Summary_v1.1.md
AI_Charter_and_Invariants_Card_v1_16.md
...
Deblock4_Forward_Roadmap_v1_8.md
