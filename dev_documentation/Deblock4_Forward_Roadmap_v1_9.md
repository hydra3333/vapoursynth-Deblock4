...
Status (v1.9): Stages 1A, 1A.1, 1B.1 and 1B.2 are COMPLETE and committed. Stage
1B.3 is the active stage (scoped v1.3; a first W3C delivery is under review).
...
Stage 1B.3   ACTIVE - runtime capability guard. CPUID/XGETBV detection over the
             verified Set-A table + Set-B XCR0; whole-level v3->v2->v1 into
             immutable ACTUAL (process-wide) and EFFECTIVE (per-instance)
             records; the shared config/print module skeleton; a first-class
             self-test exe; and a debug-only force-down seam (G10 pattern,
             force-DOWN-only, both debug options Debug-only). 1B.3 BUILDS and
             PROVES the guard; it does NOT wire dispatch to call backends and
             does NOT create the VS entry point (those are the filter stage).
             The comptime cross-check reconciles the detection membership with
             Zig std.Target named models so target
