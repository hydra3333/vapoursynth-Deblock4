# Deblock4 - New Chat Introduction for Coder (v1.30)
...
W3X-ratified MPEG-2 authority v1.05. The immediate design sequence is T5 detector/feature mathematics followed by
the T6 D4-Q14 architecture-discriminator experiment plan and then the
experiment itself. T5 may be the first frozen part of one coordinated T5+T6
package, but it must be fixed before held-out judgement. W3X may ask W3C to
independently cross-check detector/boundary mathematics, SIMD consequences,
source/tooling feasibility or a bounded experiment implementation, but **do
not write a Deblock4 filtering kernel and do not infer a kernel scope from this
handover**.
...
Project Status v1.28 section 0
    ...
    -> T5/T6 design package or active scope supplied by W3X
```
...
## 2.7a `Deblock4_Standing_Task_Register_T_Series_v1_1.md` (or later) - LIVE WORK QUEUE
