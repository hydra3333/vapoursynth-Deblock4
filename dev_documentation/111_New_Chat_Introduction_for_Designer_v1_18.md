...
PAL 576i MPEG-2 tape captures. The core design round is CLOSED: the algorithm,
the two-filter architecture, the public API, the invariants, the verification
and tiering models, and the delivery process are settled and ratified. The
project is in early implementation.

```text
Current position (2026-08-02, end of session; verify against repo):
    STAGE 1 IS COMPLETE IN FULL: 1A, 1A.1, 1B.1, 1B.2, 1B.3, 1C AND rider
    1C.1 are accepted, green in all three modes, committed and pushed. Both
    filters build (ReleaseSafe/ReleaseFast/Debug), load under VapourSynth
    R78, pass frames byte-identically across tiers, emit the rider 1C.1
    resolved-invocation "using" line + Deblock4Using frame property from the
    same stored bytes, enforce the ratified creation-error message table
    (v1_1), and hide all G10 debug seams (three-surface absence proven).
    The fifteen-gate proof matrix in build_1C_v1.bat is the standing gate.

    Prevailing baselines: charter v1_26; README design spec v1_9;
    Verification and Tiering Decisions v1_10; Toolchain Findings v1_3
    (F1-F8); Project Status v1_20; Forward Roadmap v1_17; creation-error
    table v1_1; coder intro v1_20.

    STAGE 2C (Classic scalar oracle + HolyWu differential) IS UNDERWAY as
    short-boundary W3D deliverables with a W3X checkpoint after each:
      D0 Preface + Binding Knowledge Index: COMMITTED, latest
         Deblock4_Stage_2C_D0_Preface_and_Binding_Knowledge_Index_v1_5.md
         under dev_documentation/reference/ (cite the highest version).
         K1-K26 indexed. K19 = three-layer comparison doctrine (general
         not-bit-exact / Classic external byte-exact-TARGET-with-
         investigation / internal byte-identical). K26 = external-oracle
         EXECUTION pin (G6 discipline: byte-pinned source does NOT pin
         signed-shift/float-contraction results; the differential harness
         must force opt=1 and pin compiler/flags/plugin versions/DLL
         hash - MANDATORY binary hash + negative-delta behavioural
         sentinels + rebuild-revalidation rule, because (q0-p0)<<2 on a
         negative is C++ UNDEFINED BEHAVIOUR, a different class from the
         implementation-defined right shift; the Zig oracle avoids the
         analogue via i32 multiply-by-4). K17 wording corrected to the
         settled touched-paths-only model (no clean-tree presumption).
         Section 5 records the SETTLED non-mod-8 boundary policy
         (native footprint bounds, no pad/crop wrapper) and offset policy
         (Classic REJECTS out-of-range offsets, never clamps). Section 6 =
         standing TWO-SIDED knowledge-sweep protocol, its 6.1 block
         embedded verbatim in every scope. D0 is LIVING: findings append
         as new K-numbers.
      D1 Pinned reference snapshot: COMMITTED (provenance README at
         v1.1: source-pin vs K26 reference-build-record distinction +
         its own Binding Knowledge Checklist; upstream bytes and
         SHA256SUMS unchanged) at
         dev_documentation/reference/holywu_r9/ (HolyWu VapourSynth-Deblock
         tag r9 = ratified D-CLASSIC-4; byte-exact upstream LF bytes pinned
         by SHA256SUMS.txt; DELIBERATELY outside third_party/ because the
         S3 gate enforces zero-LF over the deliverable tree - never move or
         EOL-normalise it; vectorclass/ deliberately excluded).
      D2 HolyWu Real Schedule: DELIVERED and VERIFIED. Latest
         Deblock4_Stage_2C_D2_HolyWu_Real_Schedule_v1_3.md (v1_2 resolved
         W3C verification findings F1-F11; v1_3 applied the W3C
         revision-review: edge-replication assertion removed, pad mapping
         row settled-policy, fmParallel wording, WP-1 UB-class split). Verified core (W3C PASS):
         4-pixel H.264 grid; band-by-band top-to-bottom, left-to-right,
         horizontal edge BEFORE vertical at each crossing, in-place on a
         copied frame; K7 footprints (read e-3..e+2, write e-2..e+1);
         luma-on-chroma; all integer/float formulas, biases, shifts,
         clamps; threshold tables machine-verified 61/61. Precision points
         (v1_2): the schedule is OUTPUT-DEFINING because specific overlaps
         exist (not "every edge reads prior writes" - the first has no
         predecessor); WP-1 floor-shift and float contraction are
         EXECUTION facts pinned via K26, not proved by source bytes;
         HolyWu's non-finite behaviour (non-finite p2/q2 need NOT suppress
         the core p0/q0 write) is documented as an EXTERNAL divergence
         from Deblock4's README-8.6 policy; opt=0 auto-selects SSE4 so the
         harness must force opt=1. WP-5 (c0 from aIndex) stays FAITHFUL.
         Tabled: T-1 (WP-5 at 3C quality gate), T-2 (float support, D4);
         T-3 COLLAPSED to SETTLED (non-mod-8 native bounds, README
         6.1/6.3). The HolyWu differential compares the LEGAL SHARED
         DOMAIN only (mod-8, in-range offsets, integer unless T-2).
      D3 Independent scalar obligations + sanity gate: REVIEWED by W3C
         (8 substantive findings) and revised; latest
         Deblock4_Stage_2C_D3_Scalar_Obligations_and_Sanity_Gate_v1_2.md.
         v1_2 highlights: O-5d 16-bit obligation is the LITERAL native
         matrix (x256 homogeneity was FALSE - sequential rounded writes;
         both parties recomputed identically); O-7 boundary/small-plane
         obligations (10x10 skip-ineligible, 12x6 one-orientation, 6x6
         pass-through) per README 6.1/6.2/6.5 with Appendix C eligibility
         model; gate gained G5 (max |change| <= 12) and G6 (mean <= 3.0)
         with the constant-fill counterexample as a MANDATORY negative
         control; O-1c binds all 61x3 table entries + full index domain;
         A5/B7/B8 complete strict-< coverage of ALL five comparisons;
         O-6d source immutability, O-6e canaries, O-6f i32 range proof;
         B2/B4/B5 designated as K26 negative-delta sentinels (recorded
         from the hashed reference binary); proof ROUTING split (kernel ->
         unit tests, creation errors -> vspipe cases, copy -> production
         path). Per W3C recommendation D3 v1_2 gets ONE MORE review
         together with D4 before judging any delivery.
      D4 Stage 2C coder scope: NOT STARTED. Mandatory W3C pre-
         implementation review INCLUDING the independent knowledge sweep.
         Carries: the section-6.1 sweep block verbatim; K24 shared-kernel
         authoring requirement (same kernel body later instantiates v2/v3;
         ZERO vector code in 2C); K17 delivery self-containment; K26
         harness duties (force opt=1, pin the reference build, ASSERT
         in-domain inputs: mod-8, in-range offsets); the legal-shared-
         domain differential scope; decision T-2 (float); oracle-
         construction exception acceptance basis.

    Review plan (W3X-ratified): D1 no dedicated review; D2 -> coder
    verification; D3 -> coder review; D4 -> mandatory review + sweep.
    Stage architecture: 2C = plain-Zig scalar ReleaseSafe oracle ONLY;
    @Vector enters at 4C (v2) and 5C (v3), each accepted byte-identical
    (integer) against the 2C oracle per K19 layer (c).

Immediate designer action:
    D0 v1_5, D1 provenance v1.1, D2 v1_3, D3 v1_2 are the current
    reference set (cite highest versions). THREE W3C review rounds are
    RESOLVED (D2 F1-F12; revision-review F1-F7; D3 review F1-F8). NEXT: author the D4 Stage 2C coder scope from D2 v1_2 + D3 v1_1,
    then send D3 v1_1 + D4 to W3C as ONE combined review package (D3
    review + D4 mandatory review + independent knowledge sweep). Read D0
    (latest) FIRST - it is the knowledge net and its checklist duty binds
    every deliverable you produce.

    W3D cannot run Zig or VapourSynth: verification is STATIC against
    source; W3X's toolchain run is authoritative; W3X alone accepts,
    releases and commits. W3X works LIVE in the repo tree between runs
    (docs, superseded/ stash): deliveries are SELF-CONTAINED (K17), hash
    only the files they touch, and ship scoped restore-to-base commands -
    never demand a globally pristine tree, never touch superseded/.

    KNOWN STALE (fix when next editing those files; do not be misled):
    the Concise Project Summary v1.2 still lists the HolyWu pin and a few
    2C items as open - superseded by D-CLASSIC-4/r9 and the D0..D3 set.
    The committed reference/ deliverables and this block prevail.

    Hard-won lessons that carry forward: (1) G6 bans PE-EXPORT of gated
    code, not Zig's export keyword; object-mode and DLL-root compilation
    have DIFFERENT export surfaces (F8). (2) G10 three-layer pattern for
    debug-only code; implement the settled detection contract, never
    re-derive. (3) Assertions against Zig LF output: prefer exit codes and
    positive substrings; never findstr /X on CRLF-mixed captures. (4) I7:
    a party changing criteria applied to its own work names a
    different-party verifier; W3X retains adoption authority. (5) Cross-
    review catches real errors in BOTH directions (coder found scope
    defects; designer found the G6 leak; W3X recall checks forced K19 and
    K20-K25) - the D0 section-6 sweep makes this structural. (6) The
    knowledge net is committed documents + checklists, never any chat's
    memory: when in doubt, search the doc set and cite, exactly as D0
    demands. (7) The D2 review is the template: W3C caught a policy
    reversal (Classic clamp vs reject), a settled-policy reopening
    (non-mod-8), an over-claim (source bytes do not pin shift/contraction
    results -> K26), and an omission (non-finite behaviour) - all real,
    all fixed by revision not rebuild. Author defensively; expect the
    independent sweep to find things; treat that as the system working.
```
...
2. README_Deblock4_Design_Spec_v1_9.md              CONTROLLING
...
4. Deblock4_Project_Status_v1_16.md                 INFORMATIVE
   Read for the current proof state, what is done, what is open, and why
   Stage 1C is active. Use its implementation-state sections; do not let any
...
11. PHASE 3a REVIEW SET - READ AS A SET (charter 2.3a; MIXED AUTHORITY)
    These THREE must be read together for Phase 3a review. Use the LATEST
    filename version of EACH. If a member is missing or the set is inconsistent,
    STOP and ask W3X. The set does not make every member controlling.
