...
| Zig/C ownership and lifetime policy | Recorded in charter (current v1.12) |
| Numeric and SIMD helper policy | Recorded in charter (current v1.12) |
| External-source provenance policy | Recorded in charter (current v1.12) |
...
The purpose is to confirm each object stays within its named psABI level (the level is the feature contract, not a bespoke closure) as assumed by each
...
1. the charter's canonical filename and internal version both say v1.12;
2. the README's filename and internal design revision both say v1.5;
...
therefore CONFIRMS each object emits nothing outside its level and that dispatch
checks the whole level; it does not derive a closure. It requires no new
...
build then runs for Classic first, then Deblock4. See README v1.5 section 1.0.
...
```text
v1.4  Consistency reconciliation after the document audit. Authority block and
      documentation-package section updated to charter v1.12 / README v1.5 /
      decisions v1.3 / roadmap v1.4 (was stale at v1.9/v1.1 and listed a
      "four-document package" of eight files). Named-level terminology replaces
      "smallest exact feature closure". The "Not implemented" list is split into
      shared / Classic / Deblock4 entries (Classic and Deblock4 each need their
      own registration, scalar oracle, and v2/v3 backends). Charter references
      in the completed table corrected to v1.12.
