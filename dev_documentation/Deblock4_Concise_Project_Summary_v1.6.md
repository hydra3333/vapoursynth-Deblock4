...
THE OLD DEBLOCK4 PARAMETER SURFACE BELOW IS LEGACY SCAFFOLDING. In particular,
`mpeg2_field_separated`, midpoint machinery and single per-frame grid-step
properties do NOT describe the current B2/D architecture. D4-Q16 owns the
future public-parameter/property redesign. Accepting old scaffolding parameters
in current code is not endorsement of their algorithmic meaning.
...
```text
Stage 1       COMPLETE. Shared Zig/API4/build/dispatch/filter-creation scaffold.
Stage 2C      COMPLETE. Classic ReleaseSafe scalar oracle.
Stage 3C      COLLAPSED. Its quality question is deferred.
Stage 4C      COMPLETE. Classic SSE4.1/x86-64-v2 backend.
Stage 5C      COMPLETE. Classic AVX2/x86-64-v3 backend.
M1, M2        COMPLETE. Post-5C maintenance.
Identity      0.1.0-dev+5C.

CLASSIC       COMPLETE for the ratified integer tier set.

T1            IN PROGRESS. Documentation consolidation.
              a5 search/classification repair is settled; successor-W3D
              verification of the W3C recovery ledger and W3X closure remain.
...
quality knobs waiting to be tuned; their mechanism is retired.
