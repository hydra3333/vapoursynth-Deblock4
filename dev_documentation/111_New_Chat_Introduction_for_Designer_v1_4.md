...
**Version:** 1.4
**Date:** 2026-07-30
**Status:** Informative successor orientation; not controlling; aligned to ratified charter v1.17
...
```text
Current position:
    Stages 1A, 1A.1, 1B.1 and 1B.2 are complete and committed.
    Charter ratified at v1.17 (adds G10). README design spec at v1.9.
    Stage 1B.1 proved isolated backend objects and one-DLL linkage under
    scope v1.7, after THREE retention mechanisms were empirically falsified
    (see Deblock4_Toolchain_Findings F1-F5). Stage 1B.2 (within-level object
    and assembly CONFIRMATION) is COMPLETE and committed; its standing batch
    is build_1B2_v5_REDEVELOPED.bat. Stage 1B.3 (the runtime capability guard
    plus the shared config/print module skeleton and the debug-only force-down
    seam) has been SCOPED at v1.3 and a first W3C DELIVERY is in hand awaiting
    designer review; the coder chat died after producing it.

Immediate designer action:
    Review the Stage 1B.3 W3C delivery against charter v1.17 and scope v1.3.
    Two hard-won lessons to carry. (1) From 1B.1: G6 bans PE-EXPORT of gated
    code, NOT the Zig export keyword - gated backend functions ARE declared
    export fn in their own single-target objects (object-mode export does not
    PE-export); the DLL root reaches them by @extern, address-taken never
    called; absence is enforced by a standing dumpbin /EXPORTS gate. (2) From
    1B.3: debug-only code is included by the G10 three-layer pattern - a
    source-visible C-3 conditional import (const x = if (opt) @import(..) else
    struct {};), gated content as defence in depth, and three-surface proven
    absence; this was empirically ratified by gate_pattern_test_v2. Do not let
    a successor re-derive the falsified alternatives to either.
...
4. Deblock4_Project_Status (latest version)         INFORMATIVE
   ...
   Stage 1B.3 is in flight (scoped v1.3, delivery under review). Non-
   controlling; the README and charter prevail.
...
1. The exact current ratified charter and README versions in the repo. This
   file says charter v1.17 / README v1.9; confirm nothing newer has superseded
...
2. Whether the Project Status document reflects the latest Stage 1B progress
   since this handover (Stage 1B.2 complete; 1B.3 scoped at v1.3 with a first
   delivery in review).
...
3. Current position: Stages 1A, 1A.1, 1B.1, 1B.2 complete; design round
   closed; charter v1.17, README v1.9 (or whatever you actually received -
   state it); Stage 1B.3 scoped at v1.3 with a first W3C delivery in review.
