...
**Version:** 1.2 (2026-08-14: GAIS partial confirmation recorded - G2/G3/G4
CONFIRMED via the captured response file; G1/G5/G6/G7 remain pending)
...
Chroma is determined separately and is now NORMATIVELY CITED, not only
structurally derived. ISO/IEC 13818-2:1995 | ITU-T H.262, subclause 6.1.3
(Macroblock), verbatim: "in the 4:2:0 format the chrominance blocks shall
always be organised in frame structure for the purposes of DCT coding."
Supporting definitions: 3.12 (block = 8-row by 8-column matrix of samples);
3.85 (4:2:0 macroblock = four 8x8 luminance + two corresponding 8x8
chrominance blocks); 6.1.1.8 (4:2:0 Cb/Cr matrices are one half the Y-matrix
size in BOTH dimensions); Figure 6-10 (4 Y + 1 Cb + 1 Cr block order).
CONSEQUENCE: 4:2:0 chroma DCT blocks are 8x8 ON THE CHROMA SAMPLE ARRAY and
are NEVER field-organised (dct_type is a luminance-organisation choice in
4:2:0), so after field separation each woven 8-line chroma block contributes
4 lines to each field: chroma grid x=8, y=4 IN CHROMA COORDINATES,
deterministic, no per-macroblock ambiguity. NUANCE (do not confuse): the same
subclause notes field-based MOTION PREDICTION may address 8x4 chroma regions;
that is prediction, not DCT organisation, and does not move the blockiness
grid. Do not apply the luma 4/8 dct_type reasoning to chroma.
...
# 9. GAIS confirmation checklist (to finalise this knowledge)

The primary citation was located in a public copy of ISO/IEC 13818-2:1995
(Recommendation ITU-T H.262 1995 E) during the 2026-08-12 W3D verification.
GAIS confirms, against an authoritative copy (ITU-T H.262 is a free ITU
download; use the latest edition and the 1995 base text):

```text
G1  Subclause 6.1.3 contains, verbatim or with only editorial drift, the
    sentence: "in the 4:2:0 format the chrominance blocks shall always be
    organised in frame structure for the purposes of DCT coding".
G2  Definition 3.12 defines block as an 8-row by 8-column matrix of samples.
G3  Definition 3.85 composes the 4:2:0 macroblock as 4 luma + 2 chroma 8x8
    blocks (block_count 6; Figure 6-10).
G4  Subclause 6.1.1.8 halves the 4:2:0 Cb/Cr matrices in both dimensions.
G5  The dct_type semantics (6.3.17.1) parse the bit only for frame pictures
    with frame_pred_frame_dct == 0 - confirming dct_type is a macroblock
    (luma-organisation) choice and 6.1.3 excludes 4:2:0 chroma from it.
G6  Later H.262 editions (2000/2012 + amendments) did not alter G1-G5.
G7  Implementation corroboration: a current FFmpeg mpeg12 decoder organises
    4:2:0 chroma blocks in frame structure regardless of dct_type (the
    interlaced-DCT path applies to luma only for 4:2:0).
OUTCOME RECORD (v1.2, 2026-08-14): GAIS was asked the substantive chroma-
block question and its response - captured verbatim in the package file
GAIS_MPEG2_GRID_CONFIRMATION_RESPONSE.txt, which this document PREVAILS
over - CONFIRMS: G2 (blocks are 8x8 matrices of samples), G3 (4:2:0
macroblock = 4 luma + 1 Cb + 1 Cr blocks, block_count 6; the response also
gives the 4:2:2=8 and 4:4:4=12 counts), and G4 (4:2:0 chroma arrays are
half the luma size in both dimensions; blocks are defined on each plane's
OWN sampling grid, never luma coordinates). STILL PENDING a further GAIS
pass against an authoritative ITU copy: G1 (the verbatim 6.1.3 frame-DCT
sentence), G5 (dct_type parse semantics), G6 (later-edition stability),
G7 (FFmpeg implementation corroboration). The chroma-grid conclusion
remains supported by the direct normative citation in this document plus
this partial confirmation; it is CLOSED for design use and the pending
items are belt-and-braces.
```
