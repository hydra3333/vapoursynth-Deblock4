# Deblock4 - New Chat Introduction for Designer (v1.22)
...
v1.04, ratified charter v1.29 and README v1.12. The charter and the MPEG-2
...
READ THIS SEQUENCE FIRST AFTER ANY DESIGNER-CHAT RESET:
    ...
    Project Status v1.28 section 0
        -> MPEG-2 authority v1.04 section 0
        -> D4-Q14 experiment plan / current W3X task
...
CURRENT PROJECT STATE:
 ...
 - The W3X-ratified PREVAILING MPEG-2 AUTHORITY is:
       ...
       Deblock4_MPEG2_Deblocking_Investigation_and_Decided_Architecture_v1_04.md
...
DESIGNER ROLE NOW:
 ...
 - Next designer artifact is the D4-Q14 experiment plan (T6) unless W3X directs
   otherwise. The earlier T1 broad consolidation sweep remains paused; v1.04
   already absorbed the load-bearing MPEG-2 architecture/measurement knowledge.
...
project knowledge without independent verification. The MPEG-2 authority v1.04
...
Status v1.28 and W3X-ratified MPEG-2 authority v1.04. Fixed the filename/internal
...
3. AI_Charter_and_Invariants_Card_v1_29.md (or later)         CONTROLLING
...
4. README_Deblock4_Design_Spec_v1_12.md (or later)            GENERAL RECORD
   ...
   MPEG-2 mechanism text that conflicts with v1.04 is superseded.
...
v1.04). Classic was built FIRST because it is a KNOWN algorithm with HolyWu's
...
## 3.3 MPEG-2 grid/source-mode reasoning now lives in v1.04
...
topology; exact public tokens remain an open parameter-surface item. Read v1.04
...
v1.04 is authoritative: 4:2:0 Case-(a) chroma stays frame-organised, while
...
Schedule-SB. The MPEG-2 authority v1.04 deliberately renamed processing-order
...
The current MPEG-2 authority v1.04 carries the provenance classification after a
...
measurements and issue/decision status, read MPEG-2 authority v1.04.
...
proved" or "fallback" with "must ship". Use v1.04's D4-Q/D4-D registers as the
...
```text
- D4-Q14 experiment plan and execution: per-MB truth, B2 false-confident rate,
  UNKNOWN/confidence statistics, D false-candidate/true-edge separation
...
2. MPEG-2 authority v1.04 is W3X-ratified; if its immutable file header still
   says pre-ratification draft, W3X's ratification decision is the status fact
   until a natural successor reissue records it.
...
2. Confirm Project Status v1.28 (or later) section 0 and MPEG-2 authority v1.04
   (or later) section 0 were read first.
...
5. Immediate designer action: D4-Q14 experiment plan unless W3X directs a
   different bounded task. It must use per-MB truth, predeclared metrics,
...
v1.22 Reconciled the designer handover to Project Status v1.28 and W3X-
      ratified MPEG-2 authority v1.04. Fixed title/internal-version mismatch;
