...
Status (v1.17): Stages 1A..1C COMPLETE and committed, INCLUDING rider 1C.1
(using echo, delivery v1_1): each filter emits a resolved-invocation "using"
line and a matching Deblock4Using frame property; the F6 coercion motivation
is proven live. Full fifteen-gate proof matrix green in all three modes;
both filters proven under R78 with byte-identical pass-through. Next bounded
step: Stage 2C (Classic scalar oracle + HolyWu external-reference
differential).
