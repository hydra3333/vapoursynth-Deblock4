...
**Version:** 1.2
**Date:** 2026-07-25
**Status:** Informative successor orientation; not controlling; aligned to ratified charter v1.10
...
Current position:
    ...
    Charter ratified at v1.10. README design spec at v1.2.
    ...
    (see Deblock4_Toolchain_Findings F1-F5). Stage 1B.2 (feature-closure
    spikes and assembly inspection) is the active next stage.


Immediate designer action:
    Author the Stage 1B.2 scope when W3X asks, and review coder deliveries
    against charter v1.10 and README v1.2. The hard-won 1B.1 lesson to carry:
...
```text
1. AI_Charter_and_Invariants_Card_v1_10.md          CONTROLLING
...
2. README_Deblock4_Design_Spec_v1.2.md             CONTROLLING
...
5. Deblock4_Toolchain_Findings (latest version)     INFORMATIVE
...
6. Deblock4_MPEG2_Grid_Field_DCT_Knowledge (latest version)  INFORMATIVE
...
7. Concise_Project_Summary_v1.0.md                 INFORMATIVE
   Compact cross-check that your mental model matches the user-facing shape.

8. 111_New_Chat_Introduction_for_Coder (latest)    INFORMATIVE
...
## 3.8 Why gated backend code is never PE-exported (charter G6, v1.10)
...
The v1.9 form of the corollary said gated functions must not use the export
keyword at all, and that export-table absence would then be structural. Three
Stage 1B.1 builds falsified that: without export (and with no in-unit
reference) Zig omits the function ENTIRELY, so nothing can retain it; and
object-mode export does NOT in fact create a PE export. Charter v1.10 therefore
bans PE-EXPORT rather than the keyword, and separates three properties with
distinct controls:
...
- G6: safety properties rest on explicit/structural mechanisms, never implicit
  toolchain behaviour; gated backend code is never PE-EXPORTED (v1.10 - it IS
  declared export fn; the ban is on the export table, not the keyword)
