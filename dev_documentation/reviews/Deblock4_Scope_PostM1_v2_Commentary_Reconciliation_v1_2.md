...
**Status:** W3X-RATIFIED 2026-08-15. The pre-implementation round is CLOSED
except for R4: W3C's R1-R3 were reviewed by W3D and ratified by W3X on
2026-08-15, including the M1-C7 wording amendment W3C proposed (see M1-C7
below and the revision note). IMPLEMENTATION RELEASED once W3X supplies the
current post-5C documentation generation and W3C's final delta sweep closes
R4 clean; a sweep finding routes W3D -> W3X as usual.
...
M1-C7  Never replace that cleanup with a full-width read plus a partial
       ...
       TIER-INDEPENDENT - it is not an AVX2-only concern. [AMENDED
       2026-08-15 on W3C's M1-W3C-Q1, W3D-verified, W3X-ratified: the
       earlier wording "SSE4.1 has no byte/word masked-I/O mechanism
       either" was TOO ABSOLUTE and must not be written. MASKMOVDQU
       (SSE2-era, _mm_maskmoveu_si128) DOES conditionally store selected
       BYTES. What the SSE tiers lack is the corresponding safe masked
       byte/word LOAD, and the load is the illegal half of the shortcut.
       The comment must therefore say: a byte-selected masked store
       exists, but there is no matching safe masked byte/word load, so it
       cannot legalise an over-wide right-edge read and cannot replace
       exact-span descending decomposition. The safety conclusion and
       the accepted mechanism are UNCHANGED; only the ISA claim is
       corrected. Rationale: this block's value is being trustworthy on
...
R3  ASSESSMENT of the v3 file: is its existing tail wording already
    ...
    [ANSWERED: W3C found the existing v3 sentence semantically correct
    and already free of the superseded "scalar 1" phrasing, but NOT
    K33-complete - it does not name filterHorizontalLanes(T, 1, ...)
    nor distinguish V1 from the defensive scalar-column branch. W3D
    verified this against the delivered file. The ONE authorised v3
    comment correction is therefore CONFIRMED as needed.]
...
*Revision history*
```text
v1.2 (2026-08-15) Pre-implementation round outcome ratified (R1-R3; R4 open
     pending the current documentation package). M1-C7 AMENDED on W3C's
     M1-W3C-Q1: the "SSE4.1 has no byte/word masked-I/O mechanism either"
     claim was too absolute - MASKMOVDQU stores selected bytes; what is
     absent is the safe masked byte/word LOAD, which is the illegal half.
     Safety conclusion and mechanism unchanged. R1 manual CMD inertness
     mechanism ratified as proposed (fc /b on the v2 object and DLL from
     separate prefixes/caches, base built and retained BEFORE apply;
     fc /n plus comment-stripped projections for the source-shape audit;
     no new repository file, no PowerShell, no git, no staging). R3
     answered: the one v3 comment correction is needed. W3D note: both
     files currently carry zero indented comment lines, so the
     column-zero comment-stripping projection is exact today and fails
     safe if that ever changes.
v1.1 (2026-08-15) W3X ratification recorded: inertness acceptance basis,
     two-file comments-only surface, short R1-R4 round. No content change.
