...
**Status:** W3X-RATIFIED 2026-08-12 (round 2: 4C-RAT-1..8 resolved on the W3C pre-implementation response v1.1 and the W3D review v1_0). IMPLEMENTATION RELEASED.
...
# 3b. BINDING KNOWLEDGE CHECKLIST (D0 v1_12/v1_13 standing mechanism;
W3D-authored, every line verified against the index; fixes response F1)

```text
BINDING ON 4C (glosses are 4C applications; the index text prevails):
K1   N is vector ELEMENT count, never bytes: u8 N=16, u16 N=8 (4C);
     u8 N=32, u16 N=16 (5C later).
K2   SIMD width must not define output: scalar and v2 byte-identical.
K5   R76-class miscompile mitigations are PERMANENT: explicit @Vector +
     the standing scalar-vs-SIMD differential + tail-forcing corpus (G9).
K7   Frozen edge footprint: reads e-3..e+2, writes e-2..e+1 (p1,p0,q0,q1).
K9   Oracle sequencing: the accepted 2C scalar path is the mandatory
     per-type differential reference for every 4C pixel/backend path.
K10  RS==RF production byte identity remains a standing re-run gate.
K11  Classic is Schedule A ONLY; no adjacent-edge batching; no Schedule-B
     or MPEG-2 grid import.
K12  Settled toolchain object/export/linkage facts govern the v2 object.
K13  G5: v2 instructions execute only behind the proven whole-level guard
     and the immutable per-instance selection.
K14  Any permanent test/debug seam takes the full G10 three-layer
     discipline (4C ships NONE; see the T5 mechanism in section 7).
K15  v2 is a separate target-specific object per the settled object-mode
     architecture; no PE-export doorway; no static-init v2 execution.
K16  Creation-error strings and using-echo surfaces stay byte-stable
     except where S4C-7 expressly authorises movement.
K18  Proof-domain and audit discipline: the 4C batch inherits the
     deliverable-tree domain rules of the standing matrix.
K19c Classic-internal scalar-vs-v2 equivalence is merciless integer
     byte-identity; no tolerances; no HolyWu re-run.
K21  Do-not-revisit list stands: no twin-build, no bespoke feature
     closure, no identity-driven feature subtraction.
K23  Diagnostics contract: the reported selected tier IS the executed
     tier; output deterministic and exact.
K24  One canonical formula body (S7 form): the vector body generalises
     the call boundary; it never forks or rewrites the frozen formulas.
K25  Whole-level detection only; nothing feature-grained enters selection.
K27  Destination starts copyFrame-equivalent; source properties preserved
     before audit properties.
K28  ACTUAL per-plane width/height/stride/storage; never infer chroma
     geometry from luma; stride slack is not pixels.
K29  Integer 8..16 only; 17..32 dedicated refusal; float refused.
K30  New modules (classic_vector_backend, classic_backend_v2_sse41) take
     first-class permanent names and one-way dependency; the K30-style
     audit is delivery evidence + W3D re-verification.
K31  Strides are BYTE counts; byte-row navigation or a single checked
     conversion; never silent division.
K32  (D0 v1_13) Charter v1_27 delivery mechanics supersede the K17/K18-era
     wording: no hashes, no repository-operating scripts, no PowerShell,
     no staging; manual W3X apply/backout; base confirmed with W3X.

EXPRESSLY NOT BINDING ON 4C (do not silently import):
K4   AVX2 masked-I/O warning -> 5C (moot in 4C: ratified tails use
     narrower same-body instantiations, no masks).
K6, K22, F10  float semantics -> the later bounded float step.
K8   multi-oracle caution -> satisfied structurally (2C anchored HolyWu;
     4C differentials against the internal oracle).
K17  superseded by charter v1_27 (see K32).
K20  quality-divergence bar -> deferred quality/enhancement phase.
K26  HolyWu external binary pin -> 2C evidence history; 4C never runs it.
```
...
P2  VERTICAL-EDGE SEGMENTS (the transpose case). A vertical edge at
    ...
    [RATIFIED 4C-RAT-3: the response's direct four-row lane pack is
    PRIMARY; scalar cleanup for 1..3-row bottom segments; full-scalar
    verticals the fallback on adverse T3 lowering. REFINEMENT: per-row
    memory traffic is CONTIGUOUS (6-sample reads x-3..x+2, 4-sample
    writes x-2..x+1 per row) - express loads/stores as per-row
    contiguous small vector operations with in-register repack, never
    per-tap scalar loads or per-lane scatters; T3 judges lowering.]
...
P3  TRAVERSAL ORDER. Baseline: reproduce the oracle's exact
    ...
    [RATIFIED 4C-RAT-2: the response CONFIRMED the analysis from the
    frozen source (V(x) writes through x+1; H(x+4) touches x+4 only;
    successive verticals dependent via x+1). The reorder - one full
    horizontal edge, then that band's verticals in strict increasing
    x - is APPROVED; the top band (verticals only) is untouched.]
...
P5  TAILS, CONCRETELY. For every loop that advances by N lanes: the
    ...
    [RATIFIED 4C-RAT-4: descending same-body power-of-two decomposition
    (N/2, N/4, ... 2, then scalar 1), NO masked inactive lanes; the
    response 6.4/6.5 tail tables are ADOPTED as normative. RATIFIED
    4C-RAT-5: loads/stores use DEFINED-LAYOUT array/slice <-> vector
    VALUE coercion ONLY; overlaying a vector pointer on frame memory is
    Illegal Behavior and is FORBIDDEN (Zig vectors have no defined
    byte layout).]
...
raised ceiling), THEN the 4C surface:
  ...
  4C-T5  Negative controls: the BMI2-class named-model perturbation
         ...
         rejections; a NEW control [mechanism RATIFIED 4C-RAT-6]: the
         batch copies the source to a TEMPORARY TREE OUTSIDE the
         repository, applies one narrow one-lane tail-result mutation
         to the COPY, builds and runs the mutant as the negative
         control, and 4C-T1/T2 MUST reject it; the copy is discarded;
         no repository file, git, PowerShell or staging is touched
         (C-DELIV-10/11 clean). No in-tree corruption seam ships; G10
         is not expanded; proves the differential exercises tails.
...
Chroma-grid / field-DCT / Y-vs-UV algorithm design         -> Deblock4,
    ...
    NOTHING in classic_vector_backend.zig is a design or acceptance
    basis for Deblock4: Deblock4's vector work derives from its own
    3D-frozen algorithm, its OWN dependency analysis (the 8-step grid
    changes batching legality entirely), and its OWN oracle. Only the
    engineering disciplines (defined coercion, tail decomposition,
    tier confinement, the G9 method) carry forward as patterns to
    re-derive, never as code inheritance. (W3X 2026-08-12.)
...
*Revision history*
```text
v1.2 (2026-08-12) Ratification round 2 on the W3C pre-implementation
     response v1.1 + W3D review v1_0: F1 fixed - the W3D-authored
     Binding Knowledge Checklist added as section 3b (every line
     verified against D0; K32 cross-referenced); 4C-RAT-2/3/4/5/6
     folded at their homes with the per-row-contiguous refinement;
     the Deblock4 never-a-basis clarification added to section 9.
