...
toolchain), and COMMITTED. Phase 3 is split 3a/3b. The Phase 3a delivery v1.0
(frame path plus real plugin registration) has been produced by W3C, but is NOT
yet accepted or committed: it awaits W3D static review, W3X toolchain validation,
and W3X acceptance. No further production phase is released. Phase 3b remains
the scaffolding sweep (C-STY-10), build_1C_v1.bat, the two
tests/stage_1c_*_passthrough.vpy harnesses, and the full proof matrix, and begins
only after Phase 3a acceptance and explicit W3X release.
...
review set. Do not begin Phase 3b or make fresh production changes unless W3X
supplies review corrections or releases the next bounded phase. W3X releases
phases, not W3C.
...
point. Use this introduction's IMMEDIATE NEXT ACTION plus W3X's current
direction for the live Phase 3a review state until Project Status is reconciled.
