...
## 1.1 Document authority for Stage 2C (W3X-ratified 2026-08-05)

CONTROLLING for Stage 2C work: the charter (prevailing per 2.3a), the
active scope, and its read-together authority set (this index, D2, D3,
Addenda A/B, the creation-error table, the D1 pin + provenance).
README_Deblock4_Design_Spec_v1_9 is FALLBACK GENERAL GUIDANCE: where it
conflicts with the charter, a ratified decision record, or the authority
set, THE LATTER PREVAIL; it is consulted only on an IDENTIFIED MATTER the
authority set does not settle, with the matter and section named in the
consulting party's report (feeding the scheduled post-2C README currency
audit). Specifically SUPERSEDED for Stage 2C: README 12.5/12.6 (backend
auto-selection ignores implementation availability - superseded by D4 S5)
and README 8.1 (float coverage - superseded by D4 S1).
...
K30 FIRST-CLASS MODULE DISCIPLINE APPLIES TO THE NEW 2C MODULES
      (charter C-STY-10; from successor-W3C pre-implementation review
      F2a, W3D-verified, W3X-adopted 2026-08-05).
      classic_scalar_kernel.zig, classic_edge_schedule.zig and
      classic_thresholds.zig are FIRST-CLASS permanent modules:
      permanent names (no stage numbers, no probe/smoke vocabulary);
      one-way dependency (scaffolding may import first-class code,
      never the reverse); no first-class reference to any scaffolding
      file, symbol, marker or artifact. The textual first-class audit
      returning EMPTY for scaffolding identifiers is a DELIVERY
      OBLIGATION of the 2C scope, named in the O/G crosswalk with the
      ratified two-part domain below. AUDIT CONTRACT (W3X-ratified
      2026-08-05, the narrow non-cleanup form):
      (1) NEW 2C FIRST-CLASS MODULES (the three modules plus their
          build wiring and unit-test names): filenames, declarations,
          symbols, marker strings, comments and references audited,
          case-insensitively, for stage-number/probe/smoke vocabulary
          AND for every identifier on the enumerated retired-
          scaffolding list (the Stage 1C S2 list). Expected EMPTY.
      (2) EXISTING EDITED MODULES: the 2C CHANGES introduce no new
          scaffolding reference and no reference to any enumerated
          retired identifier. Pre-existing ACCEPTED Stage 1C regression
          identifiers (e.g. runStage1CPureContracts, stage_1c=PASS) are
          NOT renamed in this scope: they are gate-asserted by the
          re-executed matrix (the build_1C_v1.bat:195 gate class re-run
          in build_2C) and renaming them would break the proof surface
          2C must keep green. Any such cleanup is a separate,
          deliberately scoped future decision (post-2C hygiene-era
          candidate, recorded in Project Status).
      Applies: D4 sections 1/7/9; every later stage that adds
      first-class modules.
K32 CURRENT DELIVERY MECHANICS (charter v1_27) SUPERSEDE THE K17/K18-ERA
      WORDING for all scopes issued from 4C onward: no commit-hash or
      per-file base-hash requirement (base = prevailing repository state
      CONFIRMED WITH W3X, who guarantees the local repo IS the base); no
      repository-operating scripts, no PowerShell machinery, no staging
      ever; application and backout are manual W3X acts (apply_to_tree
      copy / per-file command block / restore_to_base data); commit is a
      manual W3X act after W3D review and W3X acceptance. K17/K18 remain
      as history of the 2C-era mechanics; on any conflict charter v1_27
      C-DELIV-01/10/11 prevail. (Origin: W3C 4C pre-implementation
      response F2; W3X-ratified 2026-08-12.)
      Applies: every 4C+ scope and delivery; the K30-style audit routing.

K33 THE HORIZONTAL TAIL TERMINATES IN A ONE-LANE VECTOR, NOT SCALAR
      The frozen width-generic body's descending power-of-two tail
      (filterHorizontalTail) terminates by applying the SAME vector body
      at one lane - filterHorizontalLanes(T, 1, ...), "V1" - not by the
      scalar-column function. Traced in source: at N=2 with one sample
      remaining, half=1 fires, that call consumes the sample, and the
      recursion returns on remaining==0 BEFORE the N==1 branch. The
      N==1 scalar-column branch is DEFENSIVE code, unreachable from any
      ratified entry width (4C: u8 16 / u16 8; 5C: u8 32 / u16 16).
      Byte-identity is unaffected either way (V1 is the identical i32
      arithmetic), but any tail table, scope text, or mutation control
      must NAME V1 as the live terminal. The Stage 4C wording "... 2,
      then scalar 1" (4C-RAT-4) MISNAMED this and is superseded; do not
      re-import it. Practical consequence proven at 5C-T5: a one-lane
      mutation must be planted in the V1 path to be a real control - a
      mutation in the defensive scalar branch would prove nothing.
      (Origin: W3D-F1 on the 5C pre-implementation response, coder
      independently re-derived it from source and concurred; ratified
      5C-RAT-6, 2026-08-14.)
      Applies: every vector stage, tail table, and tail-mutation control.

K34 CLASSIC IS BANDWIDTH- AND VERTICAL-PATH-BOUND: WIDER SIMD BUYS LITTLE
      MEASURED at Stage 5C on the W3X host (3900X), synthetic 720x576
      YUV420P8, strength 25, 240 frames, one discarded warm-up plus three
      recorded runs per backend: v1 0.3182 s (1.000x), v2 0.2853 s
      (1.115x), v3 0.2770 s (1.149x). AVX2 is only about 3 percent faster
      than SSE4.1. This is EXPECTED, not a defect: the vertical-edge path
      is width-invariant by algorithm (Schedule A's four-step dependency
      caps vertical lanes at 4 - 4C-RAT-3), so only horizontal batching
      widens with N; and the filter is substantially memory-bound. USE
      THIS when weighing any future width or tier proposal: further SIMD
      width alone is not where the remaining time is. No number here is
      normative or a target (charter A3, P-10); performance is measured
      and recorded, never a gate.
      (Origin: Stage 5C T6 non-gating benchmark record, W3D-verified from
      the retained JSON, 2026-08-15. Closes the previously
      measurement-gated "actual AVX2 speed benefit" open thread.)
      Applies: tier/width proposals; any future optimisation scoping.

K31 STRIDE UNITS ARE EXPLICIT AND CHECKED, EVEN IN SCALAR CODE
      (charter C-SIMD-03; from successor-W3C pre-implementation review
      F2b, W3D-verified, W3X-adopted 2026-08-05 - "any conversion to
      sample units must be explicit and checked, even in scalar code").
      VapourSynth frame strides are BYTE counts. The implementation
      either (a) addresses samples through byte arithmetic with the
      byte unit named at the point of use, or (b) performs exactly ONE
      conversion to a typed sample stride, naming both units and
      asserting the conversion is exact for the plane's bytesPerSample.
      Silent division is forbidden; applies to u8 and u16 storage
      alike. TWO PERMITTED MODELS (coder's choice; unit-suffixed names
      stride_bytes / stride_samples are the recommended idiom):
      (a) BYTE ROW NAVIGATION (RECOMMENDED default per W3X preference):
          the plane base stays a byte pointer; row addresses computed
          in BYTES (base + y*stride_bytes - no division exists); ONE
          named explicit cast to the sample type per row/plane view
          for within-row SAMPLE indexing. Proof: inspection plus the
          O-6e canaries and O-8c/O-8h; no conversion assertion is
          applicable.
      (b) TYPED SAMPLE STRIDE: exactly ONE checked conversion PER
          PLANE stride/view construction - @divExact(stride_bytes,
          bytesPerSample) or an explicit stride_bytes % bytesPerSample
          == 0 assertion - naming both units; typed indexing
          thereafter. Proof: inspection + the assertion plus O-6e and
          O-8c/O-8h.
      FORBIDDEN either way: the silent unchecked division (HolyWu
      deblock.cpp:237 and zsmooth remove_grain.zig:810 both use it -
      the surveyed ecosystem idiom this rule improves on) and any
      arithmetic whose unit is ambiguous at the point of use.
      Applies: D4 A1c/A6; 4C/5C vector addressing later.
```
...
K16 Creation-error message table v1_6 is the ratified source obligation:
...
K22 FLOAT-PATH FINE INVARIANTS (V&T 3.4-3.8): structural results stay
      ...
      explicitly. F16 STORAGE-NEVER-COMPUTE (Toolchain Findings v1_4
      F10; W3X 2026-08-05): f16 is a STORAGE width only - widen to f32
      on load, compute in f32 under .strict, narrow on store (f16
      arithmetic is ~100x slow per zig #19550); the compute-width
      semantics (B = f32-compute/f16-store) is PINNED BEFORE these
      tolerance numbers are derived, because tolerances are
      semantics-dependent. F16C confined to v3 objects.
      Applies: D2 (establish the arithmetic-type fact), D3, D4;
      Toolchain Findings v1_4 F10 carries the full rule + F16-1..F16-8.
...
false claims once it produces pixels - specifically, exception items
(a)-(e): (a) the float-input refusal, (b) the integer-depth refusal (K29)
and (c) the unimplemented-backend refusal carried by creation-error table
v1_6, (d) the Classic-only IMPLEMENTED-TIER AVAILABILITY cap
...
Deblock4Tier reports the tier actually executed) with the associated
exact EFFECTIVE-vs-availability precedence test, and (e) the D-2C-1
SUMMARY-EMISSION RELOCATION (W3X-ratified 2026-08-05): the existing
always-on creation summary print moves from capability detection to
final backend selection; the already-computed ACTUAL tier and
SummaryReason are exposed/transported to the caller; the
intentionally-capped presentation reason (D-2C-3) is added; NO change
whatsoever to detection tables, instructions, CPUID/XGETBV/XCR0
handling, ACTUAL/EFFECTIVE semantics, the force-down intersection, or
the G10 paths. NO other validation, creation-string, detection-LOGIC or
using-echo change. All
...
- No change to registration, detection LOGIC, G10 modules, or the
  using-echo surfaces, and NO validation/creation-string/tier-resolution
  change EXCEPT the narrowly authorised Stage 2C items of the section-5
  exception list (a)-(e) above (refusals, cap + precedence test, and the
  D-2C-1 emission relocation).
```
...
Revision: v1.13 (2026-08-12) K32 added: charter v1_27 delivery mechanics
supersede the K17/K18-era wording (W3C 4C response F2; W3X-ratified).
No other change.
Revision: v1.12 (2026-08-12) End-of-phase F10 propagation: K22 gains the
f16 storage-never-compute rule and the pin-compute-width-before-tolerance
ordering (Toolchain Findings v1_4 F10). No other change; no obligation or
index content altered.
Revision: v1.11 (2026-08-05) Focused re-review round (W3C F2/F3/F4,
W3X-ratified): section-5 exception relabelled (a)-(e) and gains (e), the
D-2C-1 summary-emission relocation; blanket prohibitions reworded to
detection LOGIC; K30 gains the ratified narrow non-cleanup audit
contract (accepted Stage 1C regression identifiers are gate-asserted
and not renamed; cleanup recorded as a post-2C candidate); K31 gains the
per-model proof with byte-row navigation recommended per W3X preference,
@divExact/assert for the typed model, the unit-suffix idiom, and the
named forbidden silent-division idiom (HolyWu/zsmooth survey).
Revision: v1.10 (2026-08-05) Successor-W3C pre-implementation review
round: added K30 (C-STY-10 first-class discipline for the new 2C modules;
F2a) and K31 (C-SIMD-03 explicit checked stride units even in scalar
code; F2b) - proposer W3C, verifier W3D, adopter W3X. Added section 1.1
recording the W3X document-authority ruling (README v1_9 = fallback
general guidance; 12.5/12.6 and 8.1 superseded for 2C).
Revision: v1.9 (2026-08-03) W3C v1.5-package review F1: table pointers
updated to v1_6; K29 applies-line corrected to D3 O-1d (not section 8).
v1.8 (2026-08-03) W3C updated-package review: added K29 (valid
...
clean-tree presumption (F6). v1.14 (2026-08-15) appended K33 (the horizontal
tail's live terminal is a one-lane VECTOR application, V1, not the defensive
scalar-column branch; the 4C "then scalar 1" wording superseded) and K34
(measured Stage 5C benchmark: Classic is bandwidth- and vertical-path-bound,
AVX2 ~3% over SSE4.1, closing the measurement-gated AVX2-benefit thread).
v1.4 (2026-08-02) added K26 (external-oracle execution pin, G6
