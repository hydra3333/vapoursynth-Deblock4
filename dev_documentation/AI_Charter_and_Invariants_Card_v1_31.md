...
Design specification:
    ...
    status            ITS CLASSIFICATION IS UNDER A PENDING W3X RULING. This
                      charter's header calls it the "Companion specification";
                      other clauses call it the "controlling specification".
                      That inconsistency is NOT resolved here, and NOTHING
                      about its status changes: the clauses that operate on
                      the controlling specification - I2, P-09, and the scope
                      attachment/quoting rules - are UNCHANGED and continue to
                      bind exactly as before.
                      For every MPEG-2 matter the ratified MPEG-2 authority
                      PREVAILS over this document regardless of its
                      classification.
...
## v1.31 (2026-08-18) - W3D-drafted, W3X-directed 2026-08-18

NEUTRAL WORDING ONLY. NO RULE CHANGES. NO INVARIANT CHANGES. NO CHANGE TO THE
README'S STATUS OR TO ANY CLAUSE THAT OPERATES ON IT.

THE CONTRADICTION THIS RESOLVES: this charter's own header calls the README
the "Companion specification" while the embedded bootstrap template called the
same document the "Controlling specification". Two names for one document
inside one document.

THE FIX: the template heading is now the neutral "Design specification", and
the block states plainly that the classification is under a pending W3X ruling
and that nothing about the status changes here.

WHAT WAS DELIBERATELY NOT DONE, AND WHY. W3X asked whether the README's
"Controlling specification" label should simply be removed. It must not be,
yet, for two independent reasons:

```text
1. IT IS NOT ONE LABEL. "Controlling specification" is load-bearing at seven
   sites in this charter, and four of them are RULES that OPERATE on it:
       the bootstrap template heading;
       the scope-attachment list ("the controlling README/specification");
       the scope package contents list;
       "The controlling specification is roughly three thousand lines.
        Attaching it in full is required by I2";
       "Scopes quote the controlling specification sections in full, inline";
       P-09 "A coding scope may implement the controlling specification";
       "It may not amend this charter or the README unless the scope [...]".
   Deleting the status would leave I2 and P-09 operating on an object with no
   named referent - P-09 would say a coding scope may implement nothing in
   particular. A correct change must NAME THE REPLACEMENT OBJECT.

2. RECLASSIFICATION IS A PROMISE ABOUT CONTENT, AND THE CONTENT IS UNSWEPT.
   The ratified decision DEC-07 in the Standing Task Register states that the
   README becomes user-facing documentation AND that it cannot be reclassified
   until the T1 sweep adjudicates it and T3 strips it. The README is 3,792
   lines and still contains ratified Architecture A design at sections 3.11
   and 3.13. Declaring today that it holds no controlling information would be
   a promise about material nobody has read for this question.
```

WHEN THE REAL CHANGE IS DUE: after T3 strips the README. W3D is to raise it
then as a full proposal enumerating all seven sites and naming the replacement
object for I2 and P-09. W3C verifies, having found the original defect. The
reminder is registered in the Standing Task Register so it cannot be lost.
