...
CURRENT STATE:
 ...
 - THE ACTIVE WORK IS T1, NOT T5/Q14 AND NOT CODE. T1S01a5 (authority
   sections 1-8) is CLOSED - DEC-85/88, final ledger v1.10, 44 entries, all
...
DO NOT CONFUSE THE T1 POPULATIONS - THERE ARE NOW FOUR:
...
4. Immediate action: T1 DOCUMENT REVIEW, NOT CODE. a5 is CLOSED (final
   ledger v1.10, 44 entries); NOTHING is owed on it. The next W3C action is
   a5b FIRST-BATCH REVIEW when the package arrives: attack the delivered
   population/coverage map as a claim (independent re-derivation, tiling
   verification), then the entries under scope v1.15. The a5 Tier C sample
   is COMPLETE - do not select another. Do not restart the settled
   search/classification round without a concrete new defect.
5. T1 populations: frozen historical T1S00 survey 47; current adjudication 41;
   settled a5 search 46 (history); declared a5b population 40 (map A.3).
   T1/ and GAIS_investigations/ are excluded from the
...
v1.37 (2026-08-21) Deep-staleness sweep from the W3X-requested fitness check
      ...
      list still said THE LEDGER REWRITE IS NEXT, checklist item 4 still
      described the v1.6->v1.7 delta review as the next W3C action, and the
      population rule counted three. All now state the closed-a5/active-a5b
      state, the delivered coverage map and the W3C attack posture on it,
      and FOUR population numbers. The same partial-replacement class the a5
      reviews kept finding, committed in this document's own refresh.
