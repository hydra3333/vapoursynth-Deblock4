...
Status update (v1.3): Stage 1A.1 and Stage 1B.1 are both COMPLETE and
committed. Stage 1B.2 is the active next stage.

TWO FILTERS, CLASSIC FIRST: the plugin will register two filters -
deblock4.Classic (H.264, built FIRST as a known/de-risking algorithm with an
external reference oracle) and deblock4.Deblock4 (the end-goal MPEG-2 algorithm,
built SECOND on proven infrastructure). The Stage 1B/1C infrastructure work
below is filter-agnostic; the per-algorithm stages (scalar core -> quality ->
SSE4.1 -> AVX2) run once for Classic, then again for Deblock4. See README v1.4
section 1.0 and Deblock4_Verification_And_Tiering_Decisions section 8.1. The
MPEG-2 filter remains the end goal; Classic-first is a sequencing choice only.

The original note below is retained for context.
...
Stage 1B.1   Backend object isolation and one-DLL linkage   [COMPLETE]
