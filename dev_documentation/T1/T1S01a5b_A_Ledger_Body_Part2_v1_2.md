...
**Date:** 2026-08-21
**Author:** W3D
**Status:** ADJUDICATION LEDGER, BATCH 1 CORRECTION GENERATION per W3C
review T1S01a5b_B (findings F1-F9) - authority sections 9 and 10, lines
716-876, 28 entries: LED-064..081 plus ten ratified-rule split/coverage
entries (066a, 067a, 070a, 070b, 073a, 074a, 074b, 078a, 078b, 081a).
THIS DOCUMENT IS THE ARTIFACT OF RECORD and supersedes v1.1. Correction
review is bounded to the reissued/new entries and REPAIR-v1_2 blocks (see
batch summary); "agreed" entries carry their T1S01a5b_B verdicts.
...
follows without deviation:
    ...
    738-745 -> LED-066 / 066a          833-834 -> LED-076
...
CARRIED FROM A PLAN: 28 - eighteen map segments, eight of which split
under the atomic-claim rule or gained coverage entries, adding ten
(LED-066/066a, LED-067/067a, LED-070/070a/070b, LED-073/073a,
...
F6. There is no target entry count.
```
...
LED-066   (reissued at v1.2 per T1S01a5b_B F3: the uncertainty clause is
           split to LED-066a; a subset carrier is no longer used to prove
           the whole five-item proposition)
  ...
  CLAIM        "Good engineering ideas retained from A: creation-time
               ...
               operation"
  ASSERTS      A retention claim: four named engineering practices survive
               A's rejection and remain adopted project practice.
  CLASS        Reasoned project decision record (a recorded retention, not
               a codec fact).
  DISPOSITION  CURRENT-DUPLICATE
  DUPLICATE-ACTION  STAY-CANONICAL
  REASON       The RETENTION claim for these four practices is true and
               carried elsewhere: the Re-Decision Evaluation's final
               recommendation 4, lines 1072-1073 - "RETAIN A's good
               engineering ideas - immutable creation-time threshold sets,
               fixed-point scaling, no hidden state" - with lines 633-654
               (A's determinism/statelessness and float-free arithmetic
               argued as retained virtues) completing coverage of the
               no-float and deterministic/stateless items as COMBINED
               PASSAGES of one document, cited as such. The v1.1 entry
               used the 1072-1073 subset alone to prove a five-item
               proposition; W3C's F3 correctly rejected that inference,
               and the fifth item now has its own bounded entry
               (LED-066a).
               DISTINCTION HELD DELIBERATELY (unchanged from v1.1): the
               practices are separately asserted as live rules in their
               own homes (charter E2/G4/F-series; README kernel rules).
               Those assertions are the practices THEMSELVES, not the
               claim that they were retained FROM A, and were classified
               DIFFERENT.
  ...
  SWEPT        Population: 38 files. Probe family: {"retained from A",
               ...
               stateless"}. Files with hits, opened and classified:
                 authority (self): 9.1 in-range; section 8 lines 690-696
                   (research-reasoning retention list - DIFFERENT subject,
                   settled a5 ground);
                 Re-Decision Evaluation: CARRIER as combined passages
                   (1072-1073 with 633-654, per REASON);
                 Re-Decision Brief: APPLIES (164-172, predates retention);
...
LED-070b  (new at v1.2 per T1S01a5b_B F1.3: authority line 763's B2
  ...
  REASON       True and among the most widely duplicated status
               ...
               159. Same-file out-of-range copies (D4-D12 register 1658;
               Appendix material 1804-1806) are carried below; revision-
  ...
  SWEPT        Population: 38 files. Probe family: {"PRIMARY CANDIDATE",
               ...
               (canonical, in-range), 1658/1804-1806 (a6, below), 1976
  ...
  CITED-OUTSIDE-RANGE
               locations: D4-D12 register (1658); Appendix A/B material
               (1804-1806)
...
LED-074b  (new at v1.2 per T1S01a5b_B F6: the row-origin and 16-pixel
           segment geometry, split from LED-074 - duplicated facts must
           not ride inside a uniqueness claim)
  ...
  REASON       True and duplicated. Canonical home: the section 10
               ...
               rule and the tables. Known non-canonical copies: section 0
               item 9 line 137 ("per 16-pixel x segment" - settled a3
               layer, CARRIER); Re-Decision Evaluation line 71 ("For each
               16-pixel horizontal macroblock segment" - CARRIER, the
               originating proposal). The M = 16*m row-origin formulae in
               section 4.3 (lines 463-470) are settled a5 ground, cited
               not re-adjudicated.
  CONFLICTS    None.
  PREVAILS     n/a
  SWEPT        Population: 38 files. Probe family: {"16-pixel x segment",
               "occupies a 16-pixel", "M = 16*m", "16-pixel ... macroblock
               segment"}. Hits: authority 137 (a3), 463-470 (a5), 809-815
               (in-range); Re-Decision Evaluation 71 (CARRIER); no other
               file matched.
  TIER         C
  PROPOSED
  ACTION       None.
...
LED-079
  ...
  REPAIR-v1_2  (per T1S01a5b_B F7; DEC-50 evidence enumeration)
               ...
               matching; loci deduplicated per line), every file opened:
                   ...
                   AI charter v1.31                  11
                   Concise Summary v1.8               1
                   dispatch explainer v1.4            5
               ...
               RECONCILIATION OF THE COUNT: v1.1's "81" was the raw
               regex match count BEFORE per-line deduplication; 76 is
               the deduplicated locus count under the stated method.
               Both bases are stated so the number is attackable; the
               classification classes and conclusions are unchanged.
               W3C independently reproduced the 18-file result set.
  TIER         C
  PROPOSED
  ACTION       None.
  VERDICT      (W3C - correction review)
...
LED-081a  (reissued at v1.2 per T1S01a5b_B F9: disposition corrected from
  ...
  PROPAGATION  REQUIRED FOR ERRONEOUS - executed per DEC-84 scope 0.11,
               ...
               1. DECLARED SCOPE / POPULATION: the 38-file current
                  population (Population Delta v1.0); the batch-1 T1
                  process artifacts (Population Delta v1.0, Population and
                  Coverage Map v1.0); the src tree (code reliance check).
               ...
               3. CANDIDATES ENUMERATED AND CLASSIFIED:
                    ...
                    Population and Coverage Map v1.0 lines 232-234 and
                      ...
                      routing);
                    all other current "section 15" references - NON-
                      RELIANCE (they reference section 15 for the Q14
                      experiment itself, which is correct: the experiment
                      IS in section 15; only the revisit-obligation
                      routing was wrong);
...
```text
ENTRIES THIS BATCH: 28 (v1.1's 24, plus four added at v1.2 per W3C
findings: LED-066a, LED-070b, LED-073a, LED-074b)

    LED-064  CURRENT-DUPLICATE  STAY-CANONICAL   C   repaired (F1.1/F2)
    LED-065  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-066  CURRENT-DUPLICATE  STAY-CANONICAL   C   reissued (F3)
    LED-066a CURRENT-UNIQUE     n/a              C   new (F3)
    LED-067  CURRENT-DUPLICATE  STAY-CANONICAL   C   repaired (F1.1/F2)
    LED-067a CURRENT-DUPLICATE  STAY-CANONICAL   C   reissued (F4)
    LED-068  CURRENT-DUPLICATE  STAY-CANONICAL   C   repaired (F1.2)
    LED-069  CURRENT-DUPLICATE  STAY-CANONICAL   C   amended (F1.2)
    LED-070  CURRENT-DUPLICATE  STAY-CANONICAL   C   amended (F1.3)
    LED-070a CURRENT-DUPLICATE  POINTER (prov.)  C   agreed
    LED-070b CURRENT-DUPLICATE  STAY-CANONICAL   C   new (F1.3)
    LED-071  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-072  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-073  CURRENT-DUPLICATE  STAY-CANONICAL   C   reissued (F5)
    LED-073a CURRENT-UNIQUE     n/a              C   new (F5)
    LED-074  CURRENT-UNIQUE     n/a              C   reissued (F6)
    LED-074a CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-074b CURRENT-DUPLICATE  STAY-CANONICAL   C   new (F6)
    LED-075  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-076  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-077  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-078  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-078a CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-078b CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-079  CURRENT-DUPLICATE  STAY-CANONICAL   C   repaired (F7)
    LED-080  CURRENT-DUPLICATE  STAY-CANONICAL   C   agreed
    LED-081  CURRENT-DUPLICATE  STAY-CANONICAL   C   repaired (F8)
    LED-081a SUPERSEDED (ERRONEOUS)  n/a         A   reissued (F9)

DISPOSITIONS, derived by counting the table: 24 CURRENT-DUPLICATE,
3 CURRENT-UNIQUE (LED-066a, LED-073a, LED-074), 1 SUPERSEDED with
SUPERSEDED-KIND ERRONEOUS (LED-081a), zero CONFLICTING, zero
OPERATIVE-SPEC.
TIERS (derived, DEC-62): 27 C, 1 A (LED-081a).

REPAIR CONVENTION OF THIS VERSION: entries marked "reissued"/"new" are
whole entries; entries marked "repaired"/"amended" carry their v1.1 body
unchanged plus a clearly-labelled REPAIR-v1_2 block extending the named
fields. The correction review is bounded to the reissued/new entries and
the REPAIR-v1_2 blocks; "agreed" entries carry their T1S01a5b_B verdict
and are not re-opened.

ON THE NEAR-ZERO TIER-A OUTCOME AGAINST THE 0f EXPECTATION (confirmed by
W3C's own review): sections 9-10 DESCRIBE rejected architectures; they do
not ASSERT them. The batch's one Tier A (LED-081a) is an erroneous
internal cross-reference under DEC-84 - the first real ERRONEOUS case the
rule anticipated - with mandatory propagation executed in the entry. The
larger Tier-A pressure, if it comes, is expected at section 12's
rejection-proof mathematics (a later a5b batch) and at the README's own
adjudication (T1S02). ONE addition from W3C's review: the current Grid
Knowledge v1.2 still asserts old-A midpoint machinery as current
knowledge; that conflict is now recorded at LED-064/067 (authority
prevails; the document is superseded-declared and already on its T2
retirement route).

CITED-OUTSIDE-RANGE RECORDS CARRIED: 16, derived by enumerating the
entries that carry one: LED-064, 067, 067a, 068, 069, 070, 070a, 070b,
073 (section 9) and LED-078, 078a, 078b, 079, 080, 081, 081a (section
10). The other twelve entries carry none.
...
IN-BATCH RECONCILIATIONS COMPLETED: LED-070 <-> LED-074a (layer contract
vs coalescing condition); LED-066 <-> LED-066a and LED-073 <-> LED-073a
and LED-074 <-> LED-074b (atomicity splits, recorded in both directions);
LED-064/067 and LED-068/069 heading-occurrence cross-records (recorded in
both directions).

MAP AMENDMENT RECORDED (pending map v1.1, W3X commits tree artifacts):
Part B segments 738-745, 762-785, 796-803 and 806-817 now map to
LED-066/066a, LED-070/070a/070b, LED-073/073a and LED-074/074a/074b
respectively; Part D's "LED-081 -> section 15" routing is corrected to
section 16 per DEC-84 propagation.
...
(corrected as Delta v1.1, issued with this ledger), Coverage Map
v1.0:232-234/332 (correction at map v1.1). No code/kernel reliance. Full
record in the entry.

DECISION MATERIAL FOR W3X, VIA W3C CORRECTION REVIEW FIRST: LED-081a's
