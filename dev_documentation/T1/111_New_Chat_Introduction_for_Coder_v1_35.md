...
CURRENT POSITION - UPDATED AT v1.35. THE RECOVERY IS OVER.

    WHAT HAPPENED, so you do not re-derive it: a W3D designer chat died after
    delivering Classification Repair v1.1 and before the promised ledger
    rewrite. A W3C session reconstructed the ledger as v1.4, REPORTED ITS OWN
    RECONSTRUCTION AS LOSSY, and recommended rebuilding from delivered v1.3
    instead. W3X ruled v1.4 out as a source (DEC-70). A successor W3D rebuilt
    on v1.3 and the ledger has since been reviewed twice by W3C.

    v1.34 OF THIS DOCUMENT SAID "THE NEXT DESIGNER ACT IS THE LEDGER REWRITE".
    THAT IS FOUR GENERATIONS STALE. Do not act on it.

    WHERE a5 ACTUALLY IS:
        ledger v1.7 delivered, 42 entries;
        it applies W3C's recovery-closure review of v1.6;
        THE NEXT W3C ACT IS A DELTA REVIEW OF v1.6 -> v1.7.

    THE DELTA-REVIEW AGENDA IS W3C's OWN, from section 18 of
    T1S01a5_B_Recovery_Closure_Response_v1_0.md, which travels in the batch:
        1. delta-review v1.6 -> v1.7;
        2. verify LED-043, LED-055 and LED-061 END-TO-END, not merely in the
           field that was edited - all three were previously corrected in one
           field while another field of the same entry still contradicted it;
        3. verify Q-A coverage and entry ranges;
        4. verify the LED-047 tested-count repair (seven -> eight);
        5. verify LED-059 against authority section 15;
    ...
    PLUS ONE ADDITION: verify Deblock4_T1_W3C_Review_Scope_v1_12 AS A
    CONSOLIDATION - does its new section 0 state each rule as the ratifying
    decision states it, and was anything weakened or dropped in the move?
    ...
    THE BINDING REVIEW SCOPE IS NOW Deblock4_T1_W3C_Review_Scope_v1_12, NOT
    v1.11. Its new section 0 consolidates the method rules that had spread
