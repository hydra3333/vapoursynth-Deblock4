...
**Version:** 1.8 - post-repair rewrite. Built TEXTUALLY on delivered ledger v1.3 with only the deltas mandated by Classification Repair v1.1, W3C review response v1.0, and ratified register decisions, plus W3C's Tier C sample review and both recovery
closure reviews. 43 entries, all Tier C.
**Date:** 2026-08-19
...
**Binding review scope:** `Deblock4_T1_W3C_Review_Scope_v1_13.md`
**Binding work queue:** `Deblock4_Standing_Task_Register_T_Series_v1_33.md`
**Supersedes:** ledger v1.7 (same authorship and basis; v1.8 applies W3C's
delta review). Textual basis remains delivered ledger v1.3.
...
AT v1.7 THE COUNT MOVES TO 42, and the movement is itself the finding:
 +  LED-032a   authority line 225, project scope        uncovered proposition
 +  LED-037a   authority line 269, default-scope rule   uncovered proposition
 +  LED-055a   the derivation qualifiers                atomic split W3C
                                                        required at Tier C
= 42 entries at v1.7.

AT v1.8 THE COUNT IS 43. ONE ENTRY IS ADDED:
 +  LED-055b   the progressive-material generality, split out of LED-055a,
                which had bundled it with the correctness-versus-performance
                qualifier and called the pair unique.

THE COUNT WAS DRAFTED AS 45 AND CORRECTED TO 43 BY COUNTING THE ENUMERATION,
and the error is recorded because it is the third time an asserted total in
this ledger has been wrong. 42 + 1 = 43. The enumeration at 0.4i is generated
from the finished document, not written from memory - which is the only reason
the draft figure was caught.

ALL 43 ARE TIER C. ZERO TIER A. LED-063 was the ledger's only Tier A entry and
its re-disposition from CONFLICTING to CURRENT-UNIQUE computes to C under
DEC-62. W3C ruled in advance that a Tier-A count must not be preserved as a
target; it falls out of the evidence, exactly like the entry count.

TWO OF THE THREE ARE COVERAGE FAILURES, NOT REFINEMENTS. Lines 225 and 269 sat
inside the declared 223-715 range with no entry at all, through five ledger
generations and a Tier C sample, until W3C checked the source range against the
entry ranges at the closure review. AN OVERLAP CHECK AGAINST PRIOR ENTRIES IS
NOT A COVERAGE CHECK AGAINST THE SOURCE. This ledger enumerated the former
meticulously from v1.0 and never did the latter.
...
REASON       Current, and stated widely. The identity is in the README, both
             introductions, Project Status, the Concise Project Summary and
             the charter. The PAL 576i / consumer-DVD-recorder target is in
             the README and the introductions.
...
DUPLICATE-ACTION  POINTER.
             ...
             README's opening description of Deblock4 as the MPEG-2-aware
             filter aimed at PAL 576i consumer-recorder material. ALSO NAMED:
             the designer introduction's project block. NOT EXHAUSTIVE.
...
SWEPT        A BOUNDED PROPOSITION-SPECIFIC PROBE WAS RUN AT v1.8, because the
             ...
             PROBE FAMILY: "Unless explicitly stated otherwise" | "unless
             stated otherwise" | "unless otherwise stated".
             POPULATION: the settled 46-file a5 search population at 0.5.
             1 CANDIDATE FILE, CLASSIFIED:
               CANONICAL   this authority, line 269 - the only occurrence of
                           the family anywhere in the population
             The second and third probe terms returned NOTHING. That negative
             result is recorded rather than omitted.
             CLAIMED SCOPE, PER DEC-50: uniqueness within the declared
             population under the declared probe family. A document scoping its
             own conventions in wholly different vocabulary - "these are
             defaults", "except where noted" - would not have been returned,
             and this entry does not claim otherwise. The claim is narrow and
             testable, which is what it was missing at v1.7.
...
SWEPT        THE AUDIT RESULT WAS TESTED WITHIN ITS OWN POPULATION, because
             ...
             6 CANDIDATE FILES, EVERY ONE OPENED AND CLASSIFIED:
               ...
               CARRIER     Scopes/ D4 PreScope Round Brief coder response -
                           ...
                           NOT THE MAPPING.
               CARRIER     Scopes/ D4 Verification Round Brief v1.0     - same
               CARRIER     Scopes/ D4 Verification Round Brief v1.1     - same
               CARRIER     Scopes/ D4 PreScope Round Brief v1.2         - same
...
SWEPT        REBUILT FROM CLASSIFICATION REPAIR v1.1.
             ...
             redefinition. The correct record is:
               ...
               OCCURRENCES:
                 section 4.5, lines 528-529   CANONICAL occurrence, adjudicated
                                              by this entry
                 Appendix A, lines 1762-1932  CARRIER occurrence, owned by
             ...
             returned ONE file; the proposition occurs twice in it.
               CARRIER     Scopes/ D4 PreScope Round Brief coder response -
                           ...
                           pitch 2
CITED-OUTSIDE-RANGE:
             location      authority Appendix A, lines 1762-1932
...
DOCUMENT     authority v1.05, section 5, lines 562-564
             RANGE CORRECTED AT v1.8 per W3C.
...
DISPOSITION  CURRENT-UNIQUE
REASON       Unique on a SEARCHED basis - see SWEPT, which is a bounded probe
             for THIS proposition rather than evidence inherited from a probe
             that searched something else.
             WHY IT MATTERS MORE THAN IT LOOKS: without it, the whole-frame
             input contract reads as an optimisation a future scheduler could
             trade away for speed. It is the sentence that makes the contract
             non-negotiable, and losing it in a T3 reduction would reintroduce
             the rejected architecture by the back door.
CONFLICTS    none.
PREVAILS     n/a - unique. Canonical home is this section, where the qualifiers
             sit against the mechanism they qualify.
SWEPT        A BOUNDED PROPOSITION-SPECIFIC PROBE WAS RUN AT v1.8, replacing
             v1.7's inherited-evidence claim.
             PROPOSITION SEARCHED: that whole-frame input is a CORRECTNESS
             requirement rather than a performance preference.
             PROBE FAMILY: "correctness requirement, not a performance" |
             "not a performance preference" | "correctness requirement".
             POPULATION: the settled 46-file a5 search population at 0.5.
             1 CANDIDATE FILE, CLASSIFIED:
               CANONICAL   this authority, section 5 - the only occurrence of
                           any probe term anywhere in the population
             CLAIMED SCOPE, PER DEC-50: uniqueness within the declared
             population under the declared probe family. A document making the
             same point in wholly different vocabulary - "this is not
             optional", "not a speed trade-off" - would not have been returned.
             WHAT THIS PROBE DOES NOT COVER, and what v1.7 wrongly bundled into
             it: the PROGRESSIVE-MATERIAL GENERALITY, now at LED-055b, which is
             DUPLICATED.
...
DOCUMENT     authority v1.05, section 5, line 562
...
ACTION   RETAIN THE COMPOSITION IN PLACE at 6.3 as canonical; POINTER for the
         ...
         v1.7 CARRIED SUPERSEDED v1.6 ACTION PROSE IN THIS LIVE FIELD. It has
         been moved to the entry's REASON, where correction history belongs.
         A live ACTION field states the current proposal and nothing else.
         [historical note relocated - former v1.6 text began: W3D specifically
...
DISPOSITION  CURRENT-UNIQUE
             ...
             from the disposition, so a5 now has ZERO Tier A and 45 Tier C.
...
REASON       BOTH NAMED LOCATIONS RESOLVE, WHICH IS WHY THE PROPOSITION IS
             SIMPLY TRUE. The raw GAIS evidence files exist on disk - six
             documents in GAIS_investigations/ plus two response captures in
             the root - although DEC-66 removes that tree from T1 search and
             adjudication, so they are history rather than population members.
             ...
             not conjectured. Only section 24's R8 filename is wrong. Section
...
SWEPT        POPULATION AND BASIS, per DEC-50. FIVE CHECKS, not four - the
             ...
             (5) THE REFERENT IS ESTABLISHED, NOT MERELY LIKELY. UPGRADED AT
...
Q-K2 THE TIER C FINDINGS, ALL EIGHT - AND v1.6's CLAIM IS WITHDRAWN.
     v1.6 asserted that all eight were applied. THEY WERE NOT: W3C found
     LED-043, LED-055 and LED-061 only partially propagated, each corrected in
     the named field and left contradicting itself elsewhere. v1.7 applies all
     three end-to-end and the claim is made only now. The original question
