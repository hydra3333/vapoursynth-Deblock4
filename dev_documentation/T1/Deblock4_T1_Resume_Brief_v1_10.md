...
# 0a. STATE ADVANCE (v1.10) - WHAT IS OWED RIGHT NOW
...
T1S01b    Scopes/ ONLY - the architecture
          re-decision record. The GAIS
          half is REMOVED by DEC-66 .... NOT STARTED
...
SEARCH POPULATION - three mechanical path tests, no judgement:
    ...
    EXCLUDE  GAIS_investigations/ - IGNORED FOR EVERYTHING,
             search AND adjudication, same footing as superseded*  DEC-66
...
## What DEC-66 changed, because not all of it is obvious

```text
ADJUDICATION POPULATION  47 -> 41.   SEARCH POPULATION unchanged at 46.
T1S01b becomes SCOPES-ONLY.
LED-023's DISCHARGE ROUTE CHANGES AND GETS STRICTER. That entry, owed to
  T1S01a7, tests the header's blanket claim that nothing in the authority
  rests on unverified GAIS testimony. It can no longer be discharged by
  COMPARING the authority against the GAIS files - those are now ignored. It
  must be discharged CONSTRUCTIVELY: every claim must have a recorded
  non-GAIS basis. THAT IS THE SAME SHAPE AS TASK T8, and a7 and T8 should
  discharge it together rather than twice.
NOTHING IS DELETED. Ignored is not destroyed; the files stay as history and
  DEC-66 is the record of why they were set aside.
```
...
```text
  ...
  Q16  T1S00 bump to v1.5 - AUTHORISED at    -> AFTER the a5 classification
       DEC-69, delivery-describing parts        repair and ledger rewrite.
       only, frozen frame untouched, and        Do not run a fourth document
       the bump must say so in itself           in flight while a5 is
                                                mid-correction.
ANSWERED: Q7 withdrawn. Q8 sample selected. Q10 -> task T8. Q11 -> DEC-64.
Q12 -> DEC-66. Q13/Q15 -> DEC-67. Q14 -> re-sweep all 34. Q1 -> DEC-68, the
a5/a5b split is RATIFIED. Q16 -> DEC-69, authorised and sequenced.

STANDING HABIT, W3X-directed: the task register and THIS BRIEF are COMMITTED
AFTER EVERY BUMP, not at batch boundaries. Nine durable findings live only in
the register, and it sits in T1/ - which DEC-63 makes unsearchable. Its
survival depends entirely on being committed, never on being found.
```
