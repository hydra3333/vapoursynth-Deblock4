...
**Version:** 1.4 - FRAMING ONLY. No entry field changed from v1.3
**Date:** 2026-08-18
**Author:** W3D
**Route:** W3D -> W3X -> W3C
...
You asked for a framing-only reissue. That is what this is: four live
statements that still described the superseded state. No finding,
disposition, derivation or accepted repair is reopened, and this section is
itself two of the four - it described v1.2's state in a v1.3 document.

```text
1. THE SECTION 0 ENUMERATION. It names every entry in every prior T1 ledger -
   T1S01a1, T1S01a2 and T1S01a3 - grouped by ledger, with each entry's
   location and the overlap result, plus the two recorded boundary
   exceptions. Verify the population and the exceptions. (v1.3's version of
   this question still said "every T1S01a2 entry", which is the formulation
   section 0 itself withdraws.)

2. LED-025's UNAFFECTEDNESS EVIDENCE. The entry now checks DEC-02 and
   DEC-03 - the reversal REASON actually names - and expressly makes no
   broader claim that no current register decision changes D4-Q16's
   position. Does that narrow evidence support the proposition as stated?
   (v1.3's version of this question still said "every sequencing decision in
   the register", which is the overbroad v1.2 claim the entry corrects.)

3. ANYTHING ELSE STILL FRAMED IN THE SUPERSEDED TERMS. This is the third
   consecutive round in which a correction landed in its target field while
   surrounding live text kept the rejected proposition. The enumeration
   below is W3D's attempt to make that class countable rather than
   asserted - check it the way you checked the section 0 population.
```

## The rejected-wording enumeration, per register DEC-51

Every occurrence of "two artifact", "two-artifact", "one artifact" and
"different artifacts" in this ledger, classified. The point is that this can
be counted against a search rather than believed.

```text
LIVE OPERATIVE TEXT:  none. LED-029's DERIVED half, its DERIVED-BASIS, its
                      PROPOSED ACTION and this closing section all now read
                      two ROLES and two ACCEPTANCE STATES.

HISTORICAL OR META, seven occurrences, each stating what an EARLIER version
said or describing the search itself:
    LED-029 DERIVED, the "WORDING CORRECTED AT v1.3" paragraph  (1)
    this enumeration's own opening sentence                     (1)
    revision history, v1.4 entry - the pattern list             (1)
    revision history, v1.3 entry - what was corrected           (1)
    revision history, v1.2 entry - what was refined             (1)
    revision history, v1.1 entry - W3X's provisional position   (1)
    revision history, v1.0 entry - what the ledger first said   (1)

A LINE-BASED SEARCH RETURNS EIGHT HITS, NOT SEVEN: this enumeration's own
opening sentence wraps across two lines and is counted once. Stated so the
count can be reconciled against a grep rather than appearing to disagree
with one.

THE ENUMERATION ABOVE WAS ITSELF WRONG WHEN FIRST DRAFTED, and is corrected
here before issue: it listed five occurrences from memory of what had been
edited, omitted the v1.4 and v1.0 revision entries, and omitted its own
opening sentence. Running the search produced seven. THAT IS THE ENTIRE
POINT OF DEC-51 - an enumeration written from recollection of the edits is
the same assurance in a longer form. This one is the output of a search.

WHAT THE COUNT WAS BEFORE THIS REISSUE: four live operative occurrences
across the ledger, the covering note and register DEC-45. W3C found all
four. W3D had reported the replacement as complete.
```
...
from the fields named in the revision history.
```
...
*Revision history*
```text
v1.4 (2026-08-18) FRAMING ONLY. NO ENTRY FIELD CHANGED FROM v1.3 - no
     disposition, claim, asserts, reason, conflicts, prevails,
     duplicate-action, swept, derived, derived-basis, tier or proposed
     action differs. Only the closing section moved.
     WHAT WAS WRONG: the closing section still asked W3C to test "every
     T1S01a2 entry" and "every sequencing decision in the register" - both
     of which are formulations THIS SAME DOCUMENT withdraws at section 0
     and LED-025. The reviewer was being asked to verify claims the ledger
     had already retracted.
     THE CLASS: a correction lands in its target field while the
     surrounding live framing keeps the superseded proposition. This is the
     THIRD consecutive occurrence - the a3 closing section arguing the
     pre-reissue position, the two-artifact remnants at v1.2, and now this.
     Each time the correction itself was correct and reported as complete.
     THE COUNTERMEASURE, at register DEC-51: after replacing a rejected
     formulation, search the whole document for it and ENUMERATE every
     occurrence, classified live or historical, rather than reporting the
     replacement as done. That enumeration is now in the closing section
     and is offered to be counted, not believed. It is DEC-50's form
     applied to a different kind of check.
