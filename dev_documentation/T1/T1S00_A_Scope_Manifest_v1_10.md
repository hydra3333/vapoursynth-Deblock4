...
**Version:** 1.10 - TERM SET STILL FROZEN; CURRENT STATUS PARAGRAPH ONLY
**Date:** 2026-08-21
...
**Author:** v1.6 - W3D (successor session), record corrections only.
v1.5 - W3C, as a recovery reconstruction after the W3D session limit; the v1.5
header incorrectly recorded "W3D". See the v1.6 correction note below.
**Route:** W3D -> W3X -> W3C
**Status:** FROZEN HISTORICAL SEARCH FRAME; T1 IS IN PROGRESS. W3X authorised
v1.5 under DEC-69 for DELIVERY-DESCRIBING EDITS ONLY. The 90-term set, the
surveyed population tables, every recorded hit count and section 5's weaknesses
remain exactly as frozen. Later T1 search/adjudication exclusions and delivery
routing are described here without pretending the 2026-08-18 survey was rerun.
**Encoding:** US-ASCII; CRLF.

---

# v1.6 RECORD CORRECTIONS - W3X-DIRECTED, NO CONTENT CHANGE

```text
THE FROZEN FRAME IS NOT REOPENED. The 90 terms, the surveyed population
tables, every hit and line count, section 5's weaknesses and the whole of
v1.5's delivery-describing text are UNCHANGED. Three record defects are
corrected, each recorded rather than quietly fixed because an unrecorded
correction is indistinguishable from an undiscovered one - the DEC-59 lesson.

DEFECT 1 - WRONG AUTHOR OF RECORD. v1.5's header read "Author: W3D".
    IT WAS NOT WRITTEN BY W3D. The designer session reached its context limit
    after delivering Classification Repair v1.1 and before delivering anything
    further. v1.5 was produced afterwards by W3C during recovery, and its own
    revision note shows it: it describes advancing the status "through
    Classification Repair v1.1 and the W3C recovery-ledger v1.4 verification
    gate" - a gate that did not exist until W3C created ledger v1.4, after the
    designer was gone.
    WHY IT MATTERS RATHER THAN BEING A CREDIT LINE: the Standing Task Register
    was explicit for the ledger that a chat reset does not turn W3C into an
    independent verifier of W3C-authored text. That care was taken over ledger
    v1.4 and NOT taken here. A document inside the frozen search frame carrying
    the wrong party as its author misstates the provenance of the frame itself,
    and provenance is what T1 exists to establish.
    NOT IMPLIED: no criticism of the content. v1.5's edits were independently
    checked against DEC-69's five permitted classes and stay inside them. The
    frozen tables, the 90 terms, the 47-document survey, the ~5,741-line and
    1,114-hit figures and the T1S02-T1S05 assignments were verified present and
    unchanged.

DEFECT 2 - EXECUTED OUT OF THE RATIFIED SEQUENCE.
    DEC-69 authorised the bump and sequenced it explicitly: "AFTER the T1S01a5
    classification repair and ledger rewrite. It is orientation hygiene, not a
    blocker, and W3D should not have a fourth document in flight while a5 is
    mid-correction." The ledger rewrite was not delivered before v1.5 was
    produced. The bump ran first, during recovery, in the exact condition
    DEC-69 was written to prevent.
    NO REMEDY IS PROPOSED. The constraint existed to protect designer attention
    during a5, and that risk has passed - the a5 ledger rewrite is now
    delivered at ledger v1.5. Undoing or re-running the bump would cost work
    and protect nothing. WHAT IS OWED IS THE RECORD, NOT THE REMEDY. A later
    step finding that DEC-69's sequencing was not honoured has found a DECISION
    ALREADY TAKEN and should propose no correction.

DEFECT 3 - THE REVISION HISTORY HAD NO HEADING AND NO OPENING FENCE, so the
    document's code fences did not balance and the history rendered as loose
    prose.
    ATTRIBUTED ACCURATELY, BECAUSE IT WOULD BE EASY TO BLAME THE WRONG PARTY:
    THIS IS NOT W3C's. It is INHERITED FROM v1.4 OR EARLIER - v1.4 has the same
    unbalanced fence count and the same missing heading. v1.5 carried it
    forward without introducing it. Corrected at v1.6 by restoring the
    "*Revision history*" heading and the opening fence.
```

---

# v1.5 DELIVERY-ONLY CURRENCY NOTE - DEC-69

```text
THIS DOCUMENT'S SEARCH FRAME IS STILL FROZEN.

v1.5 DOES NOT REOPEN T1S00.
It does not add/remove a registered search term.
It does not alter the frozen survey population tables.
It does not alter any hit or line count.
It does not rewrite section 5's recorded weaknesses.

W3X authorised only five classes of delivery-description repair:
    1. the T1S01a sub-tranche list;
    2. the current status description;
    3. the later mechanical exclusion statement;
    4. the current definition of T1S01b;
    5. stale pinned generations in delivery-routing prose.

Therefore a reader must distinguish:
    FROZEN T1S00 SURVEY RECORD            47 documents;
    CURRENT T1 ADJUDICATION POPULATION   41 documents;
    SETTLED a5 SEARCH POPULATION          46 files.

Those later numbers do NOT retroactively edit section 3.
```
...
## 2.2 CURRENT T1 MECHANICAL EXCLUSIONS - DELIVERY DESCRIPTION ONLY

This subsection records later W3X decisions without altering the frozen
2026-08-18 section-3 survey tables.

```text
CURRENT T1 SEARCH EXCLUSIONS:

DEC-60
    Exclude any file whose path contains a directory component beginning:
        superseded
...
DEC-63
    ...
    T1/ is a WORKSHOP / PROCESS tree. Its contents can record proposals,
    reviews, evidence and recovery state, but physical presence there does not
    make material applicable project knowledge.

DEC-66
    Exclude everything under:
        GAIS_investigations/

    For T1 this tree is ignored for BOTH SEARCH AND ADJUDICATION. The files
    remain historical research captures; they are not used to establish,
    refute or adjudicate current T1 propositions.

CURRENT CONSEQUENCE:
    adjudication population = 41 documents.

a5 RE-SWEEP / CLASSIFICATION-REPAIR CONSEQUENCE:
    settled search population = 46 files.

HISTORICAL CONSEQUENCE:
    NONE. Section 3 remains the frozen 47-document survey record and section 4
    remains its frozen comparison record.
```

This is the exact distinction DEC-69 exists to make visible: later process
rules changed what subsequent T1 work searches/adjudicates, but did not change
what T1S00 actually surveyed when its frame was frozen.
...
```text
T1S00  This manifest. No adjudication. The historical frame below remains
       exactly the one frozen on 2026-08-18.

T1S01  FROZEN ORIGINAL ASSIGNMENT RECORD:
       THE ARCHITECTURE RECORD - 13 documents, ~5,741 lines, 1,114 hits
       (395 + 486 + 233; v1.0 stated 1,109, an arithmetic error W3C caught).
       The frozen survey grouped the MPEG-2 authority, all six Scopes/
       documents and all six GAIS_investigations/ documents.
       Those counts and that historical grouping are NOT rewritten by v1.5.

       CURRENT DELIVERY ROUTE:
         T1S01a   the ratified MPEG-2 authority document.
         T1S01b   SCOPES-ONLY working record: the six Scopes/ documents.
                  GAIS_investigations/ is no longer part of T1S01b because
                  DEC-66 excludes it from both T1 search and adjudication.

       T1S01a is delivered in EIGHT declared sub-tranches:

         T1S01a1   PR-1 and PR-2 only.
         T1S01a2   header + currency statements.
         T1S01a3   section 0.
         T1S01a4   section 23 tail, steps 6-10.
         T1S01a5   BODY PART 1: sections 1-8, lines 223-715.
         T1S01a5b  BODY PART 2: sections 9-13, lines 716-1098.
         T1S01a6   sections 14-22, section 24, Appendices A-D.
         T1S01a7   Appendix E older entries, the six owed items and the
                   final whole-document consistency pass.

       The a5/a5b split is RATIFIED (DEC-68). The designer must identify the
       FINAL sub-tranche of a document as the last because cross-entry
       consistency is checked there.
       ...
       CURRENT STATUS:
         ...
         a5  CLOSED 2026-08-21 under DEC-85 and DEC-88.
             UPDATED AT v1.7 - the recovery described in v1.5 and v1.6 has
             HAPPENED and those paragraphs were a generation behind.
             Classification Repair v1.1 is complete and settled.
             The W3C recovery reconstruction v1.4 was RULED OUT AS A SOURCE by
             W3X at DEC-70. Successor W3D rebuilt the ledger textually on
             delivered v1.3, so the special recovery-verification gate is
             DISCHARGED and DEC-73 returns a5 to ordinary independent W3C
             review.
             FINAL: ledger v1.10, 44 entries, all Tier C - 16 CURRENT-UNIQUE,
             28 CURRENT-DUPLICATE, zero CONFLICTING/SUPERSEDED/OPERATIVE-SPEC,
             every figure independently reproduced by W3C at its final
             verification. Closed WITH KNOWN RESIDUE, enumerated and routed at
             Standing Task Register section 0c (R1-R8). NO a5 correction
             generation may be opened for any of it.
             NOTHING IN a5 IS RATIFIED INTO ANY AUTHORITY: every PROPOSED
             ACTION remains a proposal awaiting T3.
             The Tier C sample is COMPLETE and must not be reopened - eleven
             selected by W3X, eleven reviewed, 3 AGREE / 8 DISAGREE, DEC-73.
             No new a5 search or classification round is indicated.
         a5b NOT STARTED; gated on a5 closure. It must DERIVE ITS OWN
             POPULATION rather than inherit a5's 46-file search snapshot.
         a6  NOT STARTED.
         a7  NOT STARTED; declared final a-tranche.

T1S02  README, HIGHEST COMMITTED/APPLICABLE VERSION WHEN THIS STEP BEGINS,
       part 1 - decision-status table, sections 3.11 to 6.2 and F12-F17.
       The frozen section-3 survey table still records the generation that was
       present when T1S00 ran; this delivery route is intentionally de-pinned.

T1S03  README, HIGHEST COMMITTED/APPLICABLE VERSION WHEN THIS STEP BEGINS,
       part 2 - Appendices A and B, section 20.

T1S04  The charter, HIGHEST COMMITTED VERSION (read, never stripped; holds
       PR-3), plus the HIGHEST COMMITTED/APPLICABLE D0 Binding Knowledge Index
       and D2 HolyWu Real Schedule at execution time.
       The frozen section-3 table preserves the versions actually surveyed by
       T1S00.

T1S05  All remaining applicable documents assigned by the frozen manifest,
       subject to the later explicit exclusions recorded in section 2.2.
```

T1S01 was moved ahead of the README deliberately. PR-1 gates detector
mathematics downstream. Current sequencing is T1, then T8 provenance-gap
closure, then T5 detector mathematics, then separately-ratified T6/Q14
planning. That sequencing update is delivery context only; it does not alter
the frozen T1S00 search frame.
...
```text
v1.10 (2026-08-21) CURRENT STATUS PARAGRAPH ONLY, frozen frame untouched.
     Records T1S01a5 CLOSED under DEC-85/DEC-88 with final ledger v1.10 and
     the residue routed at register 0c.
v1.9 (2026-08-20) CURRENT STATUS PARAGRAPH ONLY, frozen frame untouched.
     Advances the a5 line to ledger v1.10, the DEC-85 final correction
     generation awaiting W3C's final mechanical verification, after which a5
     closes. Corrects the header metadata date, which v1.8 left at 2026-08-19
     while describing 2026-08-20 state.
v1.8 (2026-08-20) CURRENT STATUS PARAGRAPH ONLY, under DEC-69's
     delivery-describing allowance; frozen frame untouched. Advances the a5
     line to ledger v1.9 (44 entries, all Tier C) awaiting W3C's bounded
     re-review, and records the DEC-85 closure cap.
v1.7 (2026-08-19) CURRENT STATUS PARAGRAPH ONLY, under DEC-69's
     delivery-describing allowance. THE FROZEN FRAME IS UNTOUCHED: the 90
     terms, the surveyed population tables, every hit and line count and
     section 5's weaknesses are unchanged, and so is every other word of v1.6.
     The a5 status paragraph still described the W3C recovery ledger v1.4 and
     said successor W3D must independently verify it - a gate discharged at
     DEC-70/DEC-73. It now records ledger v1.7 awaiting W3C delta review, the
     Tier C sample as complete and not reopenable, and a5b's obligation to
     derive its own population. W3C found the staleness at the recovery
     closure review.
v1.6 (2026-08-19) RECORD CORRECTIONS ONLY, W3X-directed. No content change:
     the frozen 90-term frame, the surveyed population tables, all hit/line
     counts and section 5's weaknesses are untouched, and so is every word of
     v1.5's delivery-describing text. Corrects v1.5's header, which recorded
     "Author: W3D" for a document W3C produced during recovery after the
     designer session had ended; records that the bump was executed BEFORE the
     a5 ledger rewrite when DEC-69 sequenced it after; and restores the
     revision-history heading and opening code fence, absent since v1.4 or
     earlier, which left the document's fences unbalanced.
v1.5 (2026-08-19) W3C RECOVERY RECONSTRUCTION - see the v1.6 correction note;
     the v1.5 header's "W3D" attribution was wrong.
     W3X-authorised DELIVERY-DESCRIBING-ONLY bump under
     DEC-69. THE FROZEN SEARCH FRAME WAS DELIBERATELY NOT REOPENED. Sections
     1 and 3-5 are unchanged from v1.4: the 90 registered terms, the frozen
     surveyed-population tables, all recorded hit/line counts, the previous-
     survey comparison and section 5's recorded weaknesses are preserved.
     Delivery-description changes only: the T1S01a list now names all eight
     declared sub-tranches; current status advances through Classification
     Repair v1.1 and the W3C recovery-ledger v1.4 verification gate; section
     2.2 records DEC-60/63/66 later exclusions without editing the historical
     47-document survey; T1S01b is now SCOPES-ONLY; and future-step generation
     pins are replaced by highest-committed/applicable wording. The resulting
     current populations are described, not retroactively substituted:
     47 documents in the frozen T1S00 survey, 41 in current adjudication, and
     46 files in the settled a5 search population.

v1.4 (2026-08-18) Records the two declared splits of step T1S01 in section 6 -
     into T1S01a (the ratified authority) and T1S01b (the working record
     behind it), and then T1S01a into sub-tranches - together with the rule
     that cross-entry consistency is checked at the final sub-tranche of a
     document. Both splits use the split-and-report provision this manifest
     already reserved, so this records an exercised reservation rather than a
     new licence. The frozen 90-term frame, the 47-document population and
     every exclusion are UNCHANGED.
