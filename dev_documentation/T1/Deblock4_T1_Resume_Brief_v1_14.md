...
1. THE BOUNDED ROOT-CONTINUITY REFRESH. Deliberately deferred through the
   whole recovery so the document set could be corrected ONCE against a
   settled endpoint. That endpoint now exists. In scope: Project Status,
   Forward Roadmap, Documentation Currency Audit, the Concise Project Summary,
   and the four orientation files. Also residue item R7 - stale current-status
   pointers, the class that recurs. BOUNDED means current-state only: do not
   reopen adjudication, do not restate the ledger.
2. T1S01a5b - authority sections 9-13, lines 716-1098. Blocked until 1 is
   done. See 0f for what W3C expects; it is HARDER than a5. Under Review Scope
   v1.15, which is the RATIFIED consolidated method (DEC-83/84).
3. W3X's PROJECT-VIABILITY DECISION POINT, which W3X placed explicitly AFTER
