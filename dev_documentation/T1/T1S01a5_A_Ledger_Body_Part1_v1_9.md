...
**Version:** 1.9 - post-repair rewrite. Built TEXTUALLY on delivered ledger v1.3 with only the deltas mandated by Classification Repair v1.1, W3C review response v1.0, and ratified register decisions, plus W3C's Tier C sample review and all three
...
**Binding review scope:** `Deblock4_T1_W3C_Review_Scope_v1_14.md`
**Binding work queue:** `Deblock4_Standing_Task_Register_T_Series_v1_34.md`
**Supersedes:** ledger v1.8 (same authorship and basis; v1.9 applies W3C's
v1_2 re-review, items A-P). Textual basis remains delivered ledger v1.3.
...
D7  BINDING PINS advanced. Work queue v1.23 -> v1.33; review scope v1.11 ->
        v1.13. Currency only. CORRECTED AT v1.8 - v1.7's header still carried
        the v1.11 scope and v1.30 queue while its body cited later
        generations.
...
is v1.3's 34 entries; the six at D1 are excluded; these are the remainder.
  ...
  LED-033  LED-034  LED-035  LED-036  LED-037  the stated population, the fifteen classified hit lines, the
           six-document collision enumeration with file and line numbers, and
           the KEEP-the-name recommendation for Classic-facing documents -
           THE EVIDENCE BASE, all restored verbatim.
           NOT RESTORED, AND DELIBERATELY (v1.9 reconciliation): the old
           REASON sentence "it is an audit record about this document, so no
           other document can hold it". Live LED-037 withdrew it at v1.6 as
           an invalid uniqueness proof - W3C's Tier C finding - and rests
           CURRENT-UNIQUE on the searched basis instead. A restoration list
           must not re-assert reasoning the live entry has withdrawn.
...
EXTENDED AT v1.9 WITH COUNT-CONSISTENCY: the v1.8 gate checked only terms the
...
DISPOSITION  CURRENT-UNIQUE
REASON       Both propositions are current. THE CLAIM IS NARROWED AT v1.8 and
             the narrowing is the point: this is the only record MAPPING THE
...
SWEPT        THE v1.8 LEXICAL PROBE IS SUPERSEDED AND RECORDED AS THE DEFECT
             ...
             settled 46-file population, returning FOUR files: this authority
             (CANONICAL), the Scopes PreScope brief (CARRIER, D4-D01), the
             Architecture ReDecision Brief (CARRIER), and the Architecture
             ReDecision W3C Evaluation (opened; decision-record context).
...
ACTION   RETAIN THE COMPOSITION IN PLACE at 6.3 as canonical; POINTER for the
         ...
         (v1.9: the relocated-history marker that still sat in this live
         field at v1.8 is removed; the v1.6 correction history lives in
         REASON and the revision history. A live ACTION states the current
         proposal and nothing else. Former text began: W3D specifically
...
REASON       BOTH NAMED LOCATIONS RESOLVE, WHICH IS WHY THE PROPOSITION IS
             ...
             file of that name exists anywhere in the live tree. But a STRONG
             LIKELY REFERENT does exist under a different filename: the
             W3C-D4-VERIFY-1 coder-response document in Scopes/, titled
             "Deblock4 - D4 Pre-Scope - W3C Verification and Independent
             Design Review". W3C identified it at the closure review.
             CORRECTED AT v1.7 AND RECORDED PER DEC-51. v1.5 and v1.6 said the
             report "MAY NOT" exist. That OVERSTATED the finding by conflating
...
SWEPT        POPULATION AND BASIS, per DEC-50. FIVE CHECKS, not four - the
             count is corrected at v1.8 along with the addition of (5).
...
DERIVED      TWO SECTIONS DEPEND ON THIS DOCUMENT AND ONE OF THEM IS THE
             ...
             mattered.
DERIVED-BASIS  Section 2.1's F4 and F5 records; the FIVE checks above (v1.9:
             corrected from "four searches" after check 5 was added at v1.7);
...
Q-C  STAY-CANONICAL, AND THE COUNT IS NOW 19 - RECOMPUTED MECHANICALLY
     AT v1.9 by searching the finished ledger's DUPLICATE-ACTION first lines,
     not by editing the previous number:
     LED-036  LED-041  LED-042  LED-043  LED-045  LED-046
     LED-047a  LED-048  LED-049  LED-051  LED-053c  LED-053d
     LED-055  LED-055a  LED-055b  LED-057  LED-058  LED-059  LED-061
     Each names at least one concrete non-canonical copy per DEC-43, and each
     copy names the RIGHT CLAUSE - the copy-of-the-wrong-clause failure
     (Review Scope 0.9) having now been committed and corrected twice, at
     LED-043 and LED-032a. Is any of these claims resting on a copy of a
...
Q-H  THE SPLIT SET - CURRENTISED AT v1.9. The suffix entries now stand at
     nine: LED-043a, LED-047a, LED-053c, LED-053d, LED-055a, LED-055b,
     LED-058a, LED-061a from the a5 rounds, plus the earlier LED-051a/052a/
     053a line from v1.3. (The v1.5-era framing - "five suffix entries, and
     LED-055 takes NO suffix" - is withdrawn as stale; LED-055 has since
     split twice, and LED-047 once, each under a W3C finding.) The question
...
*End of T1S01a5 ledger v1.8. Nothing here is ratified. Every PROPOSED ACTION
