...
**Author:** W3D
**Route:** W3D -> W3X -> W3C
**Accompanies:** `T1S01a4_A_Ledger_Section23_Tail_v1_4.md` - FRAMING ONLY
...
Deblock4_T1_W3C_Review_Scope_v1_11.md              UNCHANGED - you used
                                                   v1.11 last time too
Deblock4_Standing_Task_Register_T_Series_v1_19.md  was v1.16 at your last
                                                   review; v1.17 was
...
```text
1. W3X PROVISIONALLY ADOPTS LED-029's TWO-ROLE / TWO-ACCEPTANCE-STATE
   REPAIR, SUBJECT TO YOUR REVIEW - register DEC-45. It is NOT ratified.
   (CORRECTED at v1.4: this paragraph still called it the TWO-ARTIFACT
   repair. You found it.)
...
## The four framing corrections, for a final mechanical check

```text
1. LEDGER closing question 1 - now describes the v1.4 population: every
   entry in every prior T1 ledger, grouped by ledger. It had still said
   "every T1S01a2 entry".
2. LEDGER closing question 2 - now describes LED-025's narrowed evidence.
   It had still said "every sequencing decision in the register".
3. THIS NOTE's provisional-adoption paragraph - now says TWO-ROLE /
   TWO-ACCEPTANCE-STATE repair.
4. REGISTER DEC-45's STATUS paragraph - now rests on roles and acceptance
   states, and says explicitly that object identity is NOT established.
   Your recommended wording, essentially verbatim.

NO ENTRY FIELD CHANGED IN THE LEDGER. Only its closing section moved.
```

## What those four had in common, and the rule it produced

All four are one defect: a correction landed in its target field while the
surrounding live framing kept the superseded proposition. That is now THREE
consecutive rounds - the a3 closing section arguing the pre-reissue
position, the two-artifact remnants, and these. Every time the correction
was right and was reported as complete. Every time you found the remnant.

REGISTER DEC-51 is the countermeasure: after replacing a rejected
formulation, search the whole document set and ENUMERATE every occurrence,
classified live or historical, rather than reporting the replacement done.
The ledger's closing section now carries that enumeration for the
two-artifact wording, so you can count it against a search.

```text
I7: DEC-51 judges W3D's own deliverables and W3D proposed it. YOUR
VERIFICATION IS OWED AND IS ASKED FOR HERE. If enumeration is the wrong
countermeasure - or if it will become a ritual the way the first version of
the check-evidence rule would have - say so.
```
...
*Revision history*
```text
v1.4 (2026-08-18) Accompanies ledger v1.4, the framing-only reissue. Applies
     W3C's four framing corrections, records the partial-replacement failure
     mode and its enumeration countermeasure at DEC-51, and asks W3C for the
     I7 verification that rule is owed.
