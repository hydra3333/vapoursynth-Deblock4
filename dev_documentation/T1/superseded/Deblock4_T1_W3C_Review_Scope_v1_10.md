...
**THE REQUIREMENT THAT MAKES IT FALSIFIABLE, added by W3D as verifier.**
An entry claiming STAY-CANONICAL must NAME WHERE THE NON-CANONICAL COPIES
ARE. Every other action carries a test - RETAIN-SUMMARY must clear three
conditions, POINTER follows from failing them - and without this,
STAY-CANONICAL would be the only action established by assertion, and
therefore the easiest to reach for. If an entry cannot say where the other
copies live, it has not shown the statement is a DUPLICATE at all, and the
disposition itself is in question.

```text
CHARTER I7 PROVENANCE, and it runs the opposite way to 5.4:
    proposer:  W3C (T1S01a3_B v1.1, Q1)
    verifier:  W3D, which checked the gap against this scope's own text
               and added the naming requirement
