...
```text
  T1        Formal consolidation sweep ......... IN PROGRESS.
                                                 ...
                                                 any entry.
                                                 T1S01a4 IS NOW THE NEXT
                                                 WORK: section 23 steps 6-10,
                                                 carrying the DEC-32 ordering
                                                 finding, which must be
                                                 DERIVED, not guessed.
...
DEC-51  PARTIAL REPLACEMENT IS ITS OWN FAILURE MODE, AND THE COUNTERMEASURE
        IS ENUMERATION RATHER THAN INTENT.
        THE PATTERN, now three consecutive occurrences, each caught by W3C
        and none by its author:
          1. T1S01a3 v1.1 rewrote its entries and left the closing section
             arguing the pre-reissue position, asking W3C to reopen two
             ratified decisions;
          2. T1S01a4 v1.2 added the accepted two-ROLE refinement and left
             the rejected two-ARTIFACT premise live immediately above it,
             and in DEC-45, and in the covering note;
          3. T1S01a4 v1.3 corrected section 0's population and LED-025's
             sweep, and left the closing section asking W3C to verify BOTH
             of the formulations the same document had just withdrawn.
        ...
        the surrounding framing was written under the old belief and reads
        as familiar rather than as wrong. In all three cases the correction
        itself was right and was reported as complete.
        WHY IT MATTERS MORE THAN A TYPO: the stale framing is usually the
        part that TELLS THE READER WHAT TO THINK - a closing section, a
        status paragraph, a covering note. A reviewer answering it in good
        faith produces findings that contradict the correction, and someone
        then has to work out which of the two the document meant.
        THE RULE: after replacing a rejected formulation, SEARCH THE WHOLE
        DOCUMENT SET for it and ENUMERATE every occurrence found,
        classified LIVE or HISTORICAL, in the deliverable itself. Do not
        report the replacement as complete. A historical reference is
        legitimate where it plainly states what an earlier version said; the
        defect is only where the rejected wording still states the current
        question, rule or status.
        THIS IS DEC-50's FORM APPLIED TO A DIFFERENT CHECK: identify the
        population, record the result, make the claim countable. DEC-50
        covers checks used as evidence; this covers replacements reported as
        complete. Both replace an assurance with something falsifiable.
        (W3C found all three occurrences; W3D derived the rule; W3X
        2026-08-18. NOTE FOR I7: the rule judges W3D's own deliverables and
        W3D proposed it - W3C's verification is owed and is asked for in the
        T1S01a4 closing package.)
