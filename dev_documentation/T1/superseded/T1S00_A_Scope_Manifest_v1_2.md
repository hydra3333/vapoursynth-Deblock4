...
```text
EXCLUDED AS AUTHORITY - four retired folders, 352 files:
    superseded/
    Scopes/superseded/
...
RETIRED ON 2026-08-18, recorded so the moves are auditable:
```text
  -> superseded/          Task Register v1_1, v1_2, v1_3
...
## 2.0b EXPLICIT EXCLUSION - the outgoing-designer evidence zip
...
Knowledge Index v1.14 and the Task Register v1.1. Each mention is a ledger
...
## 2.0c ORPHAN-FAMILY CHECK on the retired folders - run, and its result
...
The C++ sources in that folder (`deblock.cpp`, `deblock_sse4.cpp`,
`deblock.h`) are third-party reference material, not project documents. They
are NOT swept as project knowledge; the provenance document that describes
them is.
...
W1  A LIVE DOCUMENT WITH NO MPEG-2 VOCABULARY AT ALL.
    ...
    or schedules while using none of the 42 terms. Zero-hit documents remain
...
W5  THE TERM SET IS MINE AND HAS NOT BEEN INDEPENDENTLY DERIVED.
    W3C is asked, under PR-4, to say what these 42 terms would still miss.
    That review is the intended defence against W1 and should not be treated
    as a formality.
```
...
3. Corrections are folded in as manifest v1.1 if either review finds
   anything.
