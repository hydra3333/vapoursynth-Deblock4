...
**Version:** 1.2 - NARROW REISSUE after W3C's review. Two entries WITHDRAWN
**Date:** 2026-08-18
**Author:** W3D
**Route:** W3D -> W3X -> W3C
...
EVERY T1S01a2 ENTRY, EXAMINED AND ITS RANGE COMPARED WITH 1694-1700:
  LED-001, LED-002   header/section 0 currency statements - no overlap
  LED-003            section 23 lines 1679-1681        - outside this range
  LED-004            section 23 steps 1-5, 1683-1694   - outside; note 1694
                     is the boundary line and belongs to LED-004's head
                     range as recorded there, while step 6 begins at 1694
                     in the current file. LED-025 quotes step 6 only.
  ...
  LED-010            section 23 lines 1703-1710        - OVERLAP. This is
                     the one v1.1 missed. Excluded above.
  LED-011            header block lines 25-31          - outside
  LED-012            Appendix E, v1.05 entry           - outside

EVERY T1S01a3 ENTRY: LED-013 to LED-024 and LED-020a adjudicate section 0's
  seventeen architecture items and the header block. None touches section 23.
  Checked by reading each entry's DOCUMENT field.
```
...
SWEPT        TWO SEPARATE CLAIMS ARE SWEPT HERE, because v1.1 swept only one
             ...
             Checked the Standing Task Register's decision log for every
             decision touching sequence: DEC-02 reverses T1 against T5 and
             T6, and DEC-03 withdraws the coordinated T5+T6 packaging
             permission. BOTH CONCERN THE DOCUMENTATION AND DETECTOR TRACK
             ONLY. Neither mentions D4-Q16, the parameter surface, or any
             post-Q14 development step. Checked also that the register
             carries no decision placing D4-Q16 relative to T1/T5 at all -
             it does not. The step 6 relationship is therefore untouched by
             the reversal, and is untouched because the reversal never
             reached that part of the sequence.
```
...
DERIVED      "FREEZE" AT STEP 7 CANNOT MEAN WHAT "FREEZE" MEANS AT STEP 9, and
             ...
             comparison rerun. Without this, the two-senses reading would
...
DERIVED      THE DEFECT IS NOT THE ORDER OF THE TWO STEPS. IT IS THAT STEP 8
             ...
             REFINED AT v1.2, AND THE REFINEMENT MATTERS: what is required is
             TWO ROLES AND TWO ACCEPTANCE STATES, not two permanent
             separately implemented scalar codebases. W3C supplied this and
             it corrects a real overstatement in v1.1's wording. The winning
             candidate does NOT become the oracle by winning - a later scope
             must still construct and accept the oracle under its own rule -
             but nothing in the ratified text requires the winning
             candidate's implementation to be thrown away and rewritten.
             Whether 8b reuses, refactors or hardens that code is a later
             scoping question, provided 8b's acceptance basis is
             independently authored and actually satisfied.
...
You asked for a delta review rather than a restart, and that is what this is
aimed at.
