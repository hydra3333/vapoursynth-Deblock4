...
**Version:** 1.3 - three split-disposition entries split per DEC-33(a); the STAY-CANONICAL first-use claim withdrawn as false. No finding changed.
**Date:** 2026-08-19
**Author:** W3D
**Route:** W3D -> W3X -> W3C
...
```text
DEFINED ONCE HERE, AND INHERITED BY ALL 39 SWEPT FIELDS IN THIS LEDGER, so
that the population cannot drift between one entry and the next.

THE RULE, as W3X ruled it on 2026-08-19 and as recorded at register DEC-60:

    A file is EXCLUDED if ANY folder in its path has a name beginning
    "superseded" or "scheduled_for_deletion". Everything else in the supplied
    dev_documentation tree is IN the search population.

APPLIED TO THE SUPPLIED TREE that produced this ledger:
    519  files in the tree
    432  excluded by the rule, in five folders:
             superseded/
             Scopes/superseded/
             T1/superseded/
             reference/superseded_do_not_use_files_in_this_folder/
             reviews/scheduled_for_deletion/
     87  THE SEARCH POPULATION. Every SWEPT field below ran over these.

87 IS NOT 47, AND THE TWO NUMBERS MUST NOT BE RECONCILED.
    ...
    87  the SEARCH population - deliberately wider, because a duplication or
        ...
        artifact is still a copy for the purpose of refuting a uniqueness
        claim; it simply does not get adjudicated.
Where an entry counts rather than searches, it says which population it is
counting. LED-035 is the clearest example.

WHY THIS SECTION EXISTS, recorded because the history is the point. v1.0 and
v1.1 of this ledger had no single definition: LED-033's SWEPT field
enumerated FOUR excluded folders and omitted T1/superseded/, while the
searches themselves excluded any path containing "/superseded/" - two
different implementations of one rule, never checked against each other. The
accompanying corpus manifest used the four-folder enumeration and therefore
listed 124 files as searched when 87 were. W3C FOUND IT on receiving the
refreshed corpus, before completing the entry review.
THE SEARCHES WERE NEVER WRONG. Every SWEPT result in this ledger was produced
over the 87. What was wrong was the description of them, in the artifact
supplied so the searches could be reproduced - which is the worse half to get
wrong, because it is the half a reviewer checks against.
THE UNDERLYING FAULT IS THE ONE CHARTER G3 NAMES: a target and its detection
derived from two mechanisms instead of one, with nothing to make the drift
loud. The fix is this section - one definition, stated once, inherited
everywhere - rather than thirty-nine corrected restatements.
```
...
SWEPT        Searched the live corpus as defined at section 0.5 - 87 files,
             every retired tree excluded by the DEC-60 rule - for
...
REASON       (a)-(c) are the mathematics itself and exist nowhere else in the
             ...
             coordinates and are consumers rather than copies.
CONFLICTS    none.
PREVAILS     n/a - unique. Canonical home is this section for all three
             propositions.
...
```text
NARROWED AT v1.3 under the atomic-claim rule (DEC-33(a)): the chroma-plane
coordinate declaration at line 517, formerly proposition (a) with its own
disposition in a compound DISPOSITION field, is now LED-053a. The remaining
propositions KEEP THEIR ORIGINAL LETTERS (b), (c), (d) - relettering would
break alignment with any note W3C has already made against this entry.

DOCUMENT     authority v1.05, section 4.5, lines 519-529
CLAIM        THREE propositions. (b) For progressive and Case-(a) frame pictures:
...
ASSERTS      the 4:2:0 frame-picture chroma edge coordinates, the absence of
             phase ambiguity, and the Case (b) woven-frame pitch.
CLASS        DERIVED from F1, F2 and F4, W3X-RATIFIED
DISPOSITION  CURRENT-UNIQUE
REASON       (b), (c) and (d) are the chroma geometry
...
Q-C  STAY-CANONICAL. NOT ITS FIRST USE IN T1 - v1.0 to v1.2 of this ledger
     said it was, and W3C established that T1S01a3 ledger v1.4 already
     claimed it at LED-020 and LED-021. The framing is withdrawn at LED-036;
     the question below is unchanged and still needs answering.
     THIRTEEN entries claim it in their
