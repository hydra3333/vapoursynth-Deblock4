...
# 0a. STATE ADVANCE (v1.13) - WHAT IS OWED RIGHT NOW

## Read this first if you are a successor

```text
THE POSITION IN ONE PARAGRAPH. Ledger v1.9 (44 entries, ALL Tier C) is
delivered and awaiting W3C's bounded re-review, whose agenda is section 20 of
W3C's v1_2 response. Review Scope v1.14 is the RATIFIED consolidated method -
DEC-83 ratified 0.5/0.6/0.7, DEC-84 ratified the exact SUPERSEDED propagation
wording at 0.11. W3X HAS CAPPED THE ROUNDS AT DEC-85: if the re-review is
clean, a5 CLOSES; if not, exactly ONE more bounded generation applies the
findings and a5 CLOSES ANYWAY, with residue recorded and carried to a7's
whole-document consistency pass. DO NOT START ANOTHER CORRECTION CYCLE BEYOND
THAT UNDER ANY REASONING - the cap is a W3X ruling, made deliberately after
the designer reported its own error rate honestly.

THE WORK OWED, IN ORDER:
1. Batch v5 to W3C (assembled; W3X sends).                       [WITH W3X]
2. W3C bounded re-review of ledger v1.9.                         [WITH W3C]
3a. CLEAN -> W3X closes a5. Skip to 5.
3b. NOT CLEAN -> ONE generation: apply findings, run ALL the mechanical
    gates at 0e, reissue, W3C verifies the deltas only, W3X closes a5 with
    residue recorded for a7. NO FURTHER ROUND (DEC-85).
4. (there is no 4 - the cap is the point)
5. AFTER CLOSURE: one bounded root-continuity refresh pass - Project Status,
   Roadmap, Currency Audit, orientation files - deferred deliberately per
   W3C so the whole set points at one settled endpoint. Then a5b begins:
   DERIVE ITS OWN POPULATION, produce the source-coverage map BEFORE
   adjudication (scope 0.10 / DEC-77), expect more Tier A and the hardest
   material (0f below). Then W3X's project-viability decision point, which
   W3X has said comes AFTER seeing how a5b goes.

WHAT IS SETTLED AND MUST NOT BE REOPENED: the Tier C sample (DEC-73), the
22-probe round, Classification Repair v1.1, the a5/a5b split (DEC-68),
DEC-67 methodology, the architecture. ALSO NOW SETTLED: Q-J, Q-K, Q-F, Q-G -
all answered and closed on the ledger record.
```

## What v1.9 fixed and what it cost - read before touching the ledger

```text
W3C's v1_2 response items A-P are applied in full. The two substantive
findings were both against the designer's own method, and a successor MUST
internalise them before writing any probe or split:

1. LEXICAL IS NOT SEMANTIC (DEC-67 Rule 3). Two designer probes searched
   near-verbatim wordings and missed real carriers that W3C's semantic
   families found in the SAME population. If your probe family is grammatical
   permutations of the source sentence, IT IS NOT A PROBE. Write the family
   from the PROPOSITION's concepts, then add the source wording.
2. THE ATOMIC-CLAIM RULE HAS NOW BEEN VIOLATED FOUR TIMES (LED-053, LED-055,
   LED-055a, LED-047), twice AFTER it was consolidated into the binding
   scope. If an entry's own CLAIM field says "two propositions", IT IS TWO
   ENTRIES. No conjunction-uniqueness arguments - "no document states BOTH"
   proves nothing about either.

THE MECHANICAL GATES ARE NOT OPTIONAL AND ARE SPECIFIED BY WHAT CAN BE WRONG,
not by what you remember changing (ledger 0.4h, DEC-76/81):
   - entry-sweep diff: every edited entry searched for the terms its edit
     removed, discounting explicit withdrawal records;
   - tier derivation: TIER recomputed from DISPOSITION for every entry;
   - count-consistency: EVERY numeric entry/tier claim in the document tested
     against the mechanical enumeration ("45 Tier C" survived the v1.8 gate
     because nobody knew to search for it);
   - Q-C STAY-CANONICAL count regenerated from DUPLICATE-ACTION first lines;
   - US-ASCII, CRLF, balanced fences, on every file individually.
```
