...
**Version:** 1.7 - post-repair rewrite. Built TEXTUALLY on delivered ledger v1.3 with only the deltas mandated by Classification Repair v1.1, W3C review response v1.0, and ratified register decisions, plus W3C's Tier C sample review and recovery
closure review. 42 entries.
**Date:** 2026-08-19
...
**Supersedes:** ledger v1.6 (same authorship and basis; v1.7 applies W3C's
recovery-closure review). Textual basis remains delivered ledger v1.3.
...
IT IS PARTLY MECHANICAL, AND WAS APPLIED THAT WAY AT v1.7: after each edit,
...
DOCUMENT     authority v1.05, section 1, line 225
...
CONFLICTS    none.
PREVAILS     CANONICAL HOME: the CHARTER for the project identity, which is
             where project scope is ratified and is T1S04's range. This
             authority is the canonical home for nothing in this sentence -
             it is orientation for the MPEG-2 document, not the definition of
             the project.
...
SWEPT        NO NEW SEARCH ROUND WAS RUN. The basis is LED-038's rebuilt probe
             table over the settled 46-file population at section 0.5, which
             opened and classified fifteen candidate files for the coordinate
             conventions. NONE of the four carriers - charter B1, this
             authority, the README kernel sections, the Scopes evaluation -
             carries a default-scope qualification with them.
             CLAIMED SCOPE, PER DEC-50: uniqueness within the 46-file
             population under LED-038's declared probe family, which searched
             the CONVENTIONS. A document that scoped its own conventions in
             wholly different vocabulary would not have been returned by that
             family, and this entry does not claim otherwise.
...
DOCUMENT     authority v1.05, section 5, lines 585-587
CLAIM        "The same problem exists for progressive/frame-DCT geometry.
             ...
             contract. Whole-frame input is a codec-geometry correctness
             requirement, not a performance preference."
ASSERTS      two qualifiers on the derivation: (i) its GENERALITY - the tearing
             analysis holds for progressive and frame-DCT material, not only
             for the interlaced case that motivated it; (ii) the CHARACTER of
             the resulting constraint - it binds on correctness grounds and is
             not a performance choice.
CLASS        DERIVED from F4 and the section 5 mechanism, W3X-RATIFIED
DISPOSITION  CURRENT-UNIQUE
REASON       The MECHANISM is duplicated - README Appendix A.9.3 states it, and
             so does the Scopes re-decision brief. THE QUALIFIERS ARE NOT.
             W3C's Tier C review searched the applicable corpus and did not
             find another applicable document stating, in the same complete
             form, both the progressive/frame-DCT generality and the
             correctness-requirement characterisation. THAT ASYMMETRY IS
             PRECISELY WHY LED-055 COULD NOT CARRY ONE DISPOSITION.
             WHY THESE QUALIFIERS MATTER MORE THAN THEY LOOK: without (i) a
             reader takes the constraint as an interlaced-only artefact and
             assumes progressive sources are exempt. Without (ii) the
             whole-frame contract reads as an optimisation a future scheduler
             could trade away. Either misreading reintroduces the rejected
             architecture by the back door.
CONFLICTS    none.
PREVAILS     n/a - unique. Canonical home is this section, where the qualifiers
             sit against the mechanism they qualify.
SWEPT        NO NEW SEARCH WAS RUN FOR THIS ENTRY, and that is stated plainly
             rather than left to inference.
             THE EVIDENCE IS W3C's TIER C SAMPLE REVIEW OF LED-055, which
             searched the applicable corpus under the SeparateFields probe
             family recorded at LED-055's SWEPT field and reported: the README
             carries the row-projection mechanism; other pieces of the
             authority's derivation are more specific; and no other applicable
             document was found stating the generality and
             correctness-requirement qualifications in the same complete form.
             CLAIMED SCOPE, PER DEC-50: uniqueness within the applicable corpus
             AS W3C SEARCHED IT during the Tier C round, under that probe
             family. IT DOES NOT CLAIM a fresh independent W3D probe was run at
             v1.7. A reviewer should treat this entry's uniqueness as resting
             on W3C's finding, and attack that finding rather than a W3D search
             that does not exist.
...
ACTION   RETAIN THE COMPOSITION IN PLACE at 6.3 as canonical; POINTER for the
         ...
         SUPERSEDED v1.6 TEXT, RECORDED RATHER THAN DELETED: W3D specifically
...
REASON       THE RAW GAIS EVIDENCE FILES EXIST AND ARE LOCATABLE - six
             ...
             THE FILENAME IS WRONG; THE CONTENT APPEARS TO SURVIVE. Section
             ...
             THE RAW GAIS EVIDENCE FILES remain locatable, but note their
             status has changed: DEC-66 removes GAIS_investigations/ from T1
             search and adjudication alike. They are history on disk, not
             population members. v1.5 and v1.6 described them as "live and in
             T1's population at T1S01b and T1S05" - THAT IS STALE and is
             corrected here.
...
SWEPT        POPULATION AND BASIS, per DEC-50. (1) Filename search of the
             ...
             (5) ADDED AT v1.7 FROM W3C's CLOSURE REVIEW. A STRONGER CANDIDATE
             EXISTS: the W3C-D4-VERIFY-1 coder-response document in Scopes/,
             titled "Deblock4 - D4 Pre-Scope - W3C Verification and Independent
             Design Review". It is a report FROM W3C rather than a brief TO
             W3C, and its title is close to R8's filename. THIS ENTRY RECORDS
             IT AS A STRONG LIKELY REFERENT AND DOES NOT RULE IT IN. Confirming
             it means checking that its content matches what sections 2.1 and 8
             attribute to "V4.1" and "V4.3", and section 24 is T1S01a6's range
             under DEC-55, so the confirmation belongs there.
             WHY THIS IS NOT W3D DECIDING a6's QUESTION: the entry's
             DISPOSITION is unchanged and its PREVAILS is still UNDECIDED. What
             changes is the accuracy of the CONFLICT DESCRIPTION - it is a
             naming defect with a probable surviving referent, not a possibly
             missing record. Establishing
...
*End of T1S01a5 ledger v1.7. Nothing here is ratified. Every PROPOSED ACTION
