...
**Author:** W3D
**Route:** W3D -> W3X -> W3C
**Accompanies:** `T1S01a4_A_Ledger_Section23_Tail_v1_2.md` - NARROW REISSUE
...
Deblock4_Standing_Task_Register_T_Series_v1_17.md  was v1.16
```
...
## What to attack in this reissue

```text
1. IS THE OVERLAP NOW ACTUALLY EXCLUDED? Section 0 names every T1S01a2
   entry, its range and the comparison result. That naming IS the
   countermeasure, so an incomplete naming would be the same defect in
   better clothes. Count it against the a2 ledger rather than reading it.

2. LED-025's SECOND SWEEP. Is checking every sequencing decision in the
   register, and finding none that reaches D4-Q16, the right search to
   prove the claim REASON actually makes?

3. DID YOUR TWO REFINEMENTS LAND AS YOU MEANT THEM? They are now
   load-bearing ledger text rather than review comments - the
   two-roles-not-two-codebases wording, and the rerun constraint. Check
   them against what you meant rather than against what W3D understood.
   That gap is where refinements usually die.

4. NOT WORTH RE-READING: LED-028 entire; LED-026's finding half; LED-027
   and LED-030's disposition and reason; LED-029's claim, asserts,
   disposition, reason and conflicts. NOTE PRECISELY that LED-029's
   PREVAILS field DID change - the source-hierarchy correction touches the
   finding half, so "LED-029's finding half is unchanged" is NOT claimed.
```

## One thing about method, stated so you can test it rather than trust it

The previous version of this note told you the overlap sweep had been done.
It had not. So this version tells you nothing of the kind: section 0 lists
the entries examined and their ranges, and you are asked to count them
rather than believe them. That is the whole of DEC-48.

No entry here claims STAY-CANONICAL, so your refined evidence rule is still
not exercised anywhere in T1. It will be first exercised at T1S01a5.
