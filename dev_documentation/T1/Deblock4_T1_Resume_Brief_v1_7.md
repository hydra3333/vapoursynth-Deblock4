...
# 0a. STATE ADVANCE (v1.7) - WHAT IS OWED RIGHT NOW
...
CORRECTED AT v1.7 - THIS BRIEF SAID SOMETHING FALSE AND IT PROPAGATED.
v1.3 to v1.6 said "no entry anywhere in T1 has yet claimed STAY-CANONICAL ...
a5 is where that first happens." IT WAS NOT TRUE WHEN WRITTEN. T1S01a3
ledger v1.4 already claimed STAY-CANONICAL at LED-020 (line 535) and LED-021
(line 654), both corrected to it from RETAIN-SUMMARY at a3 ledger v1.3 under
DEC-38 - which THIS BRIEF's own owed-items list refers to.
THE COST: the a5 designer read this line, repeated it in LED-036, and cited
THIS BRIEF as the basis - so an unchecked statement acquired a source. W3C
caught it. See register DEC-41 instance eleven.
WHAT IS ACTUALLY TRUE, and is the useful thing to inherit: the DEC-43
EVIDENCE REQUIREMENT - name at least one concrete non-canonical copy, do not
imply exhaustiveness - post-dates the a3 uses, so a5 is the first sub-tranche
whose STAY-CANONICAL claims were written under the refined rule. That is a
narrower and checkable statement.
```
