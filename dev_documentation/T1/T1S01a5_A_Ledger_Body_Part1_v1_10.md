...
**Version:** 1.10 - post-repair rewrite. DEC-85 FINAL CORRECTION GENERATION. Built TEXTUALLY on delivered ledger v1.3 with only the deltas mandated by Classification Repair v1.1, W3C review response v1.0, and ratified register decisions, plus W3C's Tier C sample review and all three
recovery-closure reviews. 44 entries, all Tier C.
**Date:** 2026-08-19
...
**Supersedes:** ledger v1.9 (same authorship and basis; v1.10 applies W3C's
v1_3 re-review, items A-P - the DEC-85 final correction generation). Textual
basis remains delivered ledger v1.3.
...
## 0.1 THE SUB-TRANCHE IS SPLIT, AND THE SPLIT IS RATIFIED - DEC-68

```text
THE SPLIT IS RATIFIED AT DEC-68. It is not a live W3D declaration awaiting
anything. (v1.9: earlier generations presented this section as W3D declaring
a split under the manifest's split-and-report provision; that was true when
written and is HISTORY now. The paragraph below is retained as the historical
derivation, labelled as such.)

HISTORICAL DERIVATION. DEC-53 defined T1S01a5 as authority sections 1-13, and
DEC-56 corrected the range to EXACTLY lines 223-1098. That is 876 lines and,
on the entry density the ledger format actually requires, roughly fifty
adjudicable propositions. W3D declared a split under the manifest's
split-and-report provision - the same provision under which T1S01 was split
into a and b, and under which DEC-53 itself restructured a5/a6/a7:
...
FINDING B - THE W3C VERIFICATION REPORT CONTENT SURVIVES; SECTION 24's R8
FILENAME IS WRONG. (v1.9: this finding originally read "points at a document
that may not exist", and that is no longer the state of the evidence.) W3C's
Tier-A review opened the W3C-D4-VERIFY-1 coder response and matched its
V4.1/V4.3 VERIFIED content to what sections 2.1 and 8 cite. The surviving
defect is a stale filename in R8 -
`Deblock4_D4_W3C_Verification_and_Design_Review_v1_0.md` names no existing
file - and its correction belongs to T1S01a6, which owns section 24. Recorded
at LED-063 with a CITED-OUTSIDE-RANGE record; DEC-58 is discharged for
content and open only for the reference.
```
...
## 0.4c THE ORIGINAL CLASSIFICATION-REPAIR DELTA SET (D1-D7), WITH THE
## AUTHORITY FOR EACH
...
D7  BINDING PINS advanced - HISTORICAL RECORD OF THE v1.5-ERA ADVANCE ONLY
        (queue v1.23 -> v1.30, then corrected onward at v1.8). THE CURRENT
        PINS LIVE IN THE DOCUMENT HEADER AND NOWHERE ELSE; this list is the
        original repair delta and is not advanced when the header pins move.

SCOPE OF THIS LIST, RESTATED HONESTLY AT v1.9: D1-D7 ARE THE ORIGINAL
CLASSIFICATION-REPAIR DELTA SET - what the v1.5 rebuild changed against v1.3
under the repair mandate. THEY ARE NOT THE COMPLETE v1.3-to-current delta.
Later generations added coverage entries, Tier C repairs, bounded probes,
occurrence records and recovery corrections, each recorded in its entry and in
the revision history, not here. (The sentence this paragraph replaces -
"NOTHING ELSE CHANGED. Every other word of every other field is v1.3's." -
was true of v1.5 and FALSE of every generation since; a true-when-written
absolute left standing becomes a false assurance, which is DEC-48's exact
shape.)
```
...
```text
THIS IS THE D1 / v1.5 SNAPSHOT: the 28 of v1.3's 34 entries whose disposition
structure survived the classification repair unchanged. It is a HISTORICAL
record and is not advanced by later splits.
(v1.10: at v1.9 a misapplied edit replaced LED-038 and LED-039 in this list
with restoration prose meant for 0.4f, so the promised 28-entry enumeration
printed 26. W3C counted it. ROOT CAUSE, recorded because it is a new failure
shape: the correcting edit WAS made during the v1.9 build, verified, and NEVER
WRITTEN TO DISK - the next build step reopened the unrestored file. A fix that
is not persisted is not a fix; the gates now run on the file as re-read from
disk, never on the in-memory text that claimed to contain the repair.)
...
## 0.4e THE ENTRY COUNT - CURRENT DERIVATION, THEN HISTORY

```text
CURRENT DERIVATION - THE ONLY LIVE ARITHMETIC IN THIS SECTION:
    34  delivered v1.3
    39  v1.6 - five mandated splits: LED-043a, LED-053c, LED-053d, LED-058a,
        LED-061a
    42  v1.7 - LED-032a and LED-037a (coverage failures found by W3C) and
        LED-055a (atomic split)
    43  v1.8 - LED-055b (atomic split)
    44  v1.9 - LED-047a (atomic split: the audit result separated from the
        F-to-V mapping)

ALL 44 ARE TIER C. ZERO TIER A - LED-063's re-disposition at v1.8 removed the
only Tier A entry, and DEC-62 computes tier from disposition.

THE COUNT IS DERIVED FROM THE FINISHED DOCUMENT BY ENUMERATION, MECHANICALLY,
AT EVERY ISSUE. It is never a target (DEC-62's principle, W3C's repeated
instruction, and capture 2's exact words: "entry count is never a target").

HISTORICAL RECORD - EVERYTHING BELOW THIS LINE IS HISTORY, NOT LIVE LOGIC.
Asserted totals in this ledger have been wrong three times: a draft 40
corrected to 39 by counting at v1.5, whose stale figure then survived in the
header; a draft 45 at v1.8 that survived into LED-063's live text; and the
prose of this very section at v1.7-v1.8, which still carried v1.5-era claims
("LED-055a is unmandated and is not created here", "no delivered ledger ever
had 39") after later generations had made them false. Those sentences are
REMOVED from live logic, recorded here once, and the lesson is the standing
one: COUNT THE LIST, NEVER TRUST A REMEMBERED TOTAL, and never leave a
true-when-written absolute standing after the state moves.
```
...
touched. All are restored verbatim from v1.3.
  ...
  LED-037  the evidence base: the stated population, the fifteen classified
           hit lines, the six-document enumeration with file and line
           numbers, the four separated non-collision cases, and the routing
           recommendation including the carve-out that Classic-facing
           documents keep "Schedule A" because there it names the VERIFIED
           HOLYWU ORDER.
           NOT RESTORED, AND DELIBERATELY - RECONCILED AT v1.9: the REASON's
           original a-priori reasoning ("an audit record about this document,
           so no other document can hold it"). Live LED-037 withdrew it at
           v1.6 as an invalid uniqueness proof - W3C's Tier C finding - and
           rests CURRENT-UNIQUE on the searched basis instead. Earlier text
           here said the repair's probe table "endorses" that reasoning; the
           table endorses the DISPOSITION, not the a-priori argument, and a
           restoration list must not re-assert reasoning the live entry has
           withdrawn.
...
N2  LED-053d's second carrier lies outside the adjudicated range.
    ...
    RESOLVED (v1.9 annotation): W3C answered Q-K - citing without
    adjudicating is correct - and Review Scope 0.5/0.6 now define the
    occurrence-level and CITED-OUTSIDE-RANGE formats. LED-053d carries both,
    with the Appendix-A occurrence narrowed to lines 1770-1772. Nothing here
    is open.
```

## 0.4h THE ENTRY-SWEEP GATE, AND THE THREE TIMES ITS ABSENCE COST

```text
NEW AT v1.7, AND THE MOST IMPORTANT PROCEDURAL THING IN THIS DOCUMENT.

THE RULE: AFTER EDITING ANY FIELD OF AN ENTRY, RE-READ THE WHOLE ENTRY AND
CHECK THAT NO OTHER FIELD CONTRADICTS THE EDIT.

WHY IT EXISTS. At v1.5 and v1.6 the designer corrected exactly the field each
mandate named and did not sweep the rest of the entry. W3C found three cases at
the closure review, and all three are the same failure:

    LED-055  the uniqueness claim was withdrawn in REASON while "the
             DERIVATION -> this section, uniquely" still stood in PREVAILS,
             THREE LINES ABOVE IT. W3C had rejected that exact wording in the
             Tier C sample review, so it survived TWO corrections.
    LED-061  SWEPT was rebuilt to classify the coder introduction and D2 as
             DIFFERENT while REASON and DUPLICATE-ACTION in the SAME ENTRY
             still listed both as carriers. THE ENTRY BOTH REJECTED AND RELIED
             ON THE SAME TWO FILES.
    LED-043  the fabrication-prohibition half of SWEPT was rebuilt while the
             dct_type half still named "the GAIS files" as live corpus, a
             population DEC-66 excludes.

THIS IS DEC-51 AT ENTRY LEVEL. DEC-51 was written about propositions across a
corpus; the same failure occurs across the fields of one entry, and the ledger
that enumerates DEC-51 committed it three times.

AT v1.7 THIS GATE WAS APPLIED BY HAND AND MISSED THREE DEFECTS - LED-043,
LED-059 and LED-063 each kept a sentence contradicting its own repair, and W3C
found all three. AT v1.8 IT WAS RUN AS AN ACTUAL DIFF over every edited entry,
searching for the terms each edit was meant to remove and discounting
occurrences inside an explicit withdrawal record. IT IMMEDIATELY CAUGHT THREE
LIVE DEFECTS THE DESIGNER HAD MISSED BY EYE, including a TIER field left stale
by its own disposition change. A GATE APPLIED BY THE SAME ATTENTION THAT CAUSED
THE DEFECT IS NOT A GATE.
EXTENDED AT v1.10 WITH THE RULE THAT GOVERNS ALL THE OTHERS: A MECHANICAL
COUNT IS ONLY AS GOOD AS THE RULE IT MECHANISES, AND A FAILING GATE IS AS
LIKELY TO BE WRONG AS THE DOCUMENT.
Three times now a count has disagreed with the text, and the SPLIT has been
even: twice the document was wrong (Q-C missing LED-054's mid-field claim at
v1.9; Q-H's "nine" over a list of eleven), and twice the GATE was wrong at
v1.10 - it counted LED-051a as a STAY-CANONICAL asserter when that entry only
CROSS-REFERENCES LED-045's requirement and its own action is POINTER, and it
counted the coverage entries LED-032a and LED-037a as atomic splits when they
are neither. WHEN A GATE FIRES, DETERMINE WHICH SIDE IS WRONG BEFORE EDITING
ANYTHING. A gate trusted blindly corrupts the document it protects.
THE RULES AS THEY NOW STAND: STAY-CANONICAL population = entries whose ACTIVE
DUPLICATE-ACTION asserts the claim FOR THEIR OWN PROPOSITION; atomic-split
suffixes EXCLUDE coverage additions.

ALSO EXTENDED AT v1.9 WITH COUNT-CONSISTENCY: the v1.8 gate checked only terms the
designer KNEW it had removed, so an abandoned draft figure ("45 Tier C")
survived in LED-063 because nobody knew to search for it. The gate now also
extracts EVERY numeric entry/tier claim in the document and tests each against
the mechanical enumeration. Checks are specified by what can be wrong, not by
what the author remembers changing.

IT IS PARTLY MECHANICAL: after each edit,
diff the entry's remaining fields for the terms the edit was meant to remove.
"uniquely" in LED-055, "coder introduction" and "D2" in LED-061, "GAIS" in
LED-043 - each was findable by search once the question was asked.

THE SAME DISCIPLINE ONE LEVEL UP, recorded at 0.4b: ENUMERATE THE ROUNDS, NOT
ONLY THE DELTAS. v1.5's delta list came from the classification-repair round
alone and missed two of eight Tier C findings.

AND ONE LEVEL UP AGAIN, WHICH v1.7 IS THE FIRST GENERATION TO ADDRESS: AN
OVERLAP CHECK AGAINST PRIOR ENTRIES IS NOT A COVERAGE CHECK AGAINST THE SOURCE.
Section 0.2 has enumerated 33 prior-entry ranges since v1.0 and never once
walked the source range looking for propositions with no entry. Two were
missing. See 0.4e.
```
...
---
# LED-032a  Section 1 - the project-scope and target-material statement

```text
ADDED AT v1.7. W3C found at the closure review that authority line 225 has no
entry. LED-033 begins at line 226 and adjudicates the document-purpose and
single-source proposition; the project-scope sentence before it was not
captured by any entry, so the declared 223-715 coverage was incomplete.

DOCUMENT     authority v1.05, section 1, lines 225-226
             RANGE CORRECTED AT v1.8. The sentence is physically split across
             two lines. DECLARED OVERLAP, recorded rather than hidden: line 226
             also carries the opening of LED-033's proposition. One line, two
             propositions, two entries - which is correct under the
             atomic-claim rule and must not be read as a coverage collision.
CLAIM        "Deblock4 is the project's MPEG-2-aware deblocking filter,
             initially aimed at PAL 576i material recorded by consumer DVD
             recorders."
ASSERTS      two things: WHAT Deblock4 is - the project's MPEG-2-aware
             deblocking filter - and WHAT IT WAS INITIALLY AIMED AT, PAL 576i
             from consumer DVD recorders.
CLASS        Project scope statement
DISPOSITION  CURRENT-DUPLICATE
REASON       Current, and duplicated - BUT THE CARRIERS DIFFER PER PROPOSITION,
             and v1.9 records them separately after W3C checked the settled
             snapshot and found v1.8's attribution wrong.
             THE IDENTITY ("the project's MPEG-2-aware deblocking filter") is
             in the README, both introductions, Project Status, the Concise
             Project Summary and the charter.
             THE TARGET ("PAL 576i material recorded by consumer DVD
             recorders") is NOT in the README - W3C found no "576i" occurrence
             in README v1.12 - and NOT in the designer introduction, which
             carries identity but not this clause. ITS ACTUAL CARRIER IN THE
             SETTLED SNAPSHOT IS THE CONCISE PROJECT SUMMARY v1.5: "Deblock4
             is initially focused on PAL tape material recorded to MPEG-2 by
             consumer DVD recorders."
             PROVENANCE OF THAT CARRIER EVIDENCE, STATED PLAINLY (v1.10): W3C
             verified it DIRECTLY against the settled snapshot at its v1_2
             re-review. It is NOT inherited from the LED-033/LED-042 probe
             results, which searched other propositions.
             THE v1.8 DEFECT, RECORDED PER DEC-51: a copy of the IDENTITY
             clause was allowed to stand as proof of duplication of the TARGET
             clause. That is the copy-of-the-wrong-clause failure at Review
             Scope 0.9, and it is why carrier evidence is now per proposition.
             THE WORD "INITIALLY" IS LOAD-BEARING AND IS WHY THIS IS NOT
             MERELY A TITLE LINE. It marks the target as the ORIGINATING case
             rather than the limit of applicability, which is consistent with
             LED-055a's finding that the tearing derivation generalises beyond
             the interlaced case. An entry that reduced this to "Deblock4 is
             for PAL 576i" would narrow the project.
CONFLICTS    none.
PREVAILS     CANONICAL HOME, PER PROPOSITION - both map to the same place, and
             v1.8 says so explicitly rather than leaving the second implied:
               PROJECT IDENTITY ("the project's MPEG-2-aware deblocking
                 filter")            -> THE CHARTER's project definition
               INITIAL TARGET ("PAL 576i material recorded by consumer DVD
                 recorders")         -> THE CHARTER's project definition
             The charter carries identity AND originating purpose together,
             which is why one home serves both. T1S04's range.
             THIS AUTHORITY IS THE CANONICAL HOME FOR NEITHER. The sentence is
             orientation for the MPEG-2 document, not the definition of the
             project.
DUPLICATE-ACTION  POINTER, for both propositions, to the charter.
             CONCRETE NON-CANONICAL COPIES, NAMED PER PROPOSITION AS DEC-43
             REQUIRES:
               IDENTITY  the README's opening description of Deblock4 as the
                         project's MPEG-2-aware deblocking filter; the
                         designer introduction's project block.
               TARGET    Concise Project Summary v1.5's "initially focused on
                         PAL tape material recorded to MPEG-2 by consumer DVD
                         recorders" - AN ACTUAL CARRIER OF THIS CLAUSE, which
                         the v1.8-named documents are not.
             NOT EXHAUSTIVE.
SWEPT        NO NEW SEARCH ROUND WAS RUN, and that is stated rather than
             implied. The carriers above are drawn from search results already
             recorded in this ledger - the same documents returned by LED-033's
             and LED-042's probes over the settled 46-file population at
             section 0.5.
             CLAIMED SCOPE, PER DEC-50: this establishes that the proposition
             is DUPLICATED, which is all its disposition requires. IT DOES NOT
             CLAIM an exhaustive carrier list, and no uniqueness claim is made
...
TIER     C
PROPOSED
ACTION   POINTER at T3, to the charter's project definition. Keep the word
         INITIALLY in any reduced form - dropping it converts an originating
         case into a scope limit.
VERDICT  [W3C]
```
...
---
# LED-037a  Section 1.2 - the default-scope qualification on the coordinate conventions

```text
ADDED AT v1.7. W3C found at the closure review that authority line 269 has no
entry. LED-038 to LED-040 adjudicate the individual coordinate definitions and
rules; the sentence that SCOPES them was not captured, so the declared
223-715 coverage was incomplete.

DOCUMENT     authority v1.05, section 1.2, line 269
CLAIM        "Unless explicitly stated otherwise:"
ASSERTS      a DEFAULT-SCOPE QUALIFICATION governing the coordinate-convention
             block that follows: the conventions below hold by default, and a
             local statement may override them.
CLASS        Scoping qualification
DISPOSITION  CURRENT-UNIQUE
REASON       No other document in the live corpus scopes this authority's
             coordinate conventions. The conventions themselves are duplicated
             - LED-038's rebuilt table records four files that define the edge
             convention and five more that use it - but the QUALIFICATION
             that they are defaults rather than absolutes exists only here.
             WHY A FOUR-WORD SENTENCE EARNS AN ENTRY: without it, every
             coordinate rule in section 1.2 reads as unconditional, and a
             later section that legitimately overrides one would read as a
             CONFLICT rather than as a permitted local exception. The
             qualification is what makes section 4.5's Case-(b) pitch-2 rule a
             refinement instead of a contradiction. An adjudication that
             captured the rules and dropped their scoping would create
             conflicts that do not exist in the source.
CONFLICTS    none.
PREVAILS     n/a - unique. Canonical home is this section, immediately above
             the conventions it governs. It cannot be relocated: a scoping
             sentence separated from what it scopes stops functioning.
SWEPT        A BOUNDED PROPOSITION-SPECIFIC PROBE WAS RUN AT v1.8, because the
             v1.7 entry claimed CURRENT-UNIQUE while admitting in this very
             field that no search had been run for THIS proposition. W3C found
             that admission and was right: relying on LED-038's probe, which
             searched the CONVENTIONS, cannot establish uniqueness of the
             SCOPING QUALIFICATION. An entry that names its own gap has not
             thereby earned the claim - the same lesson LED-059 taught.
             PROPOSITION SEARCHED: that a stated set of conventions holds BY
             DEFAULT and may be locally overridden.
             THE v1.8 FAMILY WAS REJECTED BY W3C AS THREE GRAMMATICAL
             PERMUTATIONS OF THE SAME SENTENCE, not reasonable independent
             reformulations - DEC-67 Rule 3. The entry even admitted that
             "these are defaults" or "except where noted" would escape it. An
             admission of a gap does not earn a claim. THE RECORD BELOW IS
             W3C's OWN EXPANDED BOUNDED CHECK, adopted at v1.9 as the entry's
             evidence.
             PROBE FAMILY (12 terms): "Unless explicitly stated otherwise" |
             "unless stated otherwise" | "unless otherwise stated" | "unless
             otherwise noted" | "unless noted otherwise" | "unless otherwise
             specified" | "except where noted" | "except as noted" | "except
             where stated" | "these are defaults" | "are defaults" |
             "by default".
             POPULATION: the settled 46-file a5 search population at 0.5.
             6 CANDIDATE FILES, EVERY ONE OPENED AND CLASSIFIED:
               CANONICAL   this authority - the default-scope qualification on
                           the coordinate conventions
               DIFFERENT   AI Charter v1.31 - unrelated "by default"
                           project/safety language
               DIFFERENT   README v1.12 - "Chroma is processed by default"
               DIFFERENT   Toolchain Findings v1.4 - FloatMode .optimized by
                           default
               DIFFERENT   Scopes/ D4 PreScope coder response -
                           heuristic/fallback behaviour by default
               DIFFERENT   Scopes/ T1 review scope v1.7 - an architecture not
                           winning "by default"
             NO SECOND CARRIER of the coordinate-convention default-scope
             proposition was found.
             CLAIMED SCOPE, PER DEC-50: uniqueness within the declared
             population under the declared 12-term family. PROVENANCE OF THE
             CHECK, stated so it is not mistaken for a W3D search: W3C ran it
             at the v1.8 delta review and W3D adopted the record; the delivered
             response is the evidence artifact.
...
TIER     C
PROPOSED
ACTION   RETAIN IN PLACE, and treat as NOT REDUCIBLE at T3. If section 1.2 is
         ever pointered or summarised, this sentence travels with the
         conventions or the conventions become absolutes.
VERDICT  [W3C]
```
...
SWEPT        (i) THE CODEC-SIGNALLING FACTS (a) AND (b). Searched the live
             corpus as defined at section 0.5 for "dct_type" and
             "frame_pred_frame_dct": found this document (many), the Grid
             Knowledge document, both introductions, Project Status, both chat
             blurbs, the Concise Summary and the Scopes/ re-decision documents.
             POPULATION CORRECTED AT v1.7, RECORDED PER DEC-51: v1.5 and v1.6
             both listed "the GAIS files" among the live corpus here. DEC-66
             REMOVES GAIS_investigations/ FROM SEARCH AND ADJUDICATION ALIKE,
             so they are not candidates and are not counted. This half of the
             field was never rebuilt when the other half was - see 0.4h.
             CONCRETE NON-CANONICAL COPY OF (a)/(b), NAMED AS DEC-43 REQUIRES:
             the SECTION 3 REGIME TABLE of this authority, which restates the
             macroblock-level dct_type behaviour and the frame_pred_frame_dct
             forcing/permitting distinction. NOT the Scopes evaluation - that
             carries (c2) only, and attributing it to (a)/(b) was an evidence
             mismatch W3C found at the closure review.
             (ii) THE NO-FABRICATION RULE (c2). REBUILT FROM CLASSIFICATION REPAIR v1.1 for the
             ...
             EVALUATION, WHICH CARRIES (c2) AND ONLY (c2).
             STALE SENTENCE REMOVED AT v1.8, RECORDED PER DEC-51: v1.7 ended
             this subsection by calling the Scopes evaluation "the concrete
             non-canonical copy the (a)/(b) STAY-CANONICAL claim needs" - three
             lines after correctly stating the opposite. The (a)/(b) copy is
             the SECTION 3 REGIME TABLE, named in (i). This is the entry-sweep
             failure section 0.7 of the review scope exists to catch, surviving
             its own repair.
             OCCURRENCE-LEVEL EVIDENCE for (c2), per review scope 0.5:
               FILE:        this authority
               FILE-CLASS:  CARRIER
                            (CORRECTED AT v1.9 from MIXED. Scope 0.4/0.5:
                            MIXED means one file, MULTIPLE MEANINGS; the same
                            proposition twice is a CARRIER with occurrences -
                            the exact misuse rule 0.5 was written to stop, and
                            v1.8 committed it while citing the rule.)
               OCCURRENCES:
                 section 2 / F5      NON-CANONICAL occurrence of (c2)
                 section 15          CANONICAL occurrence of (c2), owned by
                                     T1S01a6
             CITED-OUTSIDE-RANGE:
               location      authority section 15
               proposition   (c2), the no-fabrication rule
               evidence use  establishes that (c2)'s canonical home is outside
                             this sub-tranche, which is why (c2) is POINTER here
               owning tranche T1S01a6
               obligation    a6 must reconcile its adjudication of section 15
                             with this evidence use
...
CLAIM        THE PER-FACT PROVENANCE MAPPING: F1 direct re-verification
             against definition 3.12 and 6.1.4; F2 reclassified DERIVED with
             6.1.1.8, 6.1.1.9, 6.1.1.10 named; F3 direct re-verification
             against 6.1.3; F4 direct verification via W3C report V4.1 and
             6.1.3; F5 direct verification via W3C report V4.3.
             NARROWED AT v1.9 UNDER THE ATOMIC-CLAIM RULE. v1.8 bundled this
             mapping with THE AUDIT RESULT - "no active [H.262-VERIFIED]
             claim in this document depends on GAIS" - under one
             CURRENT-UNIQUE disposition, AND SAID IN THIS VERY FIELD THAT THE
             CLAIM CONTAINED TWO PROPOSITIONS. They do not share a status: the
             mapping is unique, the audit result is duplicated. The audit
             result is now LED-047a. THIS IS THE FOURTH ATOMIC-CLAIM
             RECURRENCE IN THIS LEDGER (LED-053, LED-055, LED-055a, here),
             and the second found by W3C after the rule was consolidated into
             the binding scope.
ASSERTS      that every tagged H.262 claim has a recorded standard citation or
             named verification report - THE MAPPING, not the audit verdict.
CLASS        SPEC-VERIFIED provenance record
DISPOSITION  CURRENT-UNIQUE
REASON       THE MAPPING PROPOSITION IS CURRENT, and it is this entry's ONLY
             proposition - the audit verdict was SPLIT TO LED-047a AT v1.9
             under the atomic-claim rule. This is the only record MAPPING THE
             F-SERIES CLAIMS TO THEIR VERIFICATION REPORTS. It is NOT the only
             place the underlying verification exists.
             (v1.10: this field opened "Both propositions are current ...
             NARROWED AT v1.8" for a full generation after the split - the
             entry-sweep residue class again, in the parent of the very split
             made to fix it.)
             WHY THE NARROWING WAS NEEDED. v1.7 said "the only provenance
             record for the F-series in the live corpus" while its SWEPT field
             tested only the TRUTH OF THE TAG AUDIT INSIDE THIS DOCUMENT. W3C
             found the gap - it is the same confusion corrected at LED-037,
             recurring one entry later: the truth of an audit is not the corpus
             uniqueness of the audit record.
             WHAT THE BOUNDED PROBE FOUND, see SWEPT (iii): the W3C
             verification content DOES exist elsewhere - four Scopes/ documents
             carry the V4.1 and V4.3 classifications. What exists nowhere else
             is the F-TO-V MAPPING. F-numbering is this authority's own scheme;
             the Scopes documents use V-numbering and never connect the two.
             THE VERIFICATION REPORTS ARE EVIDENCE BEHIND THIS RECORD, NOT
             COPIES OF IT - and LED-063 now establishes that their content
             survives, so this entry no longer depends on an unresolved
             question about whether they can be located.
CONFLICTS    none.
PREVAILS     n/a - unique.
SWEPT        THE AUDIT RESULT WAS TESTED WITHIN ITS OWN POPULATION, because
             ...
             POPULATION: every occurrence of the LITERAL STRING
             "H.262-VERIFIED" in authority v1.05, whole document, 1983 lines.
             RESULT: EIGHT hit lines - 33 (the tag definition in the header),
             286 (F1), 291, 298 (F3), 304 (F4), 310 (F5), 338 (the audit's own
             scope sentence) and 366 (the audit result sentence).
             CORRECTED AT v1.7 FROM SEVEN, AND THE CORRECTION MATTERS MORE
             THAN THE NUMBER. v1.5 and v1.6 stated a literal-string population
             and then reported a result that silently excluded line 291,
             "[DERIVED FROM H.262-VERIFIED FACTS]". The entry discussed line
             291 elsewhere, but that does not make the stated population and
             result true as written. A DECLARED POPULATION MUST MATCH THE
             REPORTED RESULT OR THE CHECK IS NOT TESTABLE - DEC-50. W3C found
             it at the closure review.
             CLASSIFICATION OF THE EIGHT:
               SEVEN are standalone [H.262-VERIFIED] tags or prose about the
               tag: 33, 286, 298, 304, 310, 338, 366.
               ONE is the COMPOUND DERIVED TAG at line 291,
               "[DERIVED FROM H.262-VERIFIED FACTS]", which marks F2 as
               DERIVED FROM verified facts rather than itself verified. It is
               a lexical match for a different proposition and does not add a
               member to the tagged population.
             (iii) THE UNIQUENESS PROBE, ADDED AT v1.8 for the narrowed claim.
             PROPOSITION SEARCHED: a record mapping F-series claims to the W3C
             verification reports that establish them.
             PROBE FAMILY: "provenance record" | "verification report V4" |
             "V4.1" | "V4.3".
             POPULATION: the settled 46-file a5 search population at 0.5.
             6 CANDIDATE FILES, EVERY ONE OPENED AND CLASSIFIED:
               CANONICAL   this authority, section 2.1 - the F-to-V mapping
               DIFFERENT   Scopes/ D4 PreScope Round Brief coder response -
                           carries V4.1 and V4.3 as VERIFIED. THE VERIFICATION,
                           NOT THE MAPPING - a hit for a different proposition.
                           (RECLASSIFIED AT v1.9 from CARRIER, per W3C: for
                           the MAPPING proposition these files carry nothing.)
               DIFFERENT   Scopes/ D4 Verification Round Brief v1.0     - same
               DIFFERENT   Scopes/ D4 Verification Round Brief v1.1     - same
               DIFFERENT   Scopes/ D4 PreScope Round Brief v1.2         - same
               DIFFERENT   AI Charter v1.31. The "provenance record" match is
                           about CREDIT AND ATTRIBUTION for contributed work,
                           a different proposition entirely.
             NONE of the four Scopes carriers uses F-numbering or connects a
             V-number to an F-number. THE MAPPING IS UNIQUE; THE VERIFICATION
             IS NOT. That distinction is what the narrowed claim rests on.
             ...
             WHAT THE CENSUS ESTABLISHES FOR THIS ENTRY: the TAGGED
             POPULATION THE MAPPING COVERS - exactly F1, F3, F4, F5 - so the
             per-fact mapping above is complete over the tagged set. THE
             AUDIT-VERDICT USE of this same census (that none rests on GAIS)
             belongs to LED-047a and is not adjudicated here.
...
# LED-047a  Section 2.1 - the GAIS-independence audit result

```text
SPLIT FROM LED-047 AT v1.9 UNDER THE ATOMIC-CLAIM RULE, on W3C's finding that
the provenance MAPPING and the AUDIT RESULT do not share a disposition.

DOCUMENT     authority v1.05, section 2.1 (audit result sentence, line 366,
             within LED-047's range)
CLAIM        "no active `[H.262-VERIFIED]` claim in this document depends on
             GAIS", with the explanation that F1 and F3 were not explicit
             enough in the original V4 report and F2 is deliberately labelled
             DERIVED rather than overstating a direct quotation.
ASSERTS      the AUDIT VERDICT: GAIS-independence of every tagged claim.
CLASS        SPEC-VERIFIED audit result
DISPOSITION  CURRENT-DUPLICATE
REASON       Current - the audit's truth is established by LED-047's
             eight-hit literal check and per-fact records. DUPLICATED: the
             DESIGNER INTRODUCTION v1.28 carries the audit result as a
             standing statement that the authority's verified claims rest on
             H.262 citations and named W3C reports, not on GAIS. W3C
             identified it as a concrete non-canonical copy at the v1.8
             delta review.
CONFLICTS    none.
PREVAILS     CANONICAL HOME: this authority, section 2.1, where the audit and
             its per-fact evidence live together. A verdict separated from
             its evidence is an assurance, which is what DEC-50 exists to
             prevent.
DUPLICATE-ACTION  STAY-CANONICAL.
             CONCRETE NON-CANONICAL COPY, NAMED AS DEC-43 REQUIRES: the
             designer introduction v1.28's GAIS-independence statement. NOT
             EXHAUSTIVE.
SWEPT        NO NEW PROBE WAS RUN, stated plainly. The evidence is (i)
             LED-047's literal-string audit, which establishes the tagged
             population this verdict covers, and (ii) W3C's v1.8 delta
             review, which named the designer-introduction copy. One named
             carrier is sufficient for a DUPLICATE disposition; no exhaustive
             carrier list is claimed.
             SCOPE LIMIT, unchanged from LED-047: this establishes nothing
             about UNTAGGED assertions - that is LED-023's blanket claim,
             owed elsewhere.
...
TIER     C
PROPOSED
ACTION   STAY-CANONICAL; at T3, the designer introduction's copy becomes a
         pointer to section 2.1 so the verdict never travels without its
         evidence.
VERDICT  [W3C]
```
...
SWEPT        REBUILT FROM CLASSIFICATION REPAIR v1.1.
             ...
             2 CANDIDATE FILES, BOTH CLASSIFIED. THE MIXED LABEL IS REPLACED
             AT v1.8 WITH OCCURRENCE-LEVEL EVIDENCE, per review scope 0.5.
             MIXED means one file carries more than one MEANING; here one file
             carries the SAME proposition at two locations. W3C accepted the
             stretch pragmatically at the Tier C round and stated it was not a
             redefinition. The correct record is:
               FILE:        this authority
               FILE-CLASS:  CARRIER
               OCCURRENCES:
                 section 4.5, lines 528-529   CANONICAL occurrence, adjudicated
                                              by this entry
                 Appendix A, lines 1770-1772  CARRIER occurrence, owned by
                                              T1S01a6, NOT adjudicated here
                                              (location narrowed at v1.9 from
                                              the whole 1762-1932 appendix to
                                              the three lines actually
                                              carrying the proposition, per
                                              W3C)
             THE GOVERNING RULE THIS RECORDS: a file-level search hit does not
             establish occurrence-level uniqueness inside that file. The probe
             returned ONE file; the proposition occurs twice in it.
               CARRIER     Scopes/ D4 PreScope Round Brief coder response -
                           Case (b) field picture: luma/chroma field grid, row
                           pitch 2
CITED-OUTSIDE-RANGE:
             location      authority Appendix A, lines 1770-1772
             proposition   (d), the Case-(b) woven-frame row-pitch-2 rule
             evidence use  a second carrier of the same proposition inside the
                           same physical file, which is why this entry's
                           STAY-CANONICAL claim has a named copy at all
             owning tranche T1S01a6
             obligation    a6 must reconcile its adjudication of Appendix A
                           with this evidence use
             Q-K IS ANSWERED AND CLOSED. W3C confirmed at the delta review that
             citing an out-of-range carrier while leaving its adjudication to
             the owning tranche is correct, and supplied this block format.
...
---
# LED-055  Section 5 - the SeparateFields row-projection mechanism

```text
NARROWED AT v1.7 UNDER THE ATOMIC-CLAIM RULE, completing a decomposition W3C
required in the Tier C sample review and found still owed at the recovery
closure review. v1.5 and v1.6 treated "the derivation" as ONE proposition,
withdrew its uniqueness claim in REASON, and left the entry bundled.

W3C's finding named three layers:
    duplicated row-projection mechanism      -> THIS ENTRY
    possibly unique derivation qualifiers    -> LED-055a
    duplicated D4-D01 conclusion             -> this entry's POINTER half

THIS ENTRY NOW COVERS THE MECHANISM AND THE CONCLUSION. The qualifiers -
progressive/frame-DCT generality, and the "correctness requirement, not
performance preference" characterisation - are LED-055a, because W3C did not
find another applicable document stating them in the same complete form and
their disposition therefore differs.

NO NEW CORPUS-SEARCH ROUND WAS RUN. This is a bounded classification repair on
material the Tier C review and Classification Repair v1.1 already exposed.
...
CLAIM        [DERIVED] "SeparateFields sends alternate rows to different
             ...
             preference."
ASSERTS      the ROW-PROJECTION MECHANISM, its fatality for Case-(a) 4:2:0
             chroma, and the CONCLUSION that SeparateFields cannot be the
             general MPEG-2 input contract.
             NARROWED AT v1.7: the derivation's GENERALITY to progressive/
             frame-DCT material and the CORRECTNESS-REQUIREMENT-NOT-
             PERFORMANCE-PREFERENCE characterisation are LED-055a. The CLAIM
             field above still quotes the whole passage, deliberately - the
             source sentence is not being edited, only its propositions
             separated.
...
CONFLICTS    none.
PREVAILS     CANONICAL HOME, PER PROPOSITION: the ROW-PROJECTION MECHANISM ->
             this section, as the fullest statement with its chroma
             consequence. The CONCLUSION "whole-frame input only" -> D4-D01 in
             the section 22 ratified decision register.
             THE WORD "UNIQUELY" IS REMOVED AT v1.7 AND ITS REMOVAL RECORDED
             PER DEC-51. v1.5 and v1.6 both said "the DERIVATION -> this
             section, uniquely" while the REASON field three lines above had
             already withdrawn that claim. W3C found the contradiction twice -
             once in the Tier C sample review and again in the recovery
             closure review. THE ENTRY CONTRADICTED ITSELF because the
             correction was applied to the field the mandate named and the
             rest of the entry was not swept. See section 0.4h.
             THE DERIVATION IS NOT ONE PROPOSITION. It is three, and they no
             longer share a disposition - see the narrowing note below.
DUPLICATE-ACTION  STAY-CANONICAL for the ROW-PROJECTION MECHANISM AND ITS
             CASE-(a) CHROMA CONSEQUENCE; POINTER for the D4-D01 conclusion.
             NARROWED AT v1.9: v1.8 said "STAY-CANONICAL for the derivation",
             and "the derivation" no longer names one proposition - the
             qualifiers are LED-055a and LED-055b with their own dispositions.
             Blanket derivation language reaching into split entries is the
             bundling this family exists to prevent.
             The mechanism stays because this section IS its canonical home -
             the fullest statement, with the chroma consequence - not because
             it is unique.
             CONCRETE NON-CANONICAL COPY OF THE MECHANISM, NAMED AS DEC-43
...
---
# LED-055a  Section 5 - the SeparateFields derivation qualifiers

```text
SPLIT FROM LED-055 AT v1.7, completing the atomic decomposition W3C required in
the Tier C sample review and found still owed at the recovery closure review.

A PROVENANCE NOTE, RECORDED BECAUSE THE IDENTIFIER WILL MISLEAD OTHERWISE. An
entry with this identifier existed in the W3C recovery reconstruction v1.4,
which W3X ruled out as a source at DEC-70; the successor W3D deleted it as
unmandated when rebuilding from v1.3. W3C's knowledge-capture debrief records
that the v1.4 entry was a DELIBERATE structural judgement rather than
collateral compression, and calls it the most important original v1.4 analysis
the rebuild did not preserve. THIS ENTRY IS NOT ADOPTED FROM v1.4. It is
re-derived on the Tier C finding's own evidence, which is a delivered artifact.
The identifier is reused because it is the natural suffix. W3C's own stated
position is that it would defend the NEED for this treatment, not the
identifier or the wording.

DOCUMENT     authority v1.05, section 5, lines 563-564
             RANGE CORRECTED AT v1.9 (v1.8's 562-564 included the generality
             sentence, which is LED-055b's; the qualifier itself spans
             563-564).
CLAIM        "Whole-frame input is a codec-geometry correctness requirement,
             not a performance preference."
ASSERTS      ONE qualifier: the CHARACTER of the constraint - whole-frame input
             binds on CORRECTNESS grounds and is not a performance choice.
             NARROWED AT v1.8, AND THE REASON IS A REAL ERROR. v1.7 bundled TWO
             propositions here - the progressive/frame-DCT GENERALITY and the
             correctness-vs-performance CHARACTER - under one CURRENT-UNIQUE
             disposition, justified by "no other applicable document states
             BOTH in the same complete form."
             THAT ARGUMENT IS INVALID, AND W3C SAID SO PLAINLY: it proves at
             most that the CONJUNCTION is unique. It proves nothing about
             either proposition separately. Committing the atomic-claim error
             inside the entry created to REPAIR an atomic-claim error is the
             sharpest instance of that failure anywhere in this ledger.
             THE GENERALITY IS NOT UNIQUE and moves to LED-055b as a
             CURRENT-DUPLICATE: the Scopes PreScope brief carries it.
CLASS        DERIVED from F4 and the section 5 mechanism, W3X-RATIFIED
DISPOSITION  CURRENT-DUPLICATE
REASON       CHANGED FROM CURRENT-UNIQUE AT v1.9, AND THE CHANGE IS A REAL
             FINDING AGAINST v1.8's METHOD. v1.8's probe searched three
             near-verbatim wordings of "correctness requirement" and returned
             only this authority. THE PROBE WAS LEXICAL WHERE DEC-67 RULE 3
             DEMANDS SEMANTIC - a rule this designer consolidated into the
             binding scope and then did not follow one document later.
             W3C ran a bounded SEMANTIC family over the same 46 files -
             whole-frame input contract | correctness requirement | geometric
             reason | no field-clip instance can deblock - and found the
             Scopes PreScope D4-D01 record carrying the proposition: whole-
             frame input contract, separated-field input NOT supported, reason
             GEOMETRIC, SeparateFields tears frame-organised blocks, no
             field-clip instance can deblock the relevant chroma. A cold
             reader recovers from that record that whole-frame input is a
             correctness constraint, not an optional performance trade. The
             Architecture ReDecision Brief carries it too.
             CURRENT-UNIQUE IS THEREFORE FALSE.
             WHY THE PROPOSITION STILL MATTERS: without it, the whole-frame
             contract reads as an optimisation a future scheduler could trade
             away for speed. Losing it in a T3 reduction would reintroduce the
             rejected architecture by the back door.
CONFLICTS    none.
PREVAILS     CANONICAL HOME: this section, beside the derivation it qualifies.
             The Scopes carrier states it as a decision reason; this section
             states it as the derived constraint itself.
DUPLICATE-ACTION  STAY-CANONICAL.
             CONCRETE NON-CANONICAL COPY, NAMED AS DEC-43 REQUIRES:
             Scopes/Deblock4_D4_PreScope_Round_Brief_for_W3C_v1_2.md, D4-D01
             record. ALSO KNOWN:
             Scopes/Deblock4_D4_Architecture_ReDecision_Brief_for_W3C_v1_0.md.
             NO EXHAUSTIVE CARRIER LIST IS CLAIMED.
SWEPT        THE v1.8 LEXICAL PROBE IS SUPERSEDED AND RECORDED AS THE DEFECT
             IT WAS: three grammatical variants of the source wording,
             returning only this authority, and thereby missing a semantic
             carrier that W3C's independent check found in the same
             population. The evidence of record is now W3C's bounded semantic
             family - whole-frame input contract | whole-frame input |
             correctness requirement | not a performance preference |
             geometric reason | no field-clip instance can deblock - over the
             settled 46-file population, returning FOUR files, ALL CLASSIFIED:
               CANONICAL   this authority
               CARRIER     Scopes PreScope brief (D4-D01)
               CARRIER     Architecture ReDecision Brief
               APPLIES     Architecture ReDecision W3C Evaluation - evaluates
                           and depends on the whole-frame contract without
                           independently stating the correctness qualifier
                           (classified at v1.10; v1.9 left it opened but
                           unclassified, which under scope 0.3 Rule 1 means
                           it was not swept).
             PROVENANCE: W3C ran the check at the v1.8 delta review; the
             delivered response is the evidence artifact. One established
             carrier suffices for the DUPLICATE disposition; nothing
             exhaustive is claimed.
...
TIER     C
PROPOSED
ACTION   RETAIN IN PLACE as canonical, and keep the qualifiers PHYSICALLY
         ADJACENT to the mechanism when T3 reduces duplicates. A pointer that
         carries the mechanism while dropping "correctness requirement, not
         performance preference" would leave the constraint looking optional,
         which is the one reading it exists to prevent.
VERDICT  [W3C]
```

---

# LED-055b  Section 5 - the progressive/frame-DCT generality of the tearing analysis

```text
SPLIT FROM LED-055a AT v1.8. v1.7 bundled this with the correctness-versus-
performance qualifier under one CURRENT-UNIQUE disposition. W3C found that the
uniqueness argument - "no other applicable document states BOTH in the same
complete form" - proves at most that the CONJUNCTION is unique, and separately
found a carrier that refutes uniqueness of THIS proposition.

DOCUMENT     authority v1.05, section 5, lines 562-563
             (range corrected at v1.9 per W3C)
CLAIM        "The same problem exists for progressive/frame-DCT geometry.
             Therefore SeparateFields cannot be the general MPEG-2 input
             contract."
ASSERTS      the GENERALITY of the tearing analysis: it holds for progressive
             and frame-DCT material, not only for the interlaced case that
             motivated it, and therefore SeparateFields fails as a GENERAL
             input contract rather than merely an interlaced one.
CLASS        DERIVED from F4 and the section 5 mechanism, W3X-RATIFIED
DISPOSITION  CURRENT-DUPLICATE
REASON       CHANGED FROM v1.7's CURRENT-UNIQUE. The Scopes/ D4 PreScope Round
             Brief carries this proposition directly in its D4-D01 reason:
             SeparateFields tears frame-organised blocks across two clips, so
             for Case (a) AND FOR PROGRESSIVE MATERIAL no field-clip instance
             can deblock 4:2:0 chroma at all. W3C identified it at the delta
             review, from candidate material already exposed by the settled
             round.
             THE CANONICAL HOME DOES NOT MOVE. This section states the
             generality as a consequence of the mechanism it has just derived;
             the Scopes brief states it as a reason for a decision.
CONFLICTS    none.
PREVAILS     CANONICAL HOME: this section, immediately after the derivation it
             generalises. A generality separated from the derivation it
             generalises stops being checkable.
DUPLICATE-ACTION  STAY-CANONICAL.
             CONCRETE NON-CANONICAL COPY, NAMED AS DEC-43 REQUIRES:
             Scopes/Deblock4_D4_PreScope_Round_Brief_for_W3C_v1_2.md, in its
             D4-D01 reason, which states that the tearing applies to Case (a)
             and to progressive material alike. NOT EXHAUSTIVE - the probe
             family below searched the correctness-versus-performance wording,
             not every paraphrase of "this applies to progressive too".
SWEPT        NO NEW PROBE WAS RUN FOR THIS PROPOSITION, and that is stated
             plainly rather than implied.
             THE EVIDENCE IS W3C's DELTA REVIEW, which quoted the Scopes
             PreScope carrier directly. That is sufficient to REFUTE
             uniqueness - one carrier does it - and this entry claims nothing
             more. A DUPLICATE disposition needs a carrier, not an exhaustive
             carrier list, which is why no fresh round was run.
             WHAT IS NOT CLAIMED: that the Scopes brief is the only other
             carrier, or that the carrier list here is complete. If T1S01b
             finds more, this entry's disposition is unaffected.
...
TIER     C
PROPOSED
ACTION   RETAIN IN PLACE as canonical, and keep it adjacent to the mechanism
         and to LED-055a's correctness qualifier when T3 reduces duplicates.
         The three propositions were separated for adjudication; they should
         not be separated on the page.
VERDICT  [W3C]
```
...
CLASS        DERIVED
DISPOSITION  CURRENT-DUPLICATE
REASON       Current. The FILE-LEVEL corpus composition is stated here and,
             in the live corpus, also in Grid Knowledge v1.2. The closing
             significance sentence is (b) of LED-058 restated.
             CORRECTED AT v1.7. v1.5 and v1.6 asserted that section 15's
             D4-Q14 experiment definition restates the corpus requirement.
             W3C READ SECTION 15 AT THE CLOSURE REVIEW: IT DOES NOT. Section
             15 defines the Q14 ground-truth fields and classes, the B2 and D
             measurement and reporting requirements, the calibration-versus-
             held-out discipline and the architecture decision rule. It does
             NOT name LG MLS, LG XP/SP/LP/EP, home_576i or home_576p.
CONFLICTS    none.
PREVAILS     CANONICAL HOME, PER PROPOSITION: the FILE-LEVEL CORPUS
             COMPOSITION -> THIS SECTION, 6.3. The SIGNIFICANCE sentence ->
             section 6.2 (b), LED-058.
             THE SECTION 15 CLAIM IS WITHDRAWN ENTIRELY AT v1.7 AND RECORDED
             PER DEC-51. v1.5 and v1.6 proposed section 15 as the canonical
             home on the reasoning that a corpus requirement belongs with the
             experiment that consumes it. THAT REASONING WAS PLAUSIBLE AND THE
             FACT WAS FALSE: section 15 does not state the composition, so it
             cannot be its home. THE ENTRY PROPOSED A HOME IT HAD NOT
             CONFIRMED, and said so in its own SWEPT field while proposing it
             anyway - which made the defect visible without preventing it.
             THIS WAS A REAL ADJUDICATION ERROR, not a framing one.
DUPLICATE-ACTION  STAY-CANONICAL for the corpus composition; POINTER for the
             significance sentence.
             CONCRETE NON-CANONICAL COPY OF THE COMPOSITION, NAMED AS DEC-43
             REQUIRES: Grid Knowledge v1.2, which carries the file-level
             composition. That document is live in T1's population until T2
             retires it. NOT EXHAUSTIVE.
             CORRECTED AT v1.8, RECORDED PER DEC-51. v1.7 continued "KNOWN
             NON-CANONICAL COPIES INCLUDE: this section, and the designer
             introduction's Q14 material" - naming THIS SECTION, which the
             same entry had just made the CANONICAL home, as a non-canonical
             copy of itself. The designer introduction is also removed: it
             names corpus CLASSES without the file names, which does not
             assert the FILE-LEVEL composition this entry adjudicates. It is
             APPLIES, not a duplicate. Both were entry-sweep residue from the
             v1.7 correction.
             KNOWN NON-CANONICAL COPIES: none beyond Grid Knowledge are
             currently named. (v1.9: the old list line naming "this section" -
             the canonical home itself - and the designer introduction's
             class-only Q14 material is REMOVED, not merely annotated; v1.8
             added the correction note but left the defective line live above
             it, which is the residue pattern 0.4h records.)
SWEPT        Searched this document for the corpus composition (terms
             ...
             Knowledge v1.2.
             THE CONDITIONAL IS RESOLVED AT v1.7 AND THE CONDITION FAILED.
             v1.6 recorded that section 15 was not adjudicated here, proposed
             it as canonical home anyway, and wrote its own escape clause:
             "if section 15 turns out not to state the composition, this
             entry's PREVAILS is wrong and 6.3 is the canonical home after
             all." W3C read section 15 and the composition is not there. THE
             ESCAPE CLAUSE IS NOW THE FINDING: 6.3 IS THE CANONICAL HOME.
             THE LESSON, RECORDED BECAUSE THE MECHANISM WAS SOUND AND STILL
             FAILED: an entry that honestly flags a conditional it has not
             checked has NOT thereby earned the right to assert it. If the
             fact is checkable from supplied material - and this one was -
             CHECK IT rather than routing it forward as a future gate.
...
TIER     C
PROPOSED
ACTION   RETAIN THE COMPOSITION IN PLACE at 6.3 as canonical; POINTER for the
         significance sentence to 6.2 (b). No conditional remains: the
         section-15 question is settled and needs nothing from T1S01a6.
...
```text
DOCUMENT     authority v1.05, section 7, lines 646-674 (CORRECTED AT v1.7 from 646-673: the CLAIM
             below includes P4's qualification, which is at line 674)
CLAIM        FOUR records.
...
REASON       Current and binding. Stated in the header's provenance
             ...
             T1S01a7), in the DESIGNER introduction as a STANDING
             EXTERNAL-RESEARCH RULE, and in both chat blurbs.
             CORRECTED AT v1.7 AND RECORDED PER DEC-51. v1.5 and v1.6 said
             "both introductions" and named the D2 HolyWu Real Schedule
             document. THIS ENTRY'S OWN REBUILT SWEPT TABLE CLASSIFIES BOTH AS
             DIFFERENT - the coder introduction's match is charter I7
             independent verification of self-affecting criteria, a process
             rule; D2's is W3C's independent verification OF THAT DOCUMENT
             under D0 section 6. THE ENTRY BOTH REJECTED AND RELIED ON THE SAME
             TWO FILES. SWEPT was rebuilt and the surrounding fields were not
             swept - see section 0.4h.
CONFLICTS    none.
...
DUPLICATE-ACTION  STAY-CANONICAL for the rule; POINTER for the precedence
             ...
             verification." KNOWN NON-CANONICAL COPIES ALSO INCLUDE both chat
             blurbs. NOT EXHAUSTIVE.
             REMOVED AT v1.7: the coder introduction and the D2 HolyWu Real
             Schedule document. Both are classified DIFFERENT in this entry's
             own SWEPT table and are not copies of this rule at all.
SWEPT        REBUILT FROM CLASSIFICATION REPAIR v1.1.
...
DISPOSITION  CURRENT-UNIQUE
             CHANGED FROM CONFLICTING AT v1.8, AND THIS IS THE MOST
             CONSEQUENTIAL SINGLE CORRECTION IN THE LEDGER.
             W3C's full Tier-A review OPENED the candidate document and
             established that the report CONTENT SURVIVES - see SWEPT (5).
             With the content located, there is no conflict left to
             adjudicate: section 8 says a detailed calibration record exists
             elsewhere, and IT DOES. Section 24's R8 carries a wrong filename
             for a surviving document, which is a REFERENCE DEFECT IN a6's
             TERRITORY, not a contradiction of section 8's proposition.
             CONFLICTING requires two statements where a decision is needed
             about which wins. There was never such a pair here.
             TIER CONSEQUENCE, STATED SO IT IS NOT MISREAD AS DRIFT: this was
             the ledger's ONLY Tier A entry. Under DEC-62 the tier is computed
             from the disposition, so a5 now has ZERO Tier A; every entry is
             Tier C, and the COUNT IS STATED ONCE, AT 0.4e, rather than
             repeated here to go stale. (v1.9: this sentence previously said
             "45 Tier C" - a figure from an abandoned draft that survived
             because the mechanical gate checked removed terms, not count
             consistency. The gate now checks both.)
             W3C anticipated this and ruled correctly - DO NOT PRESERVE A
             TIER-A COUNT AS A TARGET. The same rule as the entry count: it
             falls out of the evidence.
REASON       BOTH NAMED LOCATIONS RESOLVE, WHICH IS WHY THE PROPOSITION IS
             SIMPLY TRUE. The raw GAIS evidence files exist on disk - EIGHT
             files under GAIS_investigations/: six investigation documents
             plus GAIS_GATING_RESPONSE.txt and
             GAIS_MPEG2_GRID_CONFIRMATION_RESPONSE.txt (v1.9: corrected from
             "six ... plus two response captures in the root"; after the
             DEC-66-era move the response captures live under
             GAIS_investigations/ too, and location must be stated as it is,
             not as it was). DEC-66 removes that tree from T1 search and
             adjudication, so all eight are history rather than population
             members.
             CORRECTED AT v1.8: v1.5 to v1.7 described them here as "all live
             and in T1's population at T1S01b and T1S05", which DEC-66 made
             false. v1.7 corrected that wording later in this same entry and
             left it standing in this opening sentence.
             AND THE W3C VERIFICATION REPORT CONTENT SURVIVES - ESTABLISHED,
             not conjectured, by W3C's Tier-A review which opened the
             W3C-D4-VERIFY-1 document and matched V4.1/V4.3 VERIFIED content
             to what sections 2.1 and 8 cite. Only section 24's R8 filename is
             wrong. (v1.9: the sentence that followed here - "But a STRONG
             LIKELY REFERENT does exist..." - is REMOVED as the superseded
             uncertainty it was; establishment and likelihood cannot both be
             the live state of one field.) Section
             24's reference R8 names the report as
             `Deblock4_D4_W3C_Verification_and_Design_Review_v1_0.md`, and no
             file of that name exists anywhere in the live tree - THE FILENAME
             DEFECT, and the only one. The referent itself is the
             W3C-D4-VERIFY-1 coder-response document in Scopes/, ESTABLISHED
             at SWEPT check 6.
             (v1.10: the sentence "But a STRONG LIKELY REFERENT does exist..."
             stood LIVE here through v1.9 - directly after a v1.9 note
             claiming it had been removed. The removal note went in; the
             removal did not. HISTORICAL note, kept as history: v1.5/v1.6 said
             the report "MAY NOT" exist, v1.7 upgraded that to "strong
             likely", and W3C's Tier-A review then ESTABLISHED it. Each stage
             overstated less; only the last is live.)
             THE v1.7 CORRECTION, RECORDED PER DEC-51: v1.5 and v1.6
             OVERSTATED the finding by conflating two different defects:
                 a WRONG OR STALE FILENAME in R8            - established;
                 the REPORT CONTENT BEING ABSENT            - NOT established,
                     and now positively contradicted by a strong candidate.
             The distinction matters because the two have different remedies:
             a wrong filename is a reference correction at a6, while missing
             content would be a provenance hole feeding T8 and F8. THE
             EVIDENCE SUPPORTS THE FIRST, NOT THE SECOND.
             THE RAW GAIS EVIDENCE FILES remain locatable on disk but are NOT
             population members: DEC-66 removes GAIS_investigations/ from T1
             search and adjudication alike. They are history.
             THE OPENING SENTENCE OF THIS FIELD IS ALSO CORRECTED AT v1.8: it
             still described those files as "live and in T1's population at
             T1S01b and T1S05". That is the stale wording v1.7 corrected in
             one place and left standing in another - entry-sweep residue,
             found by W3C, and the third time this exact class of defect has
             appeared in this ledger.
CONFLICTS    NONE. WITHDRAWN AT v1.8.
             v1.5 through v1.7 recorded a conflict with section 24's R8 and
             with section 2.1. There is no conflict: R8 names a surviving
             document by the wrong filename. A stale reference is a defect in
             the reference, not a competing proposition.
             CARRIED FORWARD RATHER THAN DROPPED - the wrong filename is real
             and someone must fix it. See CITED-OUTSIDE-RANGE below.
PREVAILS     n/a - unique. Canonical home is this section: it is the only
             place in the live population stating WHERE the detailed
             calibration record lives.
             NO LONGER UNDECIDED. v1.5 to v1.7 deferred on the ground that
             resolving the conflict meant establishing what the report was,
             which belonged to a6. THE REPORT HAS NOW BEEN ESTABLISHED, by
             W3C, from a document already inside the settled population. The
             deferral has been discharged by evidence rather than by decision.
CITED-OUTSIDE-RANGE:
             location      authority section 24, reference R8
             proposition   the filename of the W3C verification report
             evidence use  R8's filename is wrong; the document it names
                           survives as the W3C-D4-VERIFY-1 coder response.
                           This entry uses that fact to establish that its own
                           pointer is TRUE, and adjudicates R8 not at all.
             owning tranche T1S01a6, under DEC-55
             obligation    a6 must correct R8's filename and reconcile its
                           adjudication with this evidence use. THE CONTENT IS
                           NOT MISSING - only the name is wrong.
SWEPT        POPULATION AND BASIS, per DEC-50. SIX CHECKS, numbered in order
             below (v1.10: the heading said FIVE while enumerating six - the
             uniqueness check added at v1.9 was inserted without updating the
             count, the exact defect class the count-consistency gate exists
             for, one field above where the gate was looking).
             (1) Filename search of the
             ...
             and this entry does NOT claim it is the referent.
             (5) THE REFERENT IS ESTABLISHED. UPGRADED AT
             v1.8 FROM W3C's FULL TIER-A REVIEW, WHICH OPENED THE DOCUMENT.
             `Scopes/Deblock4_D4_PreScope_Round_Brief_for_W3C_v1_2_CODER_RESPONSE.md`
             carries the header "Deblock4 - D4 Pre-Scope - W3C Verification and
             Independent Design Review", deliverable W3C-D4-VERIFY-1, route
             W3C -> W3X -> W3D. IT IS A REPORT FROM W3C.
             ITS CONTENT MATCHES WHAT THIS AUTHORITY CITES, section by section:
               V4.1  Chroma organisation - VERIFIED, H.262 6.1.3, 4:2:0
                     chrominance always frame-organised for DCT coding
               V4.3  dct_type signalling and semantics - VERIFIED, macroblock-
                     level syntax, frame_pred_frame_dct conditions, dct_type
                     0 frame / 1 field
             Its summary table records both as VERIFIED. Sections 2.1 and 8 of
             this authority attribute F4 and F5 to exactly "V4.1" and "V4.3".
             THE CONTENT REFERENCED BY THE AUTHORITY'S SHORTHAND SURVIVES.
             WHAT REMAINS WRONG IS ONLY THE FILENAME in section 24's R8, which
             is a6's to correct. See CITED-OUTSIDE-RANGE above.

             (6) THE UNIQUENESS CHECK FOR THE LOCATION POINTER, adopted at
             v1.9 from W3C's re-review. PROPOSITION: section 8 is the only
             place in the population stating WHERE the detailed calibration
             record lives. FAMILY: detailed calibration record | calibration
             record | raw GAIS evidence | W3C verification report.
             POPULATION: the settled 46 files. THREE candidates, all opened:
               CANONICAL   this authority, section 8 - the location pointer
               DIFFERENT   Scopes/ D4 Verification Round Brief v1.1 -
                           calibration-record planning/coverage, not a
                           location pointer
               DIFFERENT   Scopes/ D4 PreScope Round Brief v1.2 - calibration
                           planning/record requirements, not the location
             NO SECOND CARRIER FOUND; CURRENT-UNIQUE rests on this record.
             PROVENANCE: W3C ran it; the delivered response is the artifact.
             THIS CLOSES A SUSPECTED PROVENANCE HOLE RATHER THAN OPENING ONE.
             v1.5 to v1.7 framed this as a record that might not exist, which
             would have fed T8 and the F8 provenance question. It exists.
             Establishing
...
DERIVED      TWO SECTIONS DEPEND ON THIS DOCUMENT AND ONE OF THEM IS THE
             PROVENANCE AUDIT. Section 2.1's result - that no active
             H.262-VERIFIED claim depends on GAIS - rests for F4 and F5 on
             "W3C verification report V4.1" and "V4.3". THAT REPORT'S CONTENT
             IS LOCATED (SWEPT check 5), so F4 and F5's provenance is both
             recorded AND RETRIEVABLE, and LED-047's audit is externally
             checkable against the surviving report.
             (v1.9: the former live branch "if that report cannot be located,
             F4 and F5's provenance is recorded but not RETRIEVABLE" is
             REMOVED - it has been located, and a live field must not carry a
             contingency its own SWEPT has discharged.)
             F4 additionally carries a direct clause citation - H.262 6.1.3 -
             independently checkable without the report. F5 carries only the
             report reference, which is exactly why locating the report
             mattered.
DERIVED-BASIS  Section 2.1's F4 and F5 records; the SIX checks above;
             DEC-58 (discharged for content, open only for the R8 filename).
```

```text
TIER     C
PROPOSED
ACTION   CARRY THE WRONG R8 FILENAME TO T1S01a6 FOR CORRECTION, via the
         CITED-OUTSIDE-RANGE record above. That is the whole forward action.
         a6 is NOT asked to rediscover whether the report exists - it exists,
         it is the W3C-D4-VERIFY-1 coder response, and its V4.1/V4.3 content
         is matched. a6 corrects the reference and reconciles with this
         evidence use.
         (v1.9: the former action - "establish what the report is, whether it
         survives, and under what filename, IF IT CANNOT BE ESTABLISHED
         re-verify F5 against H.262 directly" - is REMOVED as discharged.
         Every question it routed forward is answered in this entry.)
VERDICT  [W3C]
```
...
Q-C  STAY-CANONICAL, AND THE COUNT IS NOW 20 - RECOMPUTED AT v1.10
     UNDER THE CORRECTED RULE: every entry whose ACTIVE DUPLICATE-ACTION
     ASSERTS A STAY-CANONICAL CLAIM FOR ANY PROPOSITION, not merely those
     whose field's first word is STAY-CANONICAL. (v1.9 counted 19 by
     first-line prefix and MISSED LED-054, whose field reads "POINTER for
     (a), (b) and (d); STAY-CANONICAL for (c)" - a mechanical count is only
     as good as the rule it mechanises, and W3C counted 20.)
     LED-036  LED-041  LED-042  LED-043  LED-045  LED-046
     LED-047a  LED-048  LED-049  LED-051  LED-053c  LED-053d
     LED-054  LED-055  LED-055a  LED-055b  LED-057  LED-058
     LED-059  LED-061
     Each names at least one concrete non-canonical copy per DEC-43, and each
     copy names the RIGHT CLAUSE. Is any of these claims resting on a copy of
...
Q-F  WITHDRAWN AT v1.8. It asked whether LED-059's conditional mapping into
     section 15 was acceptable. THERE IS NO CONDITIONAL LEFT: you read section
     15, the corpus composition is not there, and section 6.3 is the canonical
     home. The question is left visible so the withdrawal is on the record.

Q-G  ANSWERED AND WITHDRAWN AT v1.8. It asked whether CONFLICTING was right
     for LED-063 when the conflict could not be resolved inside the
     sub-tranche. YOU ANSWERED NO, and established why: there was never a
     conflict. Section 8 says a calibration record exists elsewhere and it
     does; section 24 carries a wrong filename for a surviving document. The
     entry is now CURRENT-UNIQUE and the ledger has zero Tier A entries.
     WHAT REPLACES IT, and it is a better question: does the
     CITED-OUTSIDE-RANGE record at LED-063 carry enough for a6 to correct R8
     without re-deriving the referent?

Q-H  THE SPLIT SET - CURRENTISED AT v1.10. The suffix entries stand at
     ELEVEN: LED-043a, LED-047a, LED-053c, LED-053d, LED-055a, LED-055b,
     LED-058a, LED-061a from the a5 rounds, plus LED-051a, LED-052a and
     LED-053a from v1.3. (v1.9 wrote "nine" above a list naming eleven -
     prose arithmetic disagreeing with its own enumeration, the same defect
     as every other trusted total in this ledger's history. The v1.5-era
     framing "five suffix entries, and LED-055 takes NO suffix" remains
     withdrawn as stale.) The question
     stands in its current form: does each suffix entry carry a proposition
     whose DISPOSITION genuinely differs from its parent's, and whether LED-051 and LED-055
     really need none.
...
Q-K  ANSWERED AND CLOSED. You confirmed that citing an out-of-range carrier
     while leaving its adjudication to the owning tranche is correct, and the
     formats now live at Review Scope 0.5/0.6. LED-053d carries the
     occurrence record (Appendix A narrowed to lines 1770-1772) and the
     CITED-OUTSIDE-RANGE block. Left visible so the closure is on the record.
...
Q-K2 THE TIER C FINDINGS, ALL EIGHT - CLAIM HISTORY STATED HONESTLY.
     v1.6 asserted all eight were applied; FALSE - three were partial. v1.7
     asserted the three were then applied end-to-end; ALSO FALSE - each still
     carried a contradicting sentence, and v1.8/v1.9 removed them under the
     mechanical gate. THE CLAIM AS OF v1.9: all eight findings applied, with
     every subsequent W3C re-review finding on those entries also applied.
     CHECK IT AGAINST YOUR OWN REVIEWS, NOT THIS LEDGER'S ACCOUNT - that
     instruction stands from the version of this question that was made only now. The original question
     stands unchanged below, because checking it against your own review
     rather than against this ledger's account of it is exactly what caught
     the problem.

Q-K2b THE ORIGINAL FORM OF THE QUESTION. v1.6 exists because v1.5 applied six of
...
Q-L  THE PROVENANCE RULE ITSELF - REFRAMED AT v1.9 TO MATCH THE CURRENT
     DELTA SHAPE. Section 0.4b states that no wording is taken from the W3C
     recovery reconstruction v1.4 (LED-055a and LED-047a are RE-DERIVED on
     delivered W3C findings, with the provenance notes saying so). TWO TESTS
     NOW APPLY: (1) the BOUNDED DELTA - diff v1.8 against this file; every
     difference should trace to your v1_2 response items A-P or to the
     mechanical gate's findings recorded in the revision history; (2) the
     PRESERVATION BASELINE - v1.3 text outside recorded deltas should
     survive verbatim. The original instruction follows as history: TEST IT
     DIRECTLY by comparing v1.3 against this file: every difference should
     appear in the 0.4c delta list
...
*End of T1S01a5 ledger v1.10. Nothing here is ratified. Every PROPOSED ACTION
...
```text
v1.10 (2026-08-20) DEC-85 FINAL CORRECTION GENERATION. Applies W3C's v1_3
     re-review items A-P in full; nothing disputed; NO new probes, NO
     resampling, NO method reopening. AFTER THIS GENERATION a5 CLOSES - any
     further residue is recorded for T1S01a7, not corrected in a5 (DEC-85).
     THE FINDINGS WERE ALMOST ALL THE PARTIAL-REPLACEMENT CLASS, IN TEXT THAT
     CLAIMED TO HAVE FIXED THAT CLASS: LED-047's parent still opened "Both
     propositions" a generation after the split; LED-063 still carried "But a
     STRONG LIKELY REFERENT does exist" DIRECTLY AFTER a note claiming that
     sentence had been removed; LED-059's live ACTION still held the history
     it said was relocated; LED-055a left one probe candidate unclassified,
     which under Rule 1 means unswept. All removed or classified now.
     THE COUNT MACHINERY ALSO FAILED THREE WAYS AND EACH IS RECORDED AT ITS
     SITE: 0.4d printed 26 entries under a 28 heading because the v1.9
     restore of a misapplied edit WAS MADE AND NEVER WRITTEN TO DISK (a fix
     that is not persisted is not a fix - gates now run on the file re-read
     from disk); Q-C counted 19 by first-line prefix and missed LED-054's
     mid-field STAY-CANONICAL (a mechanical count is only as good as the rule
     it mechanises - now: any active DUPLICATE-ACTION asserting the claim,
     count 20); Q-H wrote "nine" above its own list of eleven. LED-063's
     SWEPT said FIVE while enumerating six; now SIX, numbered 1-6 in order,
     with DERIVED-BASIS matched.
     ALSO: 0.4c retitled as the ORIGINAL classification-repair delta set;
     0.4d labelled as the D1/v1.5 historical snapshot with LED-038/039
     restored; D7 marked historical with current pins living in the header
     only; LED-047 census reframed as mapping/tag-population evidence;
     LED-032a target-carrier provenance stated as W3C's v1_2 direct
     verification; footer corrected from the two-generations-stale "v1.8".
     ENTRY COUNT UNCHANGED AT 44, ALL TIER C.
v1.9 (2026-08-20) BOUNDED RE-REVIEW CORRECTIONS - W3C's v1_2 response, items
     A-P, applied in full; nothing disputed. NO probe rerun of the settled 22,
     NO Tier C resampling, NO method or architecture reopening. W3X ruled
     (2026-08-20) at most two further generations before a5 closes; this is
     the first.
     THE TWO SUBSTANTIVE FINDINGS, BOTH AGAINST v1.8's OWN METHOD:
     LED-055a  CURRENT-UNIQUE -> CURRENT-DUPLICATE. v1.8's probe was LEXICAL
               where DEC-67 Rule 3 demands SEMANTIC - three near-verbatim
               wordings, returning only the authority - and W3C's semantic
               family found the Scopes PreScope D4-D01 carrier stating the
               whole-frame contract as mandatory for geometric correctness.
               The designer consolidated Rule 3 into the binding scope and
               did not follow it one document later. STAY-CANONICAL, carrier
               named, range 563-564; LED-055b range 562-563.
     LED-047   split under the atomic-claim rule - FOURTH recurrence. The
               F-to-V mapping keeps CURRENT-UNIQUE (the four Scopes V4 files
               reclassified DIFFERENT for that proposition); the
               GAIS-independence audit result is now LED-047a,
               CURRENT-DUPLICATE, with the designer introduction v1.28 as the
               named non-canonical copy.
     ADOPTED FROM W3C's OWN BOUNDED CHECKS, with provenance stated in each
     entry: LED-037a's 12-term default-scope family (six candidates, no
     second carrier - CURRENT-UNIQUE stands on real evidence now) and
     LED-063's location-pointer uniqueness check (three candidates, no second
     carrier).
     END-TO-END RESIDUE REMOVED: LED-032a's target-clause carriers corrected
     to the Concise Project Summary (the copy-of-the-wrong-clause failure);
     LED-043's occurrence FILE-CLASS MIXED -> CARRIER per scope 0.5;
     LED-053d's Appendix-A occurrence narrowed to lines 1770-1772; LED-055's
     parent action narrowed to the mechanism, no longer reaching into the
     split entries; LED-059's defective list line and relocated-history
     marker removed from live fields; LED-063's stale draft count, stale GAIS
     locations, "strong likely" uncertainty, cannot-be-located DERIVED branch
     and discharged ACTION all removed or historicalised.
     SELF-CHECK REBUILT: 0.1 records the split as RATIFIED (DEC-68); 0.3
     Finding B now states the surviving-content/wrong-filename finding; 0.4c
     scopes D1-D7 honestly as the original repair delta set; 0.4e carries ONE
     current count derivation with history quarantined below it; 0.4f
     reconciles the LED-037 restoration with the live entry's withdrawal;
     0.4g N2 annotated resolved. Q-C recomputed mechanically (nineteen);
     Q-F/Q-G/Q-J/Q-K closed on the record; Q-H/Q-K2/Q-L currentised.
     ENTRY COUNT 43 -> 44 (LED-047a). ALL TIER C.
v1.8 (2026-08-20) DELTA-REVIEW CORRECTIONS. Same author, same v1.3 textual
     basis, same provenance rule. Applies W3C's delta review of v1.7. NO probe
     rerun of the settled 22, NO Tier C resampling, NO method or architecture
     reopening. Three bounded proposition-specific probes were run, each for a
     NEW or NEWLY-NARROWED proposition, and each is recorded in full.
     LED-063 - THE MOST CONSEQUENTIAL CORRECTION. W3C's full Tier-A review
     OPENED the candidate document and established that the W3C verification
     report CONTENT SURVIVES as the W3C-D4-VERIFY-1 coder response, carrying
     V4.1 and V4.3 as VERIFIED - exactly what this authority cites. There is
     therefore NO CONFLICT: section 24's R8 holds a wrong filename for a
     surviving document. Disposition CONFLICTING -> CURRENT-UNIQUE and the
     tier recomputes A -> C under DEC-62, leaving the ledger with ZERO Tier A
     entries. A CITED-OUTSIDE-RANGE record carries the filename correction to
     a6. THIS CLOSES A SUSPECTED PROVENANCE HOLE that had been feeding toward
     T8.
     LED-055a / LED-055b - v1.7 bundled two propositions and justified
     uniqueness by "no other document states BOTH in the same complete form",
     which proves at most that the CONJUNCTION is unique. The
     progressive-material generality is DUPLICATED - the Scopes PreScope brief
     carries it - and is now LED-055b, CURRENT-DUPLICATE. LED-055a keeps only
     the correctness-versus-performance qualifier, on a bounded probe of its
     own. Range corrected to 562-564.
     LED-037a and LED-047 both claimed CURRENT-UNIQUE on evidence that tested
     something else - LED-037a on a probe that searched the conventions rather
     than their scoping, LED-047 on an audit's truth rather than its corpus
     uniqueness. Both now carry bounded proposition-specific probes. LED-047's
     claim is NARROWED to the F-to-V mapping, because four Scopes documents
     carry the verification content itself.
     LED-043, LED-059 and LED-063 each still contained a stale sentence
     contradicting its own repair. All three fixed - see 0.4h.
     LED-053d's stretched MIXED label is replaced by occurrence-level evidence
     plus a CITED-OUTSIDE-RANGE record. Q-K is answered and closed.
     LED-032a range corrected to 225-226 with the overlap declared, and both
     propositions mapped to the charter explicitly.
     ENTRY COUNT 42 -> 43. ALL TIER C; ZERO TIER A.
     Header pins corrected: review scope v1.11 -> v1.13, work queue v1.30 ->
     v1.33. Q-F and Q-G withdrawn as answered.
     THE ENTRY-SWEEP GATE WAS RUN MECHANICALLY rather than by hand, and caught
     three live defects the designer had missed by eye.
v1.7 (2026-08-19) RECOVERY-CLOSURE CORRECTIONS. Same author, same v1.3 textual
     basis, same provenance rule. Applies W3C's recovery-closure review of
     v1.6. NO search rerun, NO Tier C resampling, NO method or architecture
     reopening.
     COVERAGE - TWO PROPOSITIONS INSIDE THE DECLARED RANGE HAD NO ENTRY:
       LED-032a  authority line 225, the project-scope/target statement
       LED-037a  authority line 269, the default-scope qualification that
                 governs the whole coordinate-convention block
       LED-060   range corrected 646-673 -> 646-674; its own CLAIM includes
                 P4's qualification at line 674
     THE THREE PARTIAL TIER C REPLACEMENTS, now applied END-TO-END:
       LED-055   false "uniquely" removed from PREVAILS, and the atomic
                 decomposition W3C required at Tier C completed - the
                 derivation qualifiers are now LED-055a
       LED-061   coder introduction and D2 removed from REASON and
                 DUPLICATE-ACTION; the entry no longer contradicts its own
                 SWEPT table
       LED-043   stale GAIS/T1 population removed from the dct_type half of
                 SWEPT; the section-3 regime table named as the (a)/(b)
                 non-canonical copy instead of the (c2)-only Scopes carrier
     TESTED CLAIM - LED-047 reported seven literal hits for a literal-string
     population that returns EIGHT. Now reports eight and classifies line 291's
     compound "[DERIVED FROM H.262-VERIFIED FACTS]" tag separately.
     ADJUDICATION ERROR - LED-059 proposed authority section 15 as canonical
     home for the file-level corpus composition. W3C read section 15: it does
     not state it. The claim is withdrawn, section 6.3 is the canonical home,
     and the entry's own escape clause has become its finding.
     PROVENANCE - LED-063 said the W3C verification report "MAY NOT" exist.
     That conflated a wrong filename with missing content. A strong likely
     referent survives as the W3C-D4-VERIFY-1 coder response. The entry now
     separates the two, and its stale description of GAIS_investigations/ as
     live population is corrected under DEC-66.
     ENTRY COUNT 39 -> 42. Two of the three additions are COVERAGE FAILURES,
     not refinements - see 0.4e.
     NEW SECTION 0.4h RECORDS THE ENTRY-SWEEP GATE and the three failures that
     made it necessary, plus the two larger versions of the same error: a delta
     list built from one round of two, and an overlap check mistaken for a
     coverage check.
     Q-K2's "all eight Tier C findings applied" is WITHDRAWN as false at v1.6
     and re-asserted only now.
...
v1.5 (2026-08-19) POST-REPAIR REWRITE, by the successor W3D session.
     ...
     LED-053d, LED-058a, LED-061a. At v1.7 it is 42. Section 0.4e records why the count is
