...
Current position (2026-08-02, end of session; verify against repo):
    ...
    short-boundary W3D deliverables with a W3X checkpoint after each:
      D0 Preface + Binding Knowledge Index: COMMITTED at
         dev_documentation/reference/
         Deblock4_Stage_2C_D0_Preface_and_Binding_Knowledge_Index_v1_3.md
         (cite THAT filename/path). K1-K25 indexed; K19 = the three-layer
         comparison doctrine (general not-bit-exact / Classic external
         byte-exact-TARGET-with-investigation / internal byte-identical);
         section 6 = the standing TWO-SIDED knowledge-sweep protocol whose
         6.1 block is embedded verbatim in every scope henceforth. D0 is a
         LIVING index: new findings append as the next K-number.
      ...
      D2 HolyWu Real Schedule: DELIVERED as
         Deblock4_Stage_2C_D2_HolyWu_Real_Schedule_v1_1.md - the
         line-by-line inspection of the pinned deblock.cpp with
         file:function:line citations. Load-bearing facts: 4-pixel H.264
         grid; band-by-band top-to-bottom, left-to-right, horizontal edge
         BEFORE vertical at each crossing, SEQUENTIAL IN-PLACE (edges read
         already-modified pixels - any reorder changes output; this IS
         Schedule A); K7 footprints confirmed; luma-on-chroma confirmed;
         threshold tables reproduced and machine-verified 61/61; watchpoints
         WP-1..WP-6 (esp. WP-1 bias-then-FLOOR shifts on negatives - never
         "improve"; WP-5 c0 indexed by aIndex - faithful, do not fix);
         float path EXISTS and is numerically different (no biases, no
         clamp); mod-8 pad-filter-crop self-wrapping for non-mod-8 input
         (never fires for project material). Tabled decisions T-1 (WP-5
         at 3C quality gate), T-2 (float support, D4), T-3 (non-mod-8
         policy, D4). STATUS: awaiting W3C INDEPENDENT VERIFICATION against
         the pinned source (the D0 section-6 two-sided sweep for D2).
      D3 Independent scalar obligations + loose whole-image sanity gate:
         NOT STARTED. Integer obligations mandatory; float conditional on
         T-2. Vectors must pin WP-1 negative-delta and WP-6 boundary-
         equality cases. W3C reviews D3 before ratification (I7 spirit:
         the obligations judge the coder's future work).
      D4 Stage 2C coder scope: NOT STARTED. Mandatory W3C pre-
         ...
         ZERO vector code in 2C); K17 delivery self-containment; decisions
         T-2/T-3; oracle-construction exception acceptance basis.
...
Immediate designer action:
    Determined by where the D-sequence stands (ask W3X): if D2 coder
    verification has findings, resolve and append K-numbers to D0; else
    author D3, then D4. Read D0 v1_3 FIRST - it is the knowledge net and
    its checklist duty binds every deliverable you produce.
