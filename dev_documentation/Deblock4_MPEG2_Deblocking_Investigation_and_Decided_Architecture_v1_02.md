...
```text
STATUS AT 2026-08-16. No Deblock4 filtering code exists; the filter is a
validated pass-through shell. No kernel scope is open. The next artifact
is an EXPERIMENT, not code. Nothing below is provisional unless marked
UNDER EVALUATION.

--- SETTLED, NOT RE-OPENED ---------------------------------------------
1  INPUT CONTRACT: the plugin receives the WHOLE interleaved frame plus a
   declared source mode. SeparateFields input is NOT supported for MPEG-2
   (it tears frame-organised blocks across two clips; 4:2:0 chroma in
   Case (a) then cannot be filtered at all). [D4-D01]
2  MODE DECLARATION: three values, not two - progressive /
   frame-coded-interlaced (Case a) / field-pictures (Case b). The old
   "mpeg2_field_separated" vocabulary is retired: its name now invites
   the forbidden input. No TFF/BFF parameter - field order does not move
   any block boundary. [D4-D02, F7]
3  CHROMA IS SETTLED BY THE STANDARD for 4:2:0: chroma blocks are ALWAYS
   frame-organised (8 consecutive chroma rows) even when luma is field
   DCT. No detection, no decision. 4:2:2/4:4:4 chroma FOLLOWS luma and
   therefore CANNOT inherit that simplification. [F4, H.262-VERIFIED]
4  LUMA IS THE HARD PART: dct_type is per MACROBLOCK, invisible in
   decoded pixels. Any uniform per-frame luma grid is an approximation.
   [F5, F6]
5  VERTICAL EDGES ARE GEOMETRY-INVARIANT: they sit at x = 8k in every
   case and filter within a row. No parity splitting, no phase decision,
   and no obstacle to SIMD. Frame-vs-field DCT changes HORIZONTAL edge
   geometry only.
6  NO HIDDEN TEMPORAL STATE in v1 (fmParallel determinism). [D4-D06]
7  OWN ORACLE: nothing is inherited from Classic - not code, not proofs,
   not thresholds. [D4-D08]
8  v1 FILTERS THE NOMINAL GRID ONLY; inherited motion-shifted blockiness
   is a documented limitation, not a target. [D4-D09, D4-Q11]

--- ADOPTED, PENDING THE MEASUREMENT --------------------------------
9  PRIMARY CANDIDATE - ARCHITECTURE B2 (macroblock-anchored edge
   topology): classify each 16x16 luma macroblock FRAME / FIELD /
   UNKNOWN from pixels, then DERIVE the edge set rather than assign a
   regional "phase":
       internal edge at mb_y+8   : exists ONLY for FRAME (pitch 1)
       boundary at mb_y+16       : FRAME/FRAME -> one pitch-1 edge
                                   FIELD/FIELD -> two pitch-2 parity edges
                                   MIXED       -> two pitch-2 edges
                                   (the seam is an EXPLICIT EDGE TYPE)
       UNKNOWN                   : filter nothing there, count it
   Spans (plane, orientation, phase, pitch, parity, bounds) are compiled
   from that map and remain the SIMD unit. [D4-D12, D4-D05, D4-D07]
10 MANDATORY FALLBACK AND COMPARATOR - ARCHITECTURE D (detector-free):
   pitch-2 at macroblock-row boundaries always; the single TRUE pitch-1
   internal candidate tested strictly, optionally with the scaled-
   threshold idea. Deterministic, no detector, no UNKNOWN. [D4-D12]

--- REJECTED, WITH REASONS RECORDED ------------------------------------
11 ARCHITECTURE A (README union step-4 grid + scaled midpoint
   thresholds): REJECTED. Its geometry was an artefact of separated-field
   coordinates and does not survive the whole-frame contract; and its
   "let the activation test sort it out" premise has a PROVEN in-principle
   false-activation floor (a real compression seam and a harmless
   gradient can present identical local pixels). midpoint_threshold_scale
   is orphaned as a mechanism; its parameter fate is a kernel-scope
   question. [Appendix C]
12 ARCHITECTURE C (motion-based classification): REJECTED - fails on
   static field-coded content in both directions at once.

--- UNDER EVALUATION: THE NEXT ARTIFACT --------------------------------
13 D4-Q14 ARCHITECTURE DISCRIMINATOR EXPERIMENT (section 8) decides
   between B2 and D on measured evidence before any mathematics is
   frozen: extract per-macroblock dct_type TRUTH from real PAL MPEG-2
   bitstreams; run both legs over the decoded pictures; report B2's
   confusion matrices / confidence margins / UNKNOWN rate / FALSE-
   CONFIDENT rate (the dangerous cell), and A-D's true-vs-false candidate
   feature distributions with ROC sweeps; keep FRAME/FRAME, FIELD/FIELD
   and MIXED boundaries separate; give NO_DCT / skipped / motion-only
   macroblocks their OWN truth class. DECISION RULE: B2 ships only if its
   false-confident rate is acceptable; otherwise D ships.
   STATUS: experiment plan not yet drafted. THIS IS THE NEXT ARTIFACT.

--- STILL OPEN, NOT YET ADDRESSED --------------------------------------
14 D4-Q02 kernel mathematics; D4-Q04 eligibility/footprint at 8x8;
   D4-Q05 midpoint_threshold_scale parameter fate; D4-Q10 chroma vertical
   siting; D4-Q12 pipeline position; D4-Q13 per-MB side data; D4-Q16
   parameter-surface redesign. All are KERNEL-SCOPE questions, after 13.

--- OUTSTANDING PROCESS WORK (PAUSED BY W3X) ---------------------------
15 T1 CONSOLIDATION SWEEP: POSTPONED. 17 documents carry MPEG-2 content;
   only README 3.11/3.13 and its settled-decisions table have been read.
   Known pending items already flagged: the analyser rules require an
   unmodified-source pre-pass and per-call scratch under fmParallel;
   schedule remains output-defining; proper chroma is a separate quality
   gate; grid origin / crop ordering is load-bearing; and the current
   Deblock4LumaStepY / midpoint audit-property model CANNOT express mixed
   B2 geometry as one per-frame string.
16 Then: T2 retire the Grid Knowledge doc (superseded by this one, with
   the currency-audit edit); T3 de-duplicate MPEG-2 knowledge elsewhere
   into references; T4 boundary-set mathematics; T5 detector mathematics;
   T6 the D4-Q14 plan (depends on T5); T7 commit.
17 README STATUS RULING NEEDED: the currency audit calls it "fallback
   general guidance", which is what allowed a ratified architecture to go
   unread for four investigation rounds. A document holding ratified
   design is not fallback anything.
```
...
D4-D12  ARCHITECTURE RE-DECISION (W3X, 2026-08-16; full record in
        Appendix C). The 2026-08-16 investigation-era architecture was
        RE-OPENED when the README design spec was found to contain a
        ratified union-grid design (Architecture A) never surfaced during
        the investigation, then RE-DECIDED after a W3C evaluation round:
          - ARCHITECTURE A (union step-4 grid with scaled midpoint
            thresholds, README 3.11/3.13) is REJECTED: its geometry was an
            artefact of separated-field coordinates and does not survive
            the D4-D01 whole-frame contract (frame-DCT and field-DCT
            boundaries cease to share a footprint when woven), and the
            false-activation limit is proven in principle (a genuine
            compression seam and a harmless gradient can present
            IDENTICAL local pixels; no threshold separates identical
            observations). midpoint_threshold_scale is thereby ORPHANED
            as a mechanism; its parameter fate is decided at kernel scope.
          - ARCHITECTURE B2 (macroblock-anchored edge topology) is
            ADOPTED as PRIMARY CANDIDATE, superseding the region-phase
            form of D4-D04/D05: classify each 16x16 luma macroblock
            FRAME / FIELD / UNKNOWN; DERIVE the edge set - internal
            pitch-1 edge at mb_y+8 only for FRAME; the macroblock-row
            boundary at mb_y+16 typed by the (upper,lower) pair as one
            pitch-1 edge (FRAME/FRAME), two pitch-2 parity edges
            (FIELD/FIELD), or two pitch-2 mixed-boundary edges (the seam
            as an EXPLICIT EDGE TYPE, not a failure); vertical edges at
            x=8k are GEOMETRY-INVARIANT across all cases (within-row
            filtering; no parity split needed). UNKNOWN keeps D4-D07
            (filter nothing there, count it).
          - ARCHITECTURE D (detector-free conservative whole-frame union:
            pitch-2 at macroblock-row boundaries always; the single true
            pitch-1 internal candidate tested strictly, optionally with
            A's threshold-scaling idea) is ADOPTED as MANDATORY FALLBACK
            AND COMPARATOR.
          - Motion-based classification (C) remains rejected.
        SPAN COMPILATION (D4-D05) survives re-expressed: spans are now
        compiled FROM the macroblock/edge-topology map, still
        geometry-homogeneous, still the SIMD unit.

THE DECIDED ARCHITECTURE, one paragraph (v1.01): a per-frame,
deterministic pipeline. (1) CLASSIFY each Case (a) luma macroblock
FRAME/FIELD/UNKNOWN from decoded pixels (all other plane/mode
combinations are fixed by the declared mode and F4). (2) DERIVE EDGE
TOPOLOGY per D4-D12 and compile geometry-homogeneous spans. (3) FILTER
each span with the independently derived 8x8 boundary-vs-activity kernel
at the span's pitch. GATED on the expanded D4-Q14 discriminator
experiment (section 8): if B2's measured false-confident rate is
unacceptable on real material, Architecture D is the shipping
architecture and stage (1) collapses away.
```

# 8. [PENDING] D4-Q14 ground-truth experiment - THE GATE (results land here)

```text
RATIFIED INTENT (W3X, 2026-08-16; EXPANDED at the D4-D12 re-decision
into an ARCHITECTURE DISCRIMINATOR, not a B-validator): before any
kernel scope, on real PAL MPEG-2 material with per-macroblock dct_type
TRUTH extracted from the bitstream side:
  (i)   B2 leg: per-macroblock classification confusion matrices,
        confidence-margin distributions, UNKNOWN rates, and the
        FALSE-CONFIDENT rate (the dangerous cell), reported separately
        for FRAME/FRAME, FIELD/FIELD and MIXED macroblock-row
        boundaries;
  (ii)  A/D leg: at every candidate edge position, the distribution of
        the activation features for TRUE boundaries vs NON-boundaries,
        with ROC-style threshold sweeps - measuring the in-principle
        false-activation limit on real content;
  (iii) TRUTH CLASSES: macroblocks with no coded DCT (skipped /
        motion-only / NO_DCT) form their OWN truth class and are never
        forced into FRAME or FIELD - forcing them would poison the
        confusion matrices.
Decision rule: B2 ships only if its false-confident rate is acceptable;
otherwise D ships. UNKNOWN-rate statistics feed the D4-D07 revisit.
Ownership per D4-D11. Experiment plan: separate W3D document, next.
...
# Appendix C - The architecture re-decision of 2026-08-16 (full record)

```text
TRIGGER. While beginning the documentation consolidation sweep, W3D found
that README_Deblock4_Design_Spec (sections 3.11/3.13 and its settled-
decisions table) already contained a fully-worked, ratified Deblock4 grid
design - the union step-4 grid with scaled midpoint thresholds
("Architecture A") - which NEITHER GAIS, NOR W3C, NOR W3D surfaced during
the four-round investigation, because W3D never read it. W3X re-opened
the architecture; W3D issued a comparison brief recommending A subject to
transposition verification; W3C evaluated.

ARCHITECTURE A AS DESIGNED (for separated-field input): luma candidate
grid step-4 with position classes primary (y mod 8 == 0) and midpoint
(y mod 8 == 4); ONE activation test for both classes; midpoint uses
alpha/beta scaled by midpoint_threshold_scale via fixed-point
S = round_half_up(scale*65536), scale_threshold(t,S) =
(i64(t)*i64(S)+32768)>>16, converted ONCE at creation into two immutable
threshold sets; tc0 and the one-sample addition deliberately UNSCALED
(an activated midpoint corrects at full strength); the midpoint test
reads current destination state in canonical schedule order. Rationale
as written: frame-DCT areas project to a four-field-row pitch after
field separation, so step-4 is the smallest single candidate set
containing every real luma boundary in mixed material.

THE W3C EVALUATION - three decisive findings, all W3D-accepted:

1. THE TRANSPOSITION FAILS (geometry). A's elegance was an artefact of
   separated-field coordinates: there, frame-DCT and field-DCT
   boundaries happened to share a same-field six-tap footprint at
   different frequencies. In the WOVEN frame they are DIFFERENT EDGE
   TYPES with DIFFERENT footprints: a frame-DCT boundary (e.g. between
   frame rows 7/8) is a pitch-1 consecutive-row edge; a field-DCT
   boundary is two pitch-2 same-field edges (14/16 even, 15/17 odd).
   Literally transposing A's midpoint tests the pitch-2 pairs 6/8 and
   7/9 - the WRONG EDGE with the WRONG TAPS - and the faithful
   both-phase union DOUBLE-WRITES rows adjacent to macroblock-row
   boundaries, breaking single-pass write discipline. A does not
   survive the whole-frame contract (D4-D01), which stands on
   independent grounds (chroma tearing, F4).

2. THE FALSE-ACTIVATION LIMIT IS IN-PRINCIPLE (not tunable away). For a
   step pattern A,A,A | A+d,A+d,A+d the side-activity terms are
   IDENTICALLY ZERO, so A's gate accepts whenever d < alpha_mid - and a
   genuine two-level compression seam presents pixels IDENTICAL to a
   harmless gradient step. No local threshold can separate identical
   observations with different causes. "Let the activation test sort it
   out" therefore has a proven floor of unavoidable error on flat
   content: blur real detail, or miss real seams, tunably traded but
   never eliminated.

3. TWO W3D PRO-A ARGUMENTS WERE WRONG (recorded per project practice):
   (i) "continuous uncertainty" - midpoint_threshold_scale moves
   alpha/beta continuously, but each edge still makes a HARD pass/fail
   and corrects at FULL strength (tc0 unscaled); that is a hard decision
   with a tunable threshold, not soft confidence. (ii) "avoids flicker"
   - it avoids phase-CLASSIFIER flicker and inherits THRESHOLD flicker
   near activation boundaries. Additionally recorded: the PROCESS
   failure that a 2800-line design spec classified as "fallback general
   guidance" was never swept during the investigation; specification
   documents are now in scope for any knowledge sweep (T1 resumes with
   the full README).

WHAT W3C CONTRIBUTED BEYOND REFUTATION (adopted into D4-D12):
   B2 - classify at the natural MPEG-2 unit (the 16x16 macroblock),
   then derive EDGE TOPOLOGY rather than assign regional "phase": the
   internal mb_y+8 edge exists only for FRAME; the mb_y+16 macroblock-
   row boundary is typed by the neighbour pair (FRAME/FRAME one pitch-1
   edge; FIELD/FIELD two pitch-2 parity edges; MIXED two pitch-2 edges
   - the seam as an explicit edge type, the useful MBAFF idea
   independently derived). Vertical edges at x=8k are geometry-invariant
   (within-row filtering), removing any need for parity-split vertical
   SIMD. Architecture D as the detector-free conservative fallback:
   pitch-2 at macroblock-row boundaries always, the single TRUE pitch-1
   internal candidate tested strictly (optionally with A's
   threshold-scaling idea, which survives THERE if anywhere).

OUTCOME (W3X-ratified 2026-08-16): A rejected; B2 primary; D mandatory
fallback and comparator; C stays rejected; D4-Q14 expanded into the
discriminator experiment (section 8); kernel scope only after its
results. README consolidation findings pending T1 resume, including two
already flagged by W3C: the audit-property model cannot express mixed
B2 geometry as a single per-frame string, and 4:2:2/4:4:4 chroma
FOLLOWS luma DCT (F4 is 4:2:0-specific), so those formats cannot
inherit the fixed-chroma simplification.
```
...
*Revision history*
```text
v1.02 (2026-08-16) Added section 0 CURRENT ARCHITECTURE POSITION as the
      read-first summary of the whole state: settled items, adopted
      candidates, rejections with reasons, the pending discriminator
      experiment, still-open kernel questions, and the postponed process
      work including the README status ruling. No decision changed.
v1.01 (2026-08-16) ARCHITECTURE RE-DECIDED (D4-D12; full record in new
      Appendix C): Architecture A (the README union-grid design,
      discovered mid-consolidation) evaluated and REJECTED on W3C's
      geometric transposition proof and in-principle false-activation
      limit; B2 (macroblock-anchored edge topology) adopted as primary;
      D (detector-free conservative union) adopted as mandatory fallback
      and comparator; section 7's architecture paragraph and section 8's
      D4-Q14 experiment rewritten accordingly (discriminator, three truth
      classes incl. NO_DCT, ROC sweeps for the A/D leg). Two W3D
      reasoning errors and one W3D process failure recorded. T1
      consolidation sweep remains PAUSED pending W3X direction.
