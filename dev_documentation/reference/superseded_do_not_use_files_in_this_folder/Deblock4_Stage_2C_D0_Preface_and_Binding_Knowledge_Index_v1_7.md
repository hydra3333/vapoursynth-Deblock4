...
K26 EXTERNAL-ORACLE EXECUTION PIN (G6 discipline applied to the
      ...
      A HolyWu layer-(b) comparison therefore records and controls:
        ...
        - BEHAVIOURAL SENTINELS: negative-delta input vectors run against
          that exact binary with observed outputs recorded (covering the
          class-(ii) UB region empirically);
...
K16 Creation-error message table v1_2 is the ratified source obligation:
      the v1_1 rows unchanged PLUS the two Stage 2C Classic additions
      (float refusal; backend-unavailable refusal) under the section-5
      narrow exception. No EXISTING string is altered. Deblock4Using / the using-echo
...
refusal and (b) the unimplemented-backend refusal carried by creation-
error table v1_2, (c) the Classic-only IMPLEMENTED-TIER AVAILABILITY cap
...
Revision: v1.7 (2026-08-03) W3C revised-package review F3a: section-5
