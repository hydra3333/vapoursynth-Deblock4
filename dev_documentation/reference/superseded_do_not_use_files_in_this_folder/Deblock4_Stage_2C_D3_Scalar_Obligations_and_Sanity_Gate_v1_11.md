...
- PROOF ROUTING (per W3C D3 review F8): kernel/math/geometry O-items ->
  Zig unit tests; public creation/error O-items (O-1b, O-1d, and the
  section-8 float refusals) -> vspipe/batch
...
"Classic: integer input must be between 8 and 16 bits" (table v1_6) -
NEVER the "input video metadata is invalid" row (that means malformed
metadata, a different condition). Proof: otherwise-valid FIXED 17-bit
AND 32-bit integer creation cases (invocation-level - Addendum B
N01c1/N01c2; constructed via query_video_format, the first unsupported
depth and the highest valid depth), PLUS an exhaustive direct
validation/guard test enumerating EVERY integer depth 17..32 to close
the internal range. Float (16- and 32-bit) is a SEPARATE refusal (D4
S1); this row is integer-only.
...
error table v1_6 - offset rows unchanged since v1_1, D0 K16). E.g.
strength=25, boundary_strength_offset=40
...
"Classic: float input is not supported" (table v1_6), proved by TWO
otherwise-valid constant-format float-input cases - YUV444PH (16-bit
float) AND YUV444PS (32-bit float), the exact Addendum-B N01a/N01b
formats - both producing the same exact row, so a storage-width-specific
bug cannot pass. When Classic float
...
Q1 (W3C) EXPLICIT FINAL CLAMPS - RESOLVED by D4 S2 (retain; W3X-
   ...
   STATUS: RESOLVED by D4 S2 (W3X-ratified); leaves the register on D4
   release. (The line above is the retained historical position; it is
   SUPERSEDED.)
...
# 10. Binding Knowledge Checklist (D0 v1_11)
...
K16    O-1b encodes the ratified Classic offset-REJECTION obligations
       (never clamp). Creation-error table v1_6 ADDS the THREE ratified
...
K22    float REFUSED in 2C (D4 S1); float obligations carried forward to
       the later bounded float step, not a 2C obligation. When taken up,
       f16 is STORAGE-never-COMPUTE and the compute-width semantics is
       pinned BEFORE the tolerance numbers are derived (Toolchain
       Findings v1_4 F10; D0 K22).
K24    obligations are kernel-agnostic: they bind outputs. The kernel
       AUTHORING requirement is D4 S7's ratified form (one canonical
       non-duplicated formula body, exact u8/u16 storage, clean scalar
       boundary, no speculative width/vector API), not an unspecified
       comptime shape.
K30    the new first-class modules these obligations judge obey the
       C-STY-10 discipline; the first-class textual audit is a delivery
       obligation, named in the 7d crosswalk with the ratified TWO-PART
       domain (new modules audited in full; existing edited modules on
       their 2C CHANGES only; accepted Stage 1C regression identifiers
       not renamed) - full contract in D0 v1_11 K30 / D4 section 9 K30.
K31    stride units explicit and checked even in scalar code
       (C-SIMD-03): byte row navigation with the unit named (no
       division; recommended default), or exactly ONE checked
       conversion PER PLANE stride/view construction naming both units;
       silent division forbidden. Proof is per model (D0 v1_11 K31):
       inspection (+ the assertion for the typed model) riding O-6e
       canaries and O-8c/O-8h.
...
Revision: v1.11 (2026-08-12) End-of-phase F10 propagation: the K22
checklist row gains the f16 storage-never-compute pointer (Toolchain
Findings v1_4 F10). No obligation, vector, matrix or gate content
changed.
Revision: v1.10 (2026-08-05) Checklist mirror of the focused re-review
resolutions (W3X-ratified 2026-08-05): section-10 heading -> D0 v1_11;
K30 row carries the two-part audit-contract domain; K31 row carries the
per-model proof and per-plane conversion scoping. NO obligation, vector,
matrix, gate or appendix content changed - no re-derivation or re-review
of values is required.
Revision: v1.9 (2026-08-05) Checklist mirror of the successor-W3C
pre-implementation review adoptions (W3X-ratified 2026-08-05, reissue
chosen at W3X Q9): section-10 heading -> D0 v1_10; K30/K31 rows added.
NO obligation, vector, matrix, gate or appendix content changed - no
re-derivation or re-review of values is required.
Revision: v1.8 (2026-08-03) W3C v1.5-package review F1: table pointers
updated to v1_6. v1.7 (2026-08-03) W3C v1.4-package review: F3 routing names
O-1b/O-1d/section-8 float refusals; float proof fixed to BOTH widths
(N01a/N01b formats); O-1d fixed 17 AND 32 cases plus the exhaustive
17..32 guard; F4 K24 checklist entry points at the ratified S7 form;
F6 Q1 status marked resolved-superseded and checklist heading updated to
D0 v1_8. v1.6 (2026-08-03) W3C updated-package: F1 added O-1d integer-
