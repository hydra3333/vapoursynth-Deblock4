...
**Status:** W3X-RATIFIED 2026-08-16, corrected at v1.3 on W3C blocking
finding M2-W3C-F2 (section 4a). The mandatory discovery round is COMPLETE and
ratified (section 6a). IMPLEMENTATION IS RELEASED against the same accepted
base; no re-discovery is required.
...
JOB B - RETIRE:
  ...
  MOD  tools/audit_stage_1c_s2_sweep.ps1  TWO edits:
       ...
       (ii) [W3D-M2-F2 RATIFIED] ADD the M2-deleted paths to the
            ...
            tests/stage_1c_classic_passthrough.vpy, and the retired
            DIRECTORY PATH tools/holywu_reference (NO trailing slash,
            and DO NOT enumerate its members - see section 4a).
            Retirement then becomes ENFORCED rather than merely
            performed, and accidental resurrection is impossible.
...
# 4a. S2 RETIRED-LIST ENTRIES: THE GENERICNESS RULE (v1.3 correction)

```text
CORRECTION ON W3C BLOCKING FINDING M2-W3C-F2, W3D-VERIFIED, W3X-RATIFIED
2026-08-16. The v1.2 instruction to add "the tools/holywu_reference/
members" to the S2 retired list would have BROKEN THE BUILD on first run.
W3D verified the mechanism in the committed script and tree:

  audit_stage_1c_s2_sweep.ps1 line 20:
      $names=$ret | % {[IO.Path]::GetFileName($_)}
  - the script derives BASENAMES from the retired list and then fails if
    ANY live file in its domain CONTAINS any of those basenames.
  line 8:  $roots=@('build.zig','build.zig.zon','src','tests','tools')
  - so build.zig.zon IS in the search domain, and its line 15 contains
    "README.md",  as a legitimate paths entry.

  Enumerating tools/holywu_reference/README.md would therefore derive the
  GENERIC basename README.md and fail S2 permanently against an unrelated
  and entirely correct reference.

RATIFIED ENTRY (W3C's proposal, adopted verbatim):
  add  'tools/holywu_reference'    (directory path, no trailing slash)
Both halves of the tripwire still work:
  - Test-Path on that entry fails if the directory, or anything at its
    original path, reappears;
  - the derived basename is the DISTINCTIVE 'holywu_reference', which
    still catches surviving live references to the retired vocabulary,
    without blacklisting README.md.
No S2 algorithm change; only the list contents change; diff confinement
preserved.

THE GENERAL RULE, now binding on this and every future retirement
(W3X-ratified 2026-08-16 - the specific fix above is an instance of it):

  BEFORE adding any entry to the S2 retired list, check the basename it
  derives. If that basename is GENERIC - README.md, build.zig, main.zig,
  LICENSE, config.json and their like - DO NOT add the member path. Add
  the distinctive parent directory path instead, or if there is no
  distinctive parent, report it rather than adding it. A retirement
  tripwire that blacklists a generic filename converts an enforcement
  mechanism into a permanent build failure.

W3D CHECKED THE REST OF THE v1.2 LIST AGAINST THIS RULE. All remaining
entries derive distinctive basenames and are SAFE as written:
  build_1C_v1.bat, build_2C_v1.bat, build_4C_v1.bat,
  run_stage_4c_differential.cmd, stage_4c_scalar_v2_diff.py,
  stage_1c_classic_passthrough.vpy.
Only the HolyWu members were generic, and only because W3D wrote
"members" instead of naming the directory path.

PROVENANCE NOTE: the retired-list tripwire was W3D's own recommendation
(W3D-M2-F2), added so retirement would be enforced rather than merely
recorded. It was a sound idea carrying a defect that would have failed
the first run, and W3C found it BEFORE any file changed, with the correct
remedy attached. That is the discovery discipline working as designed.
```
...
*Revision history*
```text
v1.3 (2026-08-16) Correction on W3C blocking finding M2-W3C-F2: the v1.2
     instruction to add "the tools/holywu_reference/ members" to the S2
     retired list would have derived the GENERIC basename README.md and
     failed S2 permanently against build.zig.zon line 15 - W3D verified
     the mechanism, the domain and the collision in the committed tree.
     The ratified entry is the directory path tools/holywu_reference (no
     trailing slash), preserving both halves of the tripwire. New section
     4a adds the general GENERICNESS RULE binding on every future
     retirement, and records that W3D re-checked the remaining six
     entries as safe. No design change; no weakening of enforcement;
     implementation released against the same base.
