...
**Date:** 2026-08-03
**Author:** W3D (designer)
**Status:** DRAFT for mandatory W3C pre-implementation review (with D3 v1_6).
NOT released. W3X alone releases, accepts and commits.
**Built from:** D0 v1_8, D1 (pinned r9 snapshot + provenance v1.3),
D2 v1_4, D3 v1_6, and Addendum A v1.1 (K26 sentinel fixtures) + Addendum
B v1.1 (mandatory differential corpus) which are READ-TOGETHER PARTS of
...
IN SCOPE
  ...
  5. Tests asserting EVERY D3 v1_6 obligation on its routed proof
...
OUT OF SCOPE (do not touch, do not "improve")
  ...
  - Parameter validation, creation-error strings and tier RESOLUTION are
    unchanged EXCEPT the five narrowly authorised items (D0 v1_8 section
...
S1  FLOAT - RATIFIED BY W3X 2026-08-03 (resolves register T-2). Stage
    ...
    Creation-error table v1_4 (W3X-ratified and controlling) carries this
    ...
    between 8 and 16 bits"; N01c) - VapourSynth permits integer depths to
...
S4  ORACLE STATUS. Per the oracle-construction exception (charter G7;
    V&T 20.2; K9), this delivery is accepted against D3 v1_6's
...
A1b FORMAT AND STORAGE CONTRACT (W3C F2; K28). Explicit, not inferred:
      ...
      out-of-range : CONFIRMED API-reachable (K29: VapourSynth permits
                     ...
                     v1_4), NEVER the metadata-invalid row. Proof N01c.
...
Implement exactly what D2 v1_4 documents; D3 v1_6 judges it. The
...
D3 v1_6 in full: O-1, O-1b, O-1c (73 enumerated tuple cases + the
...
K16     creation strings unchanged EXCEPT the THREE added Classic rows of
        error-table v1_4 (S1 float refusal; K29 integer-depth refusal; S5 backend-unavailable
...
K29     valid integer depths extend to 32; A1b/S1 refuse 17..32-bit
        explicitly with the ratified row (N01c), never as malformed
        metadata.
```
...
Review THIS scope together with D3 v1_6 and Addenda A v1.1 and B v1.1. Report numbered findings on:
