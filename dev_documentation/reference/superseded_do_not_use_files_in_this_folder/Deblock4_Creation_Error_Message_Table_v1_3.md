...
- The float row is required because Stage 1C creation has no sample-type
  gate: a 32-bit float clip is accepted today and would otherwise reach an
  integer-only pixel path. Float remains a first-class future path with its
...
- PROOF STATUS: both rows are exercised by the Stage 2C proof surface -
  the float row via an otherwise-valid constant-format float-input
  creation case (no combined invalid conditions, avoiding an accidental
  validation-precedence obligation - combined-review F3), and the
  availability row via an explicit v2 (and v3) request on the 2C build;
