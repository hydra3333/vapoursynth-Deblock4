...
H6 (version-neutral: the highest committed D4; W3C revised-package F7): formats including subsampled chroma,
...
N01c1 GRAY 17-bit integer            -> "Classic: integer input must be
                                        between 8 and 16 bits"
N01c2 GRAY 32-bit integer            -> "Classic: integer input must be
                                        between 8 and 16 bits"
     (K29: valid 17..32-bit integer is API-reachable and is NOT
      malformed metadata; distinct row from float and metadata-invalid.
      EXACT construction - never a preset alias:
        fmt17 = core.query_video_format(vs.GRAY, vs.INTEGER, 17, 0, 0)
        fmt32 = core.query_video_format(vs.GRAY, vs.INTEGER, 32, 0, 0)
      then otherwise-valid constant-format BlankClip-style clips from
      those format IDs. These two end-to-end cases prove the first
      unsupported depth and the highest valid depth on the REAL creation
      path; the INTERNAL range 18..31 is closed by the exhaustive direct
      validation/guard test required by D3 O-1d - both are mandatory.)
...
N03  backend="x86_64_v3_with_avx2"
     ...
     (Debug build only)              -> "Classic: requested backend is
                                        ...
                                        availability - D4 S5)
N04  the EXACT C01 source clip with
     backend="auto"                  -> creation SUCCEEDS; always-on line
                                        + Deblock4Tier report v1 with the
                                        implementation-capped reason.
     (Fixed case, not coder-selected: this addendum is an exact matrix.)
```
...
Revision: v1.2 (2026-08-03) W3C v1.4-package review: F2 split N01c into
exact N01c1 (17-bit) + N01c2 (32-bit) with explicit query_video_format
construction and noted the mandatory exhaustive 17..32 guard test (D3
O-1d); N04 pinned to the exact C01-source case; purpose cites D4 H6
version-neutrally (F6.2). v1.1 (2026-08-03) W3C updated-package: added N01a/N01b (both
