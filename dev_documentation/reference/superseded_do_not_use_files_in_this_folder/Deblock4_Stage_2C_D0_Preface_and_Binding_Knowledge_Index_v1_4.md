...
K26 EXTERNAL-ORACLE EXECUTION PIN (G6 discipline applied to the
      reference; from W3C D2 finding F12). Byte-pinning the HolyWu SOURCE
      (D1) does NOT pin the RESULT: signed right-shift on negatives,
      negative signed left-shift, and float contraction/reassociation are
      implicit-toolchain behaviours, exactly the class charter G6 forbids
      relying on for a correctness property. A HolyWu layer-(b) comparison
      must therefore record and control, as applicable:
        ...
        - FORCED C/scalar path: opt=1, OR a proven no-DEBLOCK_X86 build
          ...
          x86, so absence of opt does NOT select the scalar oracle);
        ...
        - the exact built DLL/executable hash when a binary is the
          practical oracle.
      Applies: D2 (state which expected results depend on execution
      semantics, not just source bytes); D3 (integer vectors that assume
      floor-shift inherit this dependency until the pin exists); D4 (the
      harness pins opt=1 and the reference build, and asserts its own
      in-domain inputs); every external differential run.
```
...
- Classic OUT-OF-RANGE OFFSETS are creation ERRORS (README 3.14 + ratified
  ...
  unless T-2 opens float) and asserts its inputs are in-domain.
...
Revision: v1.4 (2026-08-02) added K26 (external-oracle execution pin, G6
