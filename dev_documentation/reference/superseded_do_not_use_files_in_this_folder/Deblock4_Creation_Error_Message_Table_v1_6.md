...
Stage 1C v1_13 source (Classic 22/22, Deblock4 31/31). Stage 2C adds
THREE Classic rows (section 2a) under the narrow D0 section-5 exception:
the float refusal and backend-unavailable refusal (added v1.2) and the
integer-depth refusal (added v1.4, per K29). No existing row is altered,
removed or reworded. This v1.5 removed residual two-row wording; v1.6
states the current three-part integer-depth proof (W3C v1.5-package
review F2).
...
v1.1 was W3X-ratified for Stage 1C; v1.2/v1.3/v1.4 add and document the
THREE Stage 2C Classic rows (float; integer-depth per K29; backend
availability) under the D0 section-5 narrow exception. (The
...
## 2a. Classic additions for Stage 2C (v1.2 + v1.4; W3X-ratified 2026-08-03)

These THREE rows exist because Stage 2C makes Classic PRODUCE PIXELS. Until
2C the filter passed frames through, so accepting a float clip, a valid
17..32-bit integer clip, or an unimplemented backend token was harmless;
once it filters, silently mistreating any of them would be a false claim
to the user. All three are refusals, not behaviour changes to any
accepted path.
...
- PROOF STATUS: all three rows are exercised by the Stage 2C proof
  surface -
  ...
  based, not storage-width based (F2); the integer-depth row via the
  three-part proof: an otherwise-valid FIXED 17-bit creation case
  (N01c1), an otherwise-valid FIXED 32-bit creation case (N01c2), AND
