...
across chats. Items are marked INVARIANT (lifted into charter G3/G7/G8/G9/G10 and F2)
or SPECIFICATION (lifted into the README design spec). The charter and README are
the controlling documents and prevail for any conflict; consult them for their
current versions.
...
specified algorithm and agree within a measured tolerance, with legitimate
backend evaluation differences (including any future explicitly approved fused
operation) treated as a feature rather than a defect; under .strict FMA is not
relied upon and ordinary a*b+c is not result-changing contracted. CPU support
...
stderr ONCE PER FILTER-INSTANCE creation (not per frame, not "on every run").
...
scalar oracle within a tolerance that is deterministic per backend. Legitimate
backend evaluation differences (including any future EXPLICITLY APPROVED fused
operation) are acceptable and are treated as a feature, not a defect. FMA is
included in the v3 target but is not relied upon under the current algorithm:
ordinary a*b+c is not result-changing contracted under .strict and no @mulAdd is
currently required, so FMA is not the expected cause of cross-backend
differences and 1B.2 must not expect FMA emission.
...
**f16 storage-never-compute (Toolchain Findings v1_4 F10; W3X-ratified 2026-08-05).**
VapourSynth float is 16-bit half and 32-bit. Zig/LLVM f16 ARITHMETIC is pathologically
slow (~100x; ziglang/zig #19550), so f16 is a STORAGE width, NEVER a COMPUTE width:
widen to f32 on load, compute in f32 under .strict (3.6), narrow on store. The 
compute-width SEMANTICS (call it B: f32-compute with a single f32->f16 rounding at
store, as distinct from true-binary16 semantics A where every intermediate rounds to
f16) must be RATIFIED AS THE ALGORITHM and PINNED BEFORE these tolerance numbers are
derived - the tolerance is semantics-dependent, so numbers derived against the wrong
compute width would be invalid. .optimized/fast-math is INCOMPATIBLE with the 3.4
non-finite policy (it leaves NaN/Inf undefined and discards signed zero) and is rejected
on that ground as well as G8. Widen at register/row granularity, never whole-plane; the
per-edge widen/compute/narrow default follows from the sequential edge dependency. F16C
conversions are an x86-64-v3 feature and stay confined to v3 objects. Full guidance and
the F16-1..F16-8 investigation checklist: Toolchain Findings v1_4 F10.
...
```text
x86_64_v1   baseline AMD64 (SSE, SSE2, plus CMOV/CX8/FPU/FXSR/MMX/OSFXSR/SCE).
            Every 64-bit CPU. -> generic/scalar.
x86_64_v2   adds SSE3, SSSE3, SSE4.1, SSE4.2, POPCNT, CMPXCHG16B, LAHF-SAHF.
            -> SSE4.1-class.
x86_64_v3   adds AVX, AVX2, BMI1, BMI2, FMA, F16C, LZCNT, MOVBE, OSXSAVE.
            -> AVX2 backend.
(x86_64_v4  adds AVX-512 subsets. OUT OF SCOPE - no W3X hardware to verify it.)

This is a READING AID consistent with section 11.2, NOT a detection source:
OSXSAVE is a v3 level MEMBER; the runtime AVX/YMM guard additionally executes
XGETBV and checks XCR0 XMM+YMM state. Detection must derive from ONE authoritative
mechanism (section 4.6), never from this paragraph.
```
...
AVX2 but missing any other v3 feature (e.g. BMI2) is NOT v3; dispatch selects the
highest lower level it fully satisfies (normally v2, otherwise v1). The
...
## 4.4 FMA in v3: included but not relied upon under .strict (DECISION)

v3 includes FMA and it is not excluded. FMA is included in the v3 target but is
not relied upon: under .strict (section 3.6) ordinary a*b+c must retain non-fused
semantics, no @mulAdd is currently required, and Stage 1B.2 must not expect FMA
emission. A future MEASURED decision could explicitly select @mulAdd. "Do not
subtract FMA" and "require FMA" are separate decisions; only the former is made.
...
frame properties, the stderr emission, test selectors, and
unsupported-backend error text. Prose and user docs add a human gloss
("AVX2-class") but the token itself is authoritative.

## 4.6 Detection implementation (RESOLVED at Stage 1B.3; scoped v1.3, charter G3 v1.18)

RESOLUTION (2026-07-30): the mechanism is now ratified and no longer open. Compile
targets use Zig's named CPU models; runtime detection reads real CPUID/XGETBV
(SDM-located, the verified 25-bit Set-A table plus the Set-B XGETBV/XCR0
(XCR0 & 0x6)==0x6 check), whole-level v3->v2->v1; a comptime membership assertion
proves the detection sets equal the named-model differences through an explicit
name map and W3X-approved exclusion list, @compileError on mismatch (charter G3,
v1.18). Detection is proven v1-only (charter G2). The investigation notes below
are retained as the reasoning that led there; they are no longer an open
question. The "prefer std.Target at runtime" phrasing below was the early
preference; the ratified answer is the comptime cross-check against std.Target
models, with real CPUID at runtime (std.Target cannot see the end-user CPU).
...
Note on Zig's built-in OSXSAVE handling (do NOT rely on it for OUR dispatch).
Zig's native-target detection (detectNativeFeatures in lib/std/zig/system/
x86.zig) DOES compute a has_avx_save flag (XSAVE and OSXSAVE both set) and gates
AVX/AVX2 on it. But that is COMPILE-TIME detection of the BUILD host under
-mcpu=native, which this project does NOT use: we compile fixed named-level
targets and dispatch at PLUGIN LOAD on the END USER's CPU, which Zig's
compile-time native path cannot see. Moreover Zig's native detection is not
infallible - ziglang/zig issue 23385 shows a case where model-based assumption
(detected Alder Lake -> assumed AVX2) overrode the actual CPUID bits via
populateDependencies() and emitted a faulting VMOVD in a VM that had AVX
disabled. Lesson for us: our RUNTIME whole-level guard must (a) verify every
feature bit of the level directly, never infer a level from a CPU model or a
headline instruction, and (b) perform its own OSXSAVE plus XGETBV/XCR0 XMM+YMM
check before permitting the v3 (or any YMM) path. OSXSAVE is a v3 level member
AND a runtime precondition; both are checked. This is exactly what whole-level
dispatch (4.3) already mandates - the Zig note does not relax it.
...
At each filter-instance creation, the plugin version, filter name, requested
backend token, selected tier, and any fallback reason are emitted to stderr ONCE
(ffmpeg-style; not per frame). "Always-on" means "not behind a debug switch",
NOT "on every run" or "per frame". This makes "which filter selected which tier,
and why" immediately visible for support and performance triage (e.g. a user
reporting slowness -> confirm which tier was selected and why a higher one was
not).
...
choice, not a change of objective. (README sections 1.0, 3.15, 20; the controlling README is stated in this record's status header.)
...
- Stage 1B.2: COMPLETE - each object confirmed within its named level.
- Stage 1B.3: ACTIVE - whole-level detection implemented per the now-RESOLVED
  section 4.6 (scoped v1.3; first W3C delivery under review). Tolerance numbers
  remain deferred until real kernels exist (a Stage 2C/2D+ item).
```
...
# 12. Revision history
v1.11 (2026-08-12) End-of-phase F10 propagation: section 3.8 gains the
f16 storage-never-compute rule and the pin-compute-width-before-tolerance
ordering (Toolchain Findings v1_4 F10). Methodology and all decisions
otherwise unchanged.


```text
v1.9  De-versioned the lift-header charter/README cite to a document reference
      (versions are named once in the status header), so the lift-header no
      longer drifts on charter/README revisions. No decision change.
v1.8  Mechanical review correction: the live "current at README v1.7" pointer in
      section 8.1 de-versioned to a section citation (the controlling README
      version is stated once in the status header), removing the only stale
      current pointer.
v1.7  Post-re-audit cleanup (incl. final package review): section 4.4 FMA
      wording unified (included-but-not-relied-upon, not "present-but-unused");
      lift-header current pointers advanced to charter v1.15 / README v1.8; section 7.1 duplicated/incomplete sentence (a
      leftover from the H5 edit) repaired; a residual FMA-as-accuracy-gain
      phrase in section 3.1 reworded to the not-relied-upon form.
v1.6  Section 4.6 augmented with the Zig native-detection nuance: Zig's
      detectNativeFeatures does gate AVX/AVX2 on has_avx_save (XSAVE+OSXSAVE),
      but that is compile-time BUILD-HOST detection under -mcpu=native, which
      this project does not use; it is also fallible (ziglang/zig #23385, a
      model-assumption overriding CPUID in a VM). Our runtime whole-level guard
      must verify every level feature bit directly and do its own OSXSAVE +
      XGETBV/XCR0 check; it must not infer a level from a CPU model. Sharpens
      H4/H3 without changing the tiering model.
v1.5  Independent re-audit corrections: section 4.5 broken sentence reunited
      (H6); section 4.1 v2/v1 feature lists completed with CMPXCHG16B/LAHF-SAHF
      and OSXSAVE, marked a reading aid consistent with 11.2 (H3.1); stderr
      cadence changed from "on every run" to once-per-filter-instance (H5);
      fallback wording "is v2" -> "highest lower level fully satisfied" (M3);
      FMA reworded from an accuracy example to a not-relied-upon target member
      (M1); date to 2026-07-29 and the live README pointer de-versioned (M6).
      Added section 20 (the two scope-blocking rules: per-type differential
      acceptance + oracle-construction exception with a loose sanity gate),
      ratified by W3X, referenced by charter v1.14 and roadmap v1.7.
