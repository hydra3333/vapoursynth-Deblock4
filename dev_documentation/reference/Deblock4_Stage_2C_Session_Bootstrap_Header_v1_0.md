...
**Status:** DRAFT for W3X. Fields marked <W3X: ...> are completed by W3X at
issuance; everything else is filled from the released Stage 2C authority set.
...
Starting commit:
    ...
    is the exact ATTACHED SOURCE TREE, not a commit hash. The attached tree
    IS the base; treat it as exact; no branch/commit commands are needed.
    Attached base SOURCE package: src.zip
    (the Stage 1C + rider 1C.1 accepted tree: build_1C_v1.bat present,
    effective_invocation_text.zig present, identity 0.1.0-dev+1C)

Active scope:
    W3D-2C-D4 v1_7 (RELEASED) - construct the deblock4.Classic ReleaseSafe
    ...
    that validates it (Deblock4_Scope_Stage_2C_D4_Classic_Scalar_Oracle_v1_7.md,
...
Permitted changed files:
    ...
    the manifest; only the stated change class per file):
        ...
        src/backend_tier_selection.zig      (ONLY the S5 implementation-
                                             availability cap)
...
Forbidden changed files:
    All others. Explicitly (D4 section 1 OUT OF SCOPE):
        ...
        capability DETECTION (src/cpu_capability_detection.zig) and the
            1B.3 guard machinery
...
Inputs supplied:
    ...
    Deblock4_Stage_2C_Issuance_Bundle_Manifest_v1_0.md: this header; the
    charter; the controlling README; D4 v1_7 with its full read-together
    authority set (D3 v1_8, Addenda A/B v1_2, D0 v1_9, D2 v1_6,
...
Required validation (W3X runs; W3C does not claim execution - C-DELIV-07):
    ...
    +2C identity, THEN the additive 2C gates: the full D3 v1_8 obligation
...
Expected result:
    ...
    matrix green end to end with OUTER_BATCH_EXIT_CODE=0; every D3 v1_8
...
Implementation acceptance for this scope:
    ...
    AUTHORED scalar obligations of D3 v1_8 IN FULL (per D4 section 5,
...
The session package contains:
    ...
    4. the active scope D4 v1_7 and its read-together authority set;
    5. all files the scope touches (the attached source tree);
    6. the scope-specific test-vector and harness contracts (D3 v1_8,
