...
D2 Appendix A, AND that the resolved (alpha,beta,c0,c1) tuple over the
FULL legal index domain (every strength 0..60 with zero offsets, plus
the four offset-extreme corners at strengths 0, 25, 60) matches the
derivation model. Six sampled tuples cannot bind unused entries.
...
B2  (110,110,110,100,100,100) -> (109,107,103,101)
      ...
      EXECUTION-PIN NOTE (D0 v1_5 K26): the -4 assumes floor semantics
      for the DIVISION step; the Zig oracle computes the delta core with
      WELL-DEFINED arithmetic (i32 multiply by 4 - never a negative
      left shift, which is C++ UB in the reference source's language
      modes) and signed >> (floor), never @divTrunc. Externally, B2/B4/B5
      are the designated NEGATIVE-DELTA BEHAVIOURAL SENTINELS: their
      outputs must be RECORDED from the exact SHA-256-hashed reference
      binary (opt=1) before they count as reproduced layer-(b) facts;
      any rebuilt reference binary requires fresh hash AND sentinel
      revalidation (K26 rebuild rule).
...
b  PLANE SELECTION: an unprocessed plane is a COPY path - byte-identical
   for every ACCEPTED format (float conditional on T-2; V&T 20.1
...
d  For all three: no read or write outside the logical plane (O-6
   canaries apply), and no resize/crop node appears in the graph.
```
...
```text
Q1 (W3C) EXPLICIT FINAL CLAMPS. Must the ReleaseSafe scalar kernel retain
...
K5/G9  vectors are implementation-independent discriminators (B2, B5,
       B7, B8).
K26    (updated to D0 v1_5) B2/B4/B5 are the designated negative-delta
       behavioural sentinels: mandatory reference-binary SHA-256,
...
eligibility tests are vacuous there), and produces the O-7 a/b/c
expected outputs above.

---

Revision: v1.3 (2026-08-03) applied W3C D3 v1.2 follow-up: F1 replaced
