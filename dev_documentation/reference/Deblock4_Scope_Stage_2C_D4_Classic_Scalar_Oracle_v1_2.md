...
H4  IN-DOMAIN ASSERTION (S3) - INSPECT THE ACTUAL RUN INPUTS, never
    ...
    comparison): missing opt; wrong DLL hash; non-mod-8 geometry; float
    input; out-of-range offset; unsupported integer bit depth.
H5  COMPARISON AND DECISIVE GATE (W3C F8). Same source clip through
    deblock4.Classic and through the reference plugin (opt=1), byte-
    compared per plane per frame. Gate semantics, explicit:
...
H6  MANDATORY CORPUS - ENUMERATED, not "several" (W3C F8):
      ...
      frames     : fixed synthetic definitions with stated dimensions,
                   frame count, and a deterministic seed where generated.
...
```text
WRAPPER MODEL (ratified): build_2C_v1.bat FIRST invokes the EXACT
accepted build_1C_v1.bat as an immutable prerequisite, verified by hash;
it FAILS on any 1C failure; it then runs ONLY the additive 2C gates.
build_1C_v1.bat is NOT modified, renamed, or copied - it is an immutable
hash-checked dependency in the manifest, not a staged delivery file.
```
...
Revision: v1.2 (2026-08-03) resolved W3C combined-review F1-F9 and
