...
| Zig/C ownership and lifetime policy | Recorded in charter (current v1.16) |
| Numeric and SIMD helper policy | Recorded in charter (current v1.16) |
| External-source provenance policy | Recorded in charter (current v1.16) |
...
Stage 1 - Zig project / build / dispatch / tiering scaffold and spikes (SHARED)
    ...
    PASS  Stage 1B.2 within-level confirmation and assembly inspection (each
          ...
          and committed
    ACTIVE 1B.3 runtime capability guard: CPUID/XGETBV detection over the
          ...
          force-down seam (G10 pattern). Scoped v1.3; a first W3C delivery is
          under designer review. No dispatch wiring and no VS entry point yet.
    OPEN  filter-creation stage: VS entry point, scaffolding sweep, dispatch
          wiring that CONSUMES the 1B.3 records to select a backend
...
## Stage 1B.3 - runtime capability guard (ACTIVE; scoped v1.3, delivery in review)
...
debug options Debug-only; force-DOWN-only). It does NOT wire dispatch to call a
backend and does NOT create the VapourSynth entry point. A first W3C delivery
exists and is under designer review. See
...
expect FMA emission. Stage 1B.2 PRODUCED the whole-level feature requirements; it did not
implement or check a runtime guard - that guard is the Stage 1B.3 artifact now
in progress.
...
111_New_Chat_Introduction_for_Coder_v1_9.md
```
