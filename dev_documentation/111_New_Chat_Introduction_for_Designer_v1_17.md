...
Current position (2026-08-02, end of session; verify against repo):
    ...
    short-boundary W3D deliverables with a W3X checkpoint after each:
      ...
      D3 Independent scalar obligations + loose whole-image sanity gate:
         DRAFTED and revised, latest
         Deblock4_Stage_2C_D3_Scalar_Obligations_and_Sanity_Gate_v1_1.md
         (v1_1 applies F9: in-range V5 + new O-1b Classic-REJECTS-out-of-
         range obligation; F2: K26 execution-pin note on floor-shift
         vectors). Hand-derived vectors cross-checked by a transparent
         reference model (Appendix B; NOT implementation source). Integer
         obligations mandatory; float conditional on T-2. Pins WP-1
         floor-vs-truncate (B2), WP-6 boundary-equality (A1/A3), schedule
         ORDER-sensitivity (O-4). AWAITING W3C review together with D4.
...
Immediate designer action:
    D0 v1_5, D1 provenance v1.1, D2 v1_3, D3 v1_1 are the current
    reference set (cite highest versions). BOTH W3C review rounds are
    RESOLVED (round 1 F1-F12; revision-review F1-F7). NEXT: author the D4 Stage 2C coder scope from D2 v1_2 + D3 v1_1,
