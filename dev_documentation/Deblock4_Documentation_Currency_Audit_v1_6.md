...
THIS BANNER IS NOT AN ADJUDICATION AND NOT PERMISSION TO SKIP THE DOCUMENT.
It is still in the T1 sweep POPULATION and must be swept like any other live
document. The incident that caused T1 was a designer skipping a document
because an index called it general guidance - a classification believed
instead of checked. A banner saying 'stale' is exactly that shape of
classification, so it says explicitly: swept, not skipped.

THE T1 TASK LIST IS Deblock4_T1_Resume_Brief SECTION 0a (highest committed
version), not this document and not any state summary elsewhere.
