...
them. On any conflict, the scope and charter govern. NOTE: where a party
proposes changes to criteria that will be applied to its own work, the change
must be marked with who proposed and who verified it (proposer and verifier
must be different parties), never silently absorbed.
...
Phase 3a review set (charter 2.3a version group):
  ...
  3. Deblock4_Stage_1C_Phase_3a_Designer_Briefing_v1_2.md   (THIS - accumulated 3a clarifications)
```
...
VERIFY: the create signature matches VSPublicFunction exactly. The accepted
Phase 2 parsing, validation, tier-selection, allocation, ownership-transfer and
filter-construction logic remains unchanged. The authorised Phase 3a changes
around that logic are narrowly bounded: (a) bridge the [*c]/optional ABI
parameters to validated idiomatic locals at the top, and (b) add the required
debug-only lifecycle creation-enter / successful-creation-exit trace calls. If
computational or ownership logic changes under cover of the signature or trace
instrumentation, that is a flag. "Body unchanged" therefore means the existing
validation/build logic is preserved, NOT that the complete function must be
byte-identical after its required Phase 3a trace instrumentation.
...
- No restructuring of the permanent switch (2.2). Existing Phase 1/2 files
  change only at the expressly authorised Phase 3a seams: router branch targets
  are filled without altering the switch structure; creation callbacks receive
  the exact C-ABI boundary/rebinding plus lifecycle trace instrumentation while
  preserving their accepted validation/build/ownership logic; and the settled
  config/bridge surfaces are extended only as required by plugin registration,
  frame copying, frame-property writes and the third G10 trace option.
```
...
*Revision history*
```text
v1.2 (2026-08-01) (W3C-proposed, W3D-verified 2026-08-01, W3X-ratified).
     Second-pass reconciliation against every designer instruction
     given during the actual Phase 3a deliverable formulation. No missing
     instruction was found, but corrected two over-broad statements that could
     mislead a new reviewer: "create body unchanged" now explicitly preserves
     the accepted parsing/validation/selection/allocation/ownership logic while
     permitting the required ABI-local rebinding and lifecycle trace calls; and
     the 3a boundary now lists the other expressly authorised edits to accepted
     Phase 1/2 files (router branch filling without switch restructuring and
     narrow config/bridge extensions). Updated this briefing's filename in the
     review set. No design or scope change.
