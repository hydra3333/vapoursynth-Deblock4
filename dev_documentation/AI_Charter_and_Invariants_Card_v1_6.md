...
**Version:** 1.6
**Date:** 2026-07-25
**Status:** Ratified charter baseline. Part 1 is unchanged.
...
Charter:
    filename          Deblock4_AI_Charter_and_Invariants_Card_v1.6.md
...
C-SIMD-06  TARGET-SPECIFIC CODE IS NOT EXECUTED BEFORE A CAPABILITY GUARD.

          SSE4.1- or AVX2-targeted code may be compiled and linked before the
          production capability detector exists. It must not be called merely
          because the build machine is expected or known informally to support
          it.

          Before any target-specific entry point is called, the same process
          must execute an explicit guard proving the complete feature contract
          required by that object. For AVX2 this includes the required CPU AVX
          and AVX2 features, OSXSAVE, and XCR0 XMM+YMM state. The guard must run
          before control can enter the target-specific function.

          During Stage 1 spikes, a test-local guard is acceptable before the
          production detector exists. On an unsupported or indeterminate host,
          the test reports SKIP and does not call the target-specific entry
          point. There is no manual, command-line, environment-variable, or
          "this machine is known to support it" bypass.

          Compiling or linking an object is not execution. Therefore an AVX2
          object may be present in the DLL before capability detection is
          complete, provided no unguarded static initialiser, registration
          path, import thunk, or test call can execute AVX2 code.
```
...
C-DELIV-06  DEBLOCK4 TOOL AND LANGUAGE ASSUMPTIONS ARE EXPLICIT.
            ...
            and document is US-ASCII and uses LF line endings unless an existing
            controlling file contract explicitly requires otherwise.
...
Ratified the charter baseline and added the controlling Deblock4 delivery
protocol for memoryless coder sessions and W3X application.

- Replaced the stale draft-review status with "Ratified charter baseline" and
  updated the bootstrap filename and internal version to v1.6.
- Added C-SIMD-06: target-specific objects may be compiled and linked before
  the production detector exists, but SSE4.1 or AVX2 entry points may execute
  only after an in-process capability guard proves their complete contract.
  Unsupported or indeterminate hosts SKIP; there is no manual bypass.
...
No invariant in Part 1 changed. This revision adds controlling delivery and
execution-safety rules outside the pinned card.
