...
EXPLICITLY APPROVED fused operation) are a FEATURE, not a defect. Under .strict
an ordinary a*b+c is not auto-fused, so FMA is present-but-unused in v3 unless
such a decision is later made - Stage 1B.2 must not expect FMA emission. Strict floating-point operation ordering is retained
