...
**Revision note:** v1.2 (after coder cold review) adds an explicit provisional
baseline-target contract forbidding native/override for the baseline units
(section 2A, closing a G2/G5 gap where standardTargetOptions defaults to native);
fixes the retention proof to a forced-undefined-symbol mechanism retaining a real
non-exported marker FUNCTION, removing the weaker data-only alternative (section
5.2); and pins structural inspection to the ReleaseFast artifacts. v1.1 had set
the real post-1A.1 commit and the structural (non-disassembly) G5 proof.
...
Toolchain gate: if W3X's dumpbin /EXPORTS shows that the chosen marker form
unexpectedly becomes a public DLL export, Stage 1B.1 STOPS and the retention
mechanism is revised rather than accepting the export. A target-specific export
is a G5 bypass and is never accepted.

If Zig 0.16's forceUndefinedSymbol (or its current equivalent) does not behave
as described on this toolchain, W3C states this in its orientation/manifest and
proposes the nearest equivalent that still retains code without a callable path,
rather than falling back to an export or a data-only marker.
...
The SSE4.1 and AVX2 objects are proved RETAINED in the link by the forced-symbol
mechanism (a successful DLL link with forceUndefinedSymbol on each target-specific
marker, corroborated by dumpbin /SYMBOLS on the installed objects), where each
marker is a real non-exported FUNCTION, and proved NON-REACHABLE:
