...
## 5.2 Charter C-DELIV-06 (tool and interop assumptions, the bridge)
...
## 5.3 Charter C-DELIV-02 and C-DELIV-03 (delivery format and anchoring)
...
## 5.4 Charter P-03 (nothing untested becomes normative)
...
From the repository root, on branch main at the reconciled working tree:
...
zig build vs-header-run -Doptimize=Debug
zig build vs-header-run -Doptimize=ReleaseSafe
zig build vs-header-run -Doptimize=ReleaseFast
...
- the bridge self-test returns 7 (a non-7 result fails the probe);
...
Under the R78 headers, Debug, ReleaseSafe, and ReleaseFast all build; the header
probe runs and reports API 4.2; the bridge self-test returns 7; and zig build
