...
```text
ONCE, AT THE START OF YOUR CHAT - THE REFERENCE CORPUS:
...
IF YOUR CHAT IS EVER REPLACED, THE CORPUS MUST BE SUPPLIED AGAIN. You have
no memory across chats. See 4.2.0 - do not proceed without it.

WITH EACH STEP:
  - that step's ledger;
  - the task register, IF it has been bumped since the corpus was
    supplied (it carries the decisions and method forward, so the newest
    version wins over the copy in the zip);
  - a statement of exactly which document and which sections that step
    adjudicates.

ANY STEP CONTAINING TIER-B ENTRIES ADDITIONALLY:
  - the current SOURCE tree. The dev_documentation zip does NOT contain
    source. See 4.2.3.
