...
response may propose narrow, argued additions):
    ...
    MOD  src/backend_tier_selection.zig (ONLY if wiring requires; the
                                        emission contract is byte-stable
                                        for existing lines)
```
...
S5C-3  PERFORMANCE IS MEASURED AND RECORDED, NEVER GATED (charter
       ...
       by evidence, closed by this stage's measurement). The delivery
       includes a repeatable benchmark step (same clip, same
       parameters, scalar vs v2 vs v3, wall-clock via the established
       vspipe form, three runs each, all numbers reported raw).
...
S5C-4  THE TWO TAIL CLASSES, RESTATED AT 256-BIT (charter section C,
       ...
       N/2, N/4, ... 2, then scalar 1) - at N=32 the chain is
       32,16,8,4,2,1 and is ALREADY IMPLEMENTED by the frozen
       recursive tail (filterHorizontalTail halves N until 1). NO
...
```text
P1  THE THIN UNIT IS A MIRROR. classic_backend_v3_avx2.zig follows
...
P2  BUILD WIRING IS A MIRROR. build.zig adds classic_v3_target
    ...
    test-classic-v3 step that runs the SAME vector unit test file
    under the v3 target - PLUS new unit tests (in the v3 unit or the
    test wiring, coder to propose placement) exercising the 5C widths
    and remainders, since the frozen body's existing tests sweep the
    4C widths. NOTE: new width-sweep tests must not require editing
    the frozen body file; propose the placement in the response (R5).
...
P3  DISPATCH ARM. classic_ar_all_frames_ready.zig: declare the two v3
    ...
    coder confirms no other site depends on that error path (R1).

P4  BENCHMARK STEP (S5C-3). Proposal: build_5C_v1.bat gains a final
    NON-GATING step timing vspipe over one fixed 720x576 u8 corpus
    clip at a fixed strength, three runs each for backend=v1/v2/v3,
    emitting raw wall-clock lines into the evidence log. No pass
    criterion; presence of the numbers is the obligation. Coder may
    propose a better repeatable form (R2).

P5  TAILS: NOTHING NEW. The frozen recursive descent already
    implements 32,16,8,4,2,1. The 5C work is PROOF at the new width,
    not mechanism: the per-width tail behaviour table for N=32/N=16
    (what executes at each remainder 1..N-1) is produced in the
    response (R4) and the corpus forces each class boundary.
```
...
R4  THE PER-WIDTH TAIL BEHAVIOUR TABLE for u8 N=32 and u16 N=16:
    ...
    which order, terminating at scalar; plus the corpus dimensions
...
5C surface:
  ...
  5C-T1  v3 unit tests, three modes: the vector body under the v3
         target at u8 N=32 / u16 N=16 against the scalar equivalents
         over all D3 fixtures plus exhaustive 8-bit single-edge
         sweeps and seeded randomised property tests (documented
         seed) at every 5C tail remainder (u8 1..31, u16 1..15).
  ...
  5C-T6  Benchmark record (NON-GATING, S5C-3): the P4 (or ratified
         alternative) timing step; raw numbers into the evidence log;
         no pass criterion.
...
4:2:0/4:2:2/4:4:4 and GRAY, plane subsets, and at least one case
where an eligible edge's segment is shorter than a full 256-bit
vector at BOTH orientations. The response R4 table and this corpus
must cross-agree before release to implementation.
```
