...
One consequence worth stating plainly: the usual acceptance gates do not apply
here, and neither does the rule about never claiming PASS - because there is
nothing to pass. What replaces it is stated in section 6.
...
You have no memory between sessions, so each one is self-contained. The FIRST
session carries more; later sessions carry only what changes.

```text
FIRST SESSION (with step T1S00):
  - this scope;
  - the task register (Deblock4_Standing_Task_Register_T_Series, v1_3 or
    later) - the work queue, the decisions behind this task, and the four
    pre-registered items in section 7 below;
  - the scope manifest itself;
  - the outgoing designer's evidence set (the zip named
    T1_Evidence_Old_Designer_3_Answers_to_Designer_4_Questions_files) -
    four documents from the previous designer. Two are RECORD and can be
    relied on. Two contain JUDGEMENT and are positions to test, not
    findings. Each says which it is.

LATER SESSIONS:
  - the task register (always - it is the document that carries the
    decisions and the method forward);
  - that step's ledger.
...
TIER A - FULL REVIEW. Every entry marked CONFLICTING; every entry marked
    SUPERSEDED where the plan is to DELETE rather than replace with a
...
- Do not touch source, do not build, do not produce a delivery package.
