...
**Date:** 2026-07-31
**Author:** W3D
**Status:** RATIFIED. All decisions (D-1..D-7 as amended), all coder-review
findings (R1-R6), all module names (N1-N8), and all README gaps (P1-P5) are
settled by W3X. This document supersedes scope v1.1 and the W3C Section 9
proposal round; the coder builds against THIS version.
**Against:** charter v1.19; README design spec v1.2; Project Status v1_14;
Forward Roadmap v1_12; W3C proposal v1_1; W3D review v1.1 of that proposal.
**Starting point:** the accepted Stage 1B.3 commit on branch main. W3X records
the exact HEAD SHA in the session bootstrap header before coding begins.
...
src/deblock4_instance_creation.zig
    ...
    and backend_tier_selection (tier); clip-dependent checks (planes bound);
    builds the immutable instance record; createVideoFilter(fmParallel) with
    THIS filter's router callbacks; settled error messages on refusal.
...
src/deblock4_ar_all_frames_ready.zig
    This filter's per-frame work: obtain source, writable copy, property
    writes (all via common_frame_mechanics / common_frame_properties), then
    the TIER switch over the frozen BackendSelection. In 1C all tier branches
    call the pass-through placeholder.

== Shared, filter-neutral ==
src/common_instance_data_structure.zig
    The instance record TYPE (definition, not storage): source node + video
    info, atomic instance id, filter_kind (diagnostics only),
    BackendSelection, validated parameter config. May carry its own
    init/deinit lifecycle helpers (node release stays single-homed with the
    node holder). Immutable after creation; each instance gets its own copy.
...
src/common_frame_properties.zig
    Writes the settled audit properties (section 7).
...
ar_* handlers      -> common_frame_mechanics, common_frame_properties
...
D-3  Pass-through placeholder: pixel pass-through with zero algorithmic
     plane construction. The output frame is produced via the standard API4
     copyFrame idiom so properties can be written (metadata-level; plane
     data untouched and bit-identical -> gate E1 checksum equality holds).
     References released/transferred on every path including errors.
...
P2  Custom luma/chroma steps (luma_step_x/y, chroma_step_x/y): integers,
    REQUIRE >= 1 (zero/negative refused). Upper bound: the relevant frame
    dimension at creation if cheaply checkable there; otherwise a settled
    constant the coder proposes at delivery for W3X ratification.
...
# 7. Frame properties (D-4)

Written on every returned frame by common_frame_properties:
...
Deblock4Version  = the deblock4_version identity string ("0.1.0-dev+1C")
plus any further names README 13.5 settles (coder enumerates; no invented
names).
```
...
E2  Frame properties per filter: Deblock4Filter correct; Deblock4Tier equals
    the host's expected effective tier; Deblock4Version equals the version
    identity.
...
E5  Full-signature validation e2e per filter: one valid full-parameter
    ...
    empty-planes, and (Deblock4) step<1 invocations each refuse with the
    settled message.
E6  Lifecycle trace (Debug, enable_trace_lifecycle=on): captured e2e output
    ...
    in the trace banner.
V1  Version single-home: configPlugin-registered version, always-on summary
    line, selftest banner, and Deblock4Version property all equal
    deblock4_version's value (scripted equality, not eyeball).
...
12.5  Two minimal .vpy harness scripts (BlankClip-based, one per filter;
      names as proposed in the N5 round).
...
```text
1. W3X records the starting HEAD SHA.
2. Coder builds against THIS scope. Any conflict between this scope and
   the tree as found: state it and stop (I5); no silent adaptation. Module
   names are FINAL as listed (W3X-approved); any proposed deviation
   requires W3X approval BEFORE transformation.
