...
(0); self-reference and inputs list corrected.
...
Active scope:
    ...
    AVX/YMM, so there is nothing to prove here; real proof owed at Stage 4C/5C),
...
Inputs supplied:
    - this scope (Deblock4_Scope_Stage_1B2_v1_3.md);
...
4C/5C, when a v3 kernel exists). This is a recorded owed item, not a pass.
```
...
B. SEPARATE RUNTIME AVX/YMM OS-STATE REQUIREMENT (not a level member)
   OSXSAVE present AND XGETBV/XCR0 XMM+YMM enablement, required for the AVX
   tiers. Stated SEPARATELY from level membership (README 12.3, 12.4;
   decisions 4.6) - it is an OS-state check, not a CPUID feature-membership bit.
...
[V5] NEW vzeroupper finding (Part 3.2): the report records that the v3 probe
     ...
     was not applicable; the real transition proof is DEFERRED to Stage 4C/5C.
...
[ ] within-level classification of all FOUR objects (generic->v1, scalar->v1,
    sse41->v2, avx2->v3) confirms each stays inside its level (no AVX-512
    anywhere; no VEX/AVX in the v1 or v2 objects), failing loudly otherwise.
[ ] the vzeroupper transition is recorded as DEFERRED per Part 3.2 (v3 probe
    emits no AVX/YMM; real proof owed at Stage 4C/5C) - not claimed as settled.
[ ] Deblock4_Stage_1B2_WithinLevel_Report.md produced with contents A-F of
    Part 3.3, keeping the three feature sets DISTINCT (A psABI runtime-checkable
    membership; B separate OSXSAVE/XGETBV/XCR0 OS-state; C Zig resolved set for
    reproducibility only, NOT imposed on the 1B.3 guard); sets from the toolchain.
...
```text
FLAG 0 (W3X-owed before issue, from W3C first-response): (a) fill the exact
  Starting commit in the header; (b) ratify FLAG 1 below. W3C accepted the tier
  authority (section 1.1) and the source-verified changes; these two items are
  the only blockers to issue.
