...
below and nothing else. v1.1 pins three implementation details after external
audit: the force-down input (DEBLOCK4_FORCE_DOWN env var with loud-fail
semantics, 5.1), the raw-inline-asm rule with a non-normative CPUID/XGETBV
reference shape (3.5), and the detection unit filename
(src/cpu_capability_detection.zig). No design change. Where this scope and the charter appear to differ, the
...
### 3.5 CPUID/XGETBV implementation rule (added v1.1)

CPUID and XGETBV are read via RAW INLINE ASSEMBLY written in this scope's
detection unit. Do NOT invent or assume a standard-library wrapper; if a
std facility appears to exist, it is not to be used here - the detection
contract must rest on explicit, visible instructions (G6 spirit), and
hallucinated wrappers are a known failure mode.

The following is a NON-NORMATIVE reference shape only (it has NOT been
compiled or verified by W3D; it is a candidate form, not canon). The coder
MUST verify the constraint semantics against Zig 0.16.0 - in particular the
handling of EBX as an output register, since RBX is callee-saved on Win64 and
historically special in inline asm - and the verified implementation is the
coder's deliverable, proven by this scope's own section 7 gates:
...
XGETBV is executed only on the OSXSAVE-confirmed path (3.4). If the coder's
verification finds the reference shape defective in any way, the corrected
form is used and the defect is reported back to W3X/W3D.
...
OFF real capability bits.

The forcing input mechanism is PINNED (v1.1): the environment variable
`DEBLOCK4_FORCE_DOWN`, read ONCE at instance initialization, inside the gated
module only. Semantics:

```text
    Accepted values:  the short tier forms "v1" and "v2" exactly (lower case).
                      (There is no "v3" value: forcing to the top tier is not
                      forcing; see clamping below for requests at/above actual.)
    Absent:           no forcing; the seam is inert even though compiled in.
    Present, invalid: LOUD failure at instance construction (creation error or
                      unmistakable stderr + refusal to proceed as if unforced).
                      A mis-typed test intent must never silently do nothing.
    At/above actual:  clamps to actual capability with a LOUD notice; the
                      structural mask-by-intersection rule below guarantees the
                      result can never exceed actual.
```
...
- src/cpu_capability_detection.zig       (new; CPUID/XGETBV raw asm, capability
                                          record, whole-level resolve)
