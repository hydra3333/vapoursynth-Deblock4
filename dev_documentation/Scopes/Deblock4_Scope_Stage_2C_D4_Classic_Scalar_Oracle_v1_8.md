...
**Date:** 2026-08-05
**Author:** W3D (designer)
**Status:** RELEASED AS THE REVIEW AUTHORITY; W3X ratified 2026-08-03 and,
on 2026-08-05, ratified the resolutions of the successor-W3C
pre-implementation review (F1 blocker resolved by D-2C-1..6; K30/K31
adopted; sections 7b/8/9/11 amended). SCOPE RELEASE and IMPLEMENTATION
RELEASE are separate W3X acts: do NOT begin implementation until W3X
EXPLICITLY RELEASES IMPLEMENTATION after the focused re-review of this
v1_8 resolves. Read with D3 v1_9. W3X alone releases, accepts and commits.
**Built from:** D0 v1_10, D1 (pinned r9 snapshot + provenance v1.4),
D2 v1_6, D3 v1_9, and Addendum A v1.2 (K26 sentinel fixtures) + Addendum
...
IN SCOPE
  ...
  5. Tests asserting EVERY D3 v1_9 obligation on its routed proof
...
OUT OF SCOPE (do not touch, do not "improve")
  ...
  - Parameter validation, creation-error strings and tier RESOLUTION are
    unchanged EXCEPT the five narrowly authorised items (D0 v1_10 section
    5): (1) the S1 float refusal, (2) the K29 integer-depth refusal
    (A1b), (3) the S5 backend-unavailable refusal, (4) the S5
    implementation-availability cap, and (5) the EFFECTIVE-over-
    availability precedence test. Items 1-3 are creation-error rows;
...
S4  ORACLE STATUS. Per the oracle-construction exception (charter G7;
    V&T 20.2; K9), this delivery is accepted against D3 v1_9's
...
Implement exactly what D2 v1_6 documents; D3 v1_9 judges it. The
...
D3 v1_9 in full: O-1, O-1b, O-1c (73 enumerated tuple cases + the
...
# 9. Binding Knowledge Checklist (D0 v1_10)
...
K30     first-class module discipline (charter C-STY-10) applies to the
        ...
        first-class reference to scaffolding identifiers. The textual
        first-class audit returning EMPTY is a DELIVERY OBLIGATION of
        this scope, named in the O/G crosswalk with its exact domain
        (the new modules plus every existing module this scope edits).
K31     stride units are explicit and checked EVEN IN SCALAR CODE
        (charter C-SIMD-03): VapourSynth strides are BYTE counts;
        either byte addressing with the unit named at the point of use,
        or exactly ONE conversion to sample stride naming both units
        and asserting exactness for the plane's bytesPerSample. Silent
        division forbidden; u8 and u16 alike. Proof rides the D3 O-6e
        canaries and O-8c/O-8h geometry tests plus code inspection.
```
...
Review THIS scope together with D3 v1_9 and Addenda A v1.2 and B v1.2. Report numbered findings on:
