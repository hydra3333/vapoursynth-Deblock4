...
shapes pinned (P1/P3). v1.3 resolves the W3C follow-up review (W3C_v1_1, R1-R5): version/requested-
backend/reason-rule pinned into the instance API and summary line (R1); test
step and standalone baseline-v1 detection inspection object added to the build
graph (R2); seed name-mapping and exclusion list provided with a propose-then-
approve completion flow (R3); environment acquisition mechanism and failure
mapping pinned (R4); validation deliverable filename pinned (R5). Where this
scope and the charter appear to differ, the charter wins and the discrepancy is
brought to W3X; do not resolve it in code.
...
New validation deliverable (R5, pinned): ONE root-level batch,

```text
    build_1B3_v1.bat        the complete Stage 1B.3 runner INCLUDING the
                            standing three-surface absence gate, the selftest
                            execution matrix, and the build-reject checks
                            (one file, root level, versioned family name -
                            matching the 1B.2 practice; the batch is stage
                            scaffolding under 2.6 and correctly carries the
                            stage-numbered name)
```
...
pub const plugin = struct {
    ...
    // Value set/updated by W3X at commit time; initial value below.
...
SEED TABLE AND COMPLETION FLOW (R3, pinned). The following W3D-provisional
seed is APPROVED as a starting point:

```text
    Name mapping (Zig feature tag <-> psABI/this-scope name):
        x87   <-> FPU
        cx16  <-> CMPXCHG16B
        sahf  <-> LAHF-SAHF
        bmi   <-> BMI1

    Exclusion candidates (present in Zig model sets, excluded from the
    membership comparison, each with its reason):
        64bit         architecture precondition, not a level-table member
        crc32         not a separate psABI row; covered by SSE4.2
        xsave         not a separate psABI row; OSXSAVE is the level row
        <tuning/codegen properties>  compiler tuning, not ISA membership

    Policy rows (this scope's side only, never expected from Zig sets):
        OSFXSR, SCE   OS-state policy rows (3.3)
```

W3C COMPLETES this table against the actual Zig 0.16.0 named-model feature
sets (the Stage 1B.2 model captures, supplied in the session package) and
submits the COMPLETE mapping + exclusion list in the delivery for W3X/W3D
approval BEFORE the implementing commit is accepted. W3C does not silently
classify anything outside the seed; every completion entry carries a one-line
reason, and any entry W3C is unsure of is flagged as a question rather than
classified.
...
with `<version>` sourced from `deblock4_config.plugin.version_string` (R1) and
`backend=` rendering the RequestedBackend enum value. ONLY when a reason
exists is a single reason clause appended. THE REASON RULE IS DETERMINISTIC
(R1, pinned):

```text
    1. If the force-down seam is ACTIVE (compiled, enabled, valid env value),
       the reason is the force-down, and it takes precedence over any hardware
       reason:
           reason=forced-down(<ceiling>) actual=<actual-tier-name>
    2. Otherwise, if the resolved ACTUAL tier is below v3, the reason lists
       ALL missing requirements of the level IMMEDIATELY ABOVE the resolved
       actual tier (that level only, not higher ones), in the order the
       features are listed in the 1.2 tables, comma-separated, followed by
       "absent, not <level>":
           reason=BMI2,LZCNT absent, not x86_64_v3
       The XCR0 set-B failure renders as the pseudo-feature name XCR0.YMM in
       that list.
    3. Otherwise (actual v3, no forcing) there is no reason clause.
```

Example:

```text
    deblock4: 0.1.0-dev selftest backend=auto tier=x86_64_v2_with_sse41 reason=BMI2 absent, not x86_64_v3
```
...
ACQUISITION MECHANISM AND FAILURE CONTRACT (R4, pinned): the value is read via
a NON-ALLOCATING bounded fixed-buffer mechanism (GetEnvironmentVariableW-class
API into a small stack buffer; no allocator, so no out-of-memory path exists).
Mapping of every outcome:

```text
    variable absent                          -> inert (no forcing)
    present and exactly "v1" or "v2"         -> that ceiling
    present but anything else                -> InvalidForceDownValue (LOUD)
       (wrong spelling, wrong case, overlong for the buffer, undecodable
        wide characters - all are "an invalid value" and are reported with
        detail in the loud message)
    any OTHER acquisition failure            -> @panic with a clear message
       (debug-only code; a loud crash at construction is correct and must
        never be silently swallowed or misreported as unforced)
```

The public error set is unchanged: error{ InvalidForceDownValue }.
...
```text
    ...
    pub const RequestedBackend = enum { auto, x86_64_v1, x86_64_v2, x86_64_v3 };
    ...
    pub fn initInstanceCapabilities(
        ...
        emits the 4.1 always-on summary line (using requested and the
        deblock4_config.plugin.version_string), emits the 4.2 dump when
        ...
        REQUESTED-BACKEND SEMANTICS IN 1B.3 (R1, pinned): `requested` is a
        typed enum (no string parsing at this layer; the filter stage maps
        user input to this enum later). In Stage 1B.3 it is CARRIED and
        REPORTED on the summary line only; it does NOT alter capability
        computation, and dispatch honouring/clamping of a requested backend is
        a later stage. The self-test passes .auto in its normal runs and at
        least one non-auto value to prove the line renders it.
```
...
- Add the `test` step (R2): a test artifact over the modules containing the
  7.1 unit tests, wired with the SAME `deblock4_build_options` options module
  as the other artifacts, runnable as `zig build test`.
- Add the standalone detection inspection object (R2, option A): an
  `addObject` of `src/cpu_capability_detection.zig`, artifact name
  `cpu_capability_detection`, with the target EXPLICITLY pinned to the
  baseline named model `x86_64` (v1) - the same mechanism the backend objects
  use - plus a named step `detection-object`. This object exists so 7.2's
  dumpbin /DISASM proof has a standalone artifact; it is not linked anywhere.
...
- The standalone detection object (section 6, step `detection-object`,
  baseline-v1 target) contains NO v2 or v3 instructions (dumpbin /DISASM
  inspection, the 1B.2 method and deny families). Detection is v1-only.
...
- build_1B3_v1.bat                        (R5: the single root-level runner;
                                           contains the G10 three-surface
                                           standing gate over both artifacts,
                                           positive control, build-reject
                                           checks, selftest matrix, unit-test
                                           invocation, and the 1B.2
                                           no-regression checks)
```

Session package (P2): assembled by W3X at handoff - exact commit/branch, the
charter v1.17, the current source files named in 2.1, the README/design spec
containing 13.6, the 1B.2 validation batch, and the Stage 1B.2 named-model
capture files (required by 3.6/R3). This scope's file
