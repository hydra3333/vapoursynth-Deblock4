...
src/deblock4_instance_creation.zig
    ...
    (routers are phase-3 stubs until then). Settled error messages on refusal.
```

Phase-2 acceptance: selection unit tests pass; an instance can be constructed
and freed (router may be a minimal stub returning a source-frame request) in a
Debug e2e smoke; constant-format refusal proven.
