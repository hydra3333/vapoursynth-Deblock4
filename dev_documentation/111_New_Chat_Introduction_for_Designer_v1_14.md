...
Immediate designer action:
    ...
    order. Phases 1, 2 AND 3a are accepted, green in all three modes, committed
    and pushed. Phase 3b was released and debugged through corrections
    v1_0..v1_12 (repository main = the v1_12-applied state; base artifact
    designer_interaction/deliveries/
    Deblock4_Stage_1C_Phase_3b_W3C_delivery_v1_12.zip). The proof matrix
    passes everything except one final gate: a REAL G6 violation (three G10
    markers PE-exported from the Debug DLL; root cause pub export fn =
    dllexport in the DLL compilation). The failing gate is CORRECT - never
    relax it; the fix is source-side. READ
    Deblock4_Stage_1C_Phase_3b_Coder_Resume_Brief_v1_0.md - it PREVAILS on
    current state and carries the directed fix and paid-for lessons. The
    historic Phase 3a review directives below record HOW 3a was verified:
    ...
    its release-absence proof remains 3b. (Both filters verified live under R78:
    byte-identical pass-through across tiers, all error cases with Deblock4's
    own messages.)
