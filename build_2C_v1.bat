...
set "TRANSCRIPT_TEMP=%TEMP%\Deblock4_build_2C_v1_%RANDOM%_%RANDOM%_full_output.log"
"%ComSpec%" /D /V:OFF /C call "%~f0" --worker >"%TRANSCRIPT_TEMP%" 2>&1
set "OUTER_EXIT_CODE=%ERRORLEVEL%"

set "TRANSCRIPT_DIR=%~dp0zig-out\inspection_2C"
if not exist "%TRANSCRIPT_DIR%" md "%TRANSCRIPT_DIR%" >nul 2>nul
set "TRANSCRIPT_FILE=%TRANSCRIPT_DIR%\build_2C_v1_full_output.log"
...
set "DIAGNOSTIC_INDEX=%TRANSCRIPT_DIR%\build_2C_v1_diagnostic_index.txt"
...
rem ============================================================================
rem Deblock4 Stage 2C complete proof matrix.
rem
rem Implements D4 v1.9 sections 7 and 7c: B1-B2, G1-G2, E1-E6, V1, S1-S3,
...
set "DEBLOCK4_INSPECTION_DIR=zig-out\inspection_2C"
set "DEBLOCK4_BUILD_ROOT=zig-out\stage_2c"
set "DEBLOCK4_CACHE_ROOT=.zig-cache\stage_2c"
...
set "DEBLOCK4_HOLYWU_RUNNER=!DEBLOCK4_PROJECT_PREFIX!tools\holywu_reference\run_stage_2c_holywu_reference.cmd"
if not defined DEBLOCK4_REFERENCE_SOURCE_ROOT set "DEBLOCK4_REFERENCE_SOURCE_ROOT=!DEBLOCK4_PROJECT_PREFIX!dev_documentation\reference\holywu_r9"
if not defined DEBLOCK4_VS_INCLUDE_ROOT set "DEBLOCK4_VS_INCLUDE_ROOT=!DEBLOCK4_PROJECT_PREFIX!third_party\vapoursynth\include"
...
if not exist "%DEBLOCK4_HOLYWU_RUNNER%" (echo ERROR: HolyWu reference runner missing & exit /b 1)
if not exist "!DEBLOCK4_REFERENCE_SOURCE_ROOT!\SHA256SUMS.txt" (echo ERROR: pinned HolyWu reference snapshot missing & exit /b 1)
if not exist "!DEBLOCK4_VS_INCLUDE_ROOT!\VapourSynth4.h" (echo ERROR: VapourSynth4.h missing from H0 include root & exit /b 1)
if not exist "!DEBLOCK4_VS_INCLUDE_ROOT!\VSHelper4.h" (echo ERROR: VSHelper4.h missing from H0 include root & exit /b 1)
...
if /I not "!OBSERVED_ZIG_VERSION!"=="0.16.0" (
    echo ERROR: Stage 2C requires Zig 0.16.0.
    exit /b 1
)

echo === Prepare Stage 2C proof directories
...
if errorlevel 1 goto :fail
echo === Stage 2C proof directories ready
...
rem T-S5-3/T-S5-5 Debug force-down cases. Classic always executes v1.
...
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" valid_auto x86_64_v1_baseline STAGE_2C_CLASSIC_PASS "!MODE_DIR!\classic_force_v2.txt" 1 1
call :find_present "Classic force-v2 implementation cap" "!MODE_DIR!\classic_force_v2.txt" "tier=x86_64_v1_baseline reason=intentionally-capped(x86_64_v1_baseline) actual=x86_64_v3_with_avx2"
...
>>"!DEBLOCK4_INSPECTION_DIR!\proof_matrix_summary.txt" echo STAGE2_D3_O_G K31 S5 H0_H6 PASS
...
set "E2E_DIR=%~1"
set "DEBLOCK4_FORCE_DOWN="
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" valid_auto x86_64_v1_baseline STAGE_2C_CLASSIC_PASS "!E2E_DIR!\classic_valid_auto.txt" 1 1
...
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" valid_coercion x86_64_v1_baseline STAGE_2C_CLASSIC_PASS "!E2E_DIR!\classic_valid_coercion.txt" 1 1
...
call :run_vpy_case "!DEBLOCK4_CLASSIC_SCRIPT!" n04 x86_64_v1_baseline STAGE_2C_N04_PASS "!E2E_DIR!\classic_n04.txt" 1 1
call :find_present "N04 exact implementation cap summary" "!E2E_DIR!\classic_n04.txt" "tier=x86_64_v1_baseline reason=intentionally-capped(x86_64_v1_baseline) actual=x86_64_v3_with_avx2"
exit /b !ERRORLEVEL!
...
:run_holywu_reference
set "MARKER=H0-H6 isolated HolyWu r9 scalar differential"
echo === !MARKER!
set "DEBLOCK4_PLUGIN_PATH=!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_BUILD_ROOT!\ReleaseSafe\bin\Deblock4.dll"
set "DEBLOCK4_HOLYWU_INSPECTION_DIR=!DEBLOCK4_PROJECT_PREFIX!!DEBLOCK4_INSPECTION_DIR!\holywu_reference"
set "CMD=call tools\holywu_reference\run_stage_2c_holywu_reference.cmd"
call "tools\holywu_reference\run_stage_2c_holywu_reference.cmd"
set "exit_code=!ERRORLEVEL!"
if not "!exit_code!"=="0" goto :fail
exit /b 0
